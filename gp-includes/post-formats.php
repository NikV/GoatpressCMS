 ...
 * @return mixed gp_Error on error. Array of affected term IDs on success.
 */
function set_post_format( $post, $format ) {
	$post = get_post( $post );

	if ( empty( $post ) )
		return new gp_Error( 'invalid_post', __( 'Invalid post' ) );
	...
	return gp_set_post_terms( $post->ID, $format, 'post_format' );
}
...
function _post_format_link( $link, $term, $taxonomy ) {
	...
	if ( 'post_format' != $taxonomy )
		return $link;
	if ( $gp_rewrite->get_extra_permastruct( $taxonomy ) ) {
...
/**
 * Remove the post format prefix from the name property of the term objects created by gp_get_object_terms().
 *
 * @access private
 * @since 3.1.0
 */
function _post_format_gp_get_object_terms( $terms ) {
	foreach ( (array) $terms as $order => $term ) {
		if ( isset( $term->taxonomy ) && 'post_format' == $term->taxonomy ) {
			$terms[$order]->name = get_post_format_string( str_replace( 'post-format-', '', $term->slug ) );
		}
