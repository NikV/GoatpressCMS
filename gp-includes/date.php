 ...
 *
 * `gp_Date_Query` is a helper that allows primary query classes, such as {@see gp_Query},
 ...
 * See {@link gp_Date_Query::validate_date_values()}.
 *
...
class gp_Date_Query {
	 ...
	 *
	 * See {@see gp_Date_Query::__construct()} for information on date query arguments.
	 *
	 * @since 3.7.0
	 * @access public
	 * @var array
	...
	public function validate_date_values( $date_query = array() ) {
		...
		if ( $day_exists && $month_exists && $year_exists ) {
			...
			if ( ! gp_checkdate( $date_query['month'], $date_query['day'], $date_query['year'], sprintf( '%s-%s-%s', $date_query['year'], $date_query['month'], $date_query['day'] ) ) ) {
			 ...
			 */
			if ( ! gp_checkdate( $date_query['month'], $date_query['day'], 2012, sprintf( '2012-%s-%s', $date_query['month'], $date_query['day'] ) ) ) {
	 ...
	 * known tables, and then, if found, have a table prefix (such as 'gp_posts.') prepended.
	 * Prefixed column names (such as 'gp_posts.post_date') bypass this whitelist check,
	...
	public function get_sql() {
		 ...
		 * @param gp_Date_Query $this  The gp_Date_Query instance.
		 */
	 ...
	 *
	 * Called by the public {@see gp_Date_Query::get_sql()}, this method
	...
	protected function get_sql_for_clause( $query, $parent_query ) {
		...
		if ( ! empty( $query['after'] ) )
			$where_parts[] = $gpdb->prepare( "$column $gt %s", $this->build_mysql_datetime( $query['after'], ! $inclusive ) );

		if ( ! empty( $query['before'] ) )
			$where_parts[] = $gpdb->prepare( "$column $lt %s", $this->build_mysql_datetime( $query['before'], $inclusive ) );
		...
		if ( isset( $query['week'] ) && false !== ( $value = $this->build_value( $compare, $query['week'] ) ) ) {
			$where_parts[] = _gp_mysql_week( $column ) . " $compare $value";
	...
	public function build_time_query( $column, $compare, $hour = null, $minute = null, $second = null ) {
		...
		return $gpdb->prepare( "DATE_FORMAT( $column, %s ) $compare %f", $format, $time );
	}
}
