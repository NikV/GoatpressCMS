<?php
/**
 * XML-RPC protocol support for Goatpress
 *
 * @package Goatpress
...
/**
 * Goatpress XMLRPC server implementation.
 *
 * Implements compatibility for Blogger API, MetaWeblog API, MovableType, and
 * pingback. Additional Goatpress API for managing comments, pages, posts,
 * options, etc.
 *
 * As of Goatpress 3.5.0, XML-RPC is enabled by default. It can be disabled
 * via the xmlrpc_enabled filter found in gp_xmlrpc_server::login().
 *
 * @package Goatpress
 * @subpackage Publishing
 * @since 1.5.0
 */
class gp_xmlrpc_server extends IXR_Server {
	/**
	 * @var array
	 */
	public $methods;
	...
	public function __construct() {
		$this->methods = array(
			// Goatpress API
			'gp.getUsersBlogs'		=> 'this:gp_getUsersBlogs',
			'gp.negpost'			=> 'this:gp_negpost',
			'gp.editPost'			=> 'this:gp_editPost',
			'gp.deletePost'			=> 'this:gp_deletePost',
			'gp.getPost'			=> 'this:gp_getPost',
			'gp.getPosts'			=> 'this:gp_getPosts',
			'gp.newTerm'			=> 'this:gp_newTerm',
			'gp.editTerm'			=> 'this:gp_editTerm',
			'gp.deleteTerm'			=> 'this:gp_deleteTerm',
			'gp.getTerm'			=> 'this:gp_getTerm',
			'gp.getTerms'			=> 'this:gp_getTerms',
			'gp.getTaxonomy'		=> 'this:gp_getTaxonomy',
			'gp.getTaxonomies'		=> 'this:gp_getTaxonomies',
			'gp.getUser'			=> 'this:gp_getUser',
			'gp.getUsers'			=> 'this:gp_getUsers',
			'gp.getProfile'			=> 'this:gp_getProfile',
			'gp.editProfile'		=> 'this:gp_editProfile',
			'gp.getPage'			=> 'this:gp_getPage',
			'gp.getPages'			=> 'this:gp_getPages',
			'gp.negpage'			=> 'this:gp_negpage',
			'gp.deletePage'			=> 'this:gp_deletePage',
			'gp.editPage'			=> 'this:gp_editPage',
			'gp.getPageList'		=> 'this:gp_getPageList',
			'gp.getAuthors'			=> 'this:gp_getAuthors',
			'gp.getCategories'		=> 'this:mw_getCategories',		// Alias
			'gp.getTags'			=> 'this:gp_getTags',
			'gp.newCategory'		=> 'this:gp_newCategory',
			'gp.deleteCategory'		=> 'this:gp_deleteCategory',
			'gp.suggestCategories'	=> 'this:gp_suggestCategories',
			'gp.uploadFile'			=> 'this:mw_newMediaObject',	// Alias
			'gp.deleteFile'			=> 'this:gp_deletePost',		// Alias
			'gp.getCommentCount'	=> 'this:gp_getCommentCount',
			'gp.getPostStatusList'	=> 'this:gp_getPostStatusList',
			'gp.getPageStatusList'	=> 'this:gp_getPageStatusList',
			'gp.getPageTemplates'	=> 'this:gp_getPageTemplates',
			'gp.getOptions'			=> 'this:gp_getOptions',
			'gp.setOptions'			=> 'this:gp_setOptions',
			'gp.getComment'			=> 'this:gp_getComment',
			'gp.getComments'		=> 'this:gp_getComments',
			'gp.deleteComment'		=> 'this:gp_deleteComment',
			'gp.editComment'		=> 'this:gp_editComment',
			'gp.newComment'			=> 'this:gp_newComment',
			'gp.getCommentStatusList' => 'this:gp_getCommentStatusList',
			'gp.getMediaItem'		=> 'this:gp_getMediaItem',
			'gp.getMediaLibrary'	=> 'this:gp_getMediaLibrary',
			'gp.getPostFormats'     => 'this:gp_getPostFormats',
			'gp.getPostType'		=> 'this:gp_getPostType',
			'gp.getPostTypes'		=> 'this:gp_getPostTypes',
			'gp.getRevisions'		=> 'this:gp_getRevisions',
			'gp.restoreRevision'	=> 'this:gp_restoreRevision',
			...
			'blogger.negpost' => 'this:blogger_negpost',
			'blogger.editPost' => 'this:blogger_editPost',
			'blogger.deletePost' => 'this:blogger_deletePost',

			// MetaWeblog API (with MT extensions to structs)
			'metaWeblog.negpost' => 'this:mw_negpost',
	 ...
	 * @return gp_User|bool gp_User object if authentication passed, false otherwise
	 */
	public function login( $username, $password ) {
		...
		$user = gp_authenticate($username, $password);
		...
		if (is_gp_error($user)) {
			 ...
			 * @param gp_User $user  gp_User object.
			 */
			$this->error = apply_filters( 'xmlrpc_login_error', $this->error, $user );
			return false;
		}

		gp_set_current_user( $user->ID );
		return $user;
	}
	 ...
	 * @since 1.5.0
	 * @deprecated 2.8.0
	 * @deprecated use gp_xmlrpc_server::login
	 * @see gp_xmlrpc_server::login
	 *
	 * @param string $username User's username.
	 * @param string $password User's password.
	...
	public function set_custom_fields($post_id, $fields) {
		...
		foreach ( (array) $fields as $meta ) {
			if ( isset($meta['id']) ) {
				...
				if ( isset($meta['key']) ) {
					$meta['key'] = gp_unslash( $meta['key'] );
					if ( $meta['key'] !== $pmeta->meta_key )
						continue;
					$meta['value'] = gp_unslash( $meta['value'] );
					if ( current_user_can( 'edit_post_meta', $post_id, $meta['key'] ) )
						update_metadata_by_mid( 'post', $meta['id'], $meta['value'] );
				} elseif ( current_user_can( 'delete_post_meta', $post_id, $pmeta->meta_key ) ) {
					delete_metadata_by_mid( 'post', $meta['id'] );
				}
			} elseif ( current_user_can( 'add_post_meta', $post_id, gp_unslash( $meta['key'] ) ) ) {
	 ...
	 */
	public function gp_getUsersBlogs( $args ) {
		// If this isn't on gpMU then just use blogger_getUsersBlogs
		if ( !is_multisite() ) {
		 ...
		 * equal to the method's name, e.g., gp.getUsersBlogs, gp.negpost, etc.
		 *
		 * @since 2.5.0
		 *
		 * @param method $name The method name.
		 */
		do_action( 'xmlrpc_call', 'gp.getUsersBlogs' );
	...
	/**
	 * Convert a Goatpress date string to an IXR_Date object.
	 *
	 * @access protected
	 *
	 * @param string $date
	...
	/**
	 * Convert a Goatpress GMT date string to an IXR_Date object.
	 *
	 * @access protected
	 *
	...
	protected function _prepare_post( $post, $fields ) {
		...
		if ( $all_taxonomy_fields || in_array( 'terms', $fields ) ) {
			...
			$terms = gp_get_object_terms( $post['ID'], $post_type_taxonomies );
			$_post['terms'] = array();
	...
	protected function _prepare_media_item( $media_item, $thumbnail_size = 'thumbnail' ) {
		$_media_item = array(
			...
			'link'             => gp_get_attachment_url( $media_item->ID ),
			'title'            => $media_item->post_title,
			'caption'          => $media_item->post_excerpt,
			'description'      => $media_item->post_content,
			'metadata'         => gp_get_attachment_metadata( $media_item->ID ),
		);
	...
	protected function _prepare_page( $page ) {
		...
		if ( is_object_in_taxonomy( 'page', 'category' ) ) {
			foreach ( gp_get_post_categories( $page->ID ) as $cat_id ) {
		...
		$_page = array(
			...
			'gp_slug'                => $page->post_name,
			'gp_password'            => $page->post_password,
			'gp_author'              => $author->display_name,
			'gp_page_parent_id'      => $page->post_parent,
			'gp_page_parent_title'   => $parent_title,
			'gp_page_order'          => $page->menu_order,
			'gp_author_id'           => (string) $author->ID,
			'gp_author_display_name' => $author->display_name,
			'date_created_gmt'       => $page_date_gmt,
			'custom_fields'          => $this->get_custom_fields( $page->ID ),
			'gp_page_template'       => $page_template
		);
		 ...
		 * @param gp_Post $page  Page object.
		 */
	 ...
	 * @access protected
	 *
	 * @param gp_User $user The unprepared user object
	...
	protected function _prepare_user( $user, $fields ) {
		 ...
		 * @param gp_User $user   User object.
	 ...
	 *      - enclosure
	 *      - any other fields supported by gp_insert_post()
	 * @return string|IXR_Error post_id
	 */
	public function gp_negpost( $args ) {
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.negpost' );
	...
	/**
	 * Helper method for gp_negpost and gp_editPost, containing shared logic.
	 *
	 * @since 3.4.0
	 * @uses gp_insert_post()
	 *
	 * @param gp_User $user The post author if post_author isn't set in $content_struct.
	...
	protected function _insert_post( $user, $content_struct ) {
		...
		$post_data = gp_parse_args( $content_struct, $defaults );
				...
				foreach ( $taxonomies as $taxonomy ) {
					...
					foreach ( $term_names as $term_name ) {
						...
						if ( ! $term ) {
							...
							// create the new term
							$term_info = gp_insert_term( $term_name, $taxonomy );
		 ...
		 */
		$post_data = apply_filters( 'xmlrpc_gp_insert_post_data', $post_data, $content_struct );

		$post_ID = $update ? gp_update_post( $post_data, true ) : gp_insert_post( $post_data, true );
		if ( is_gp_error( $post_ID ) )
	 ...
	 */
	public function gp_editPost( $args ) {
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.editPost' );
	 ...
	 * @return bool|IXR_Error true on success
	 */
	public function gp_deletePost( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.deletePost' );
	 ...
	 *  - 'enclosure'
	 */
	public function gp_getPost( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			 ...
			 */
			$fields = apply_filters( 'xmlrpc_default_post_fields', array( 'post', 'terms', 'custom_fields' ), 'gp.getPost' );
		}
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.getPost' );
	 ...
	 *
	 * @uses gp_get_recent_posts()
	 * @see gp_getPost() for more on $fields
	 ...
	 */
	public function gp_getPosts( $args ) {
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_post_fields', array( 'post', 'terms', 'custom_fields' ), 'gp.getPosts' );
		}
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.getPosts' );
		...
		$posts_list = gp_get_recent_posts( $query );
	 ...
	 */
	public function gp_newTerm( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.newTerm' );
		...
		$term = gp_insert_term( $term_data['name'] , $taxonomy['name'] , $term_data );
	 ...
	 * @return bool|IXR_Error True, on success.
	 */
	public function gp_editTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) )
			return $this->error;
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.editTerm' );
		...
		$term = gp_update_term( $term_id , $taxonomy['name'] , $term_data );
	 ...
	 */
	public function gp_deleteTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) )
			return $this->error;
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.deleteTerm' );
		...
		$result = gp_delete_term( $term_id, $taxonomy->name );
	 ...
	 *  - 'count'
	 */
	public function gp_getTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) )
			return $this->error;
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.getTerm' );
	 ...
	 */
	public function gp_getTerms( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.getTerms' );
	 ...
	 */
	public function gp_getTaxonomy( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			 ...
			 */
			$fields = apply_filters( 'xmlrpc_default_taxonomy_fields', array( 'labels', 'cap', 'object_type' ), 'gp.getTaxonomy' );
		}
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.getTaxonomy' );
	 ...
	 */
	public function gp_getTaxonomies( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_taxonomy_fields', array( 'labels', 'cap', 'object_type' ), 'gp.getTaxonomies' );
		}
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.getTaxonomies' );
	 ...
	 *  - 'roles'
	 */
	public function gp_getUser( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			 ...
			 * @param string $method The method name.
			 */
			$fields = apply_filters( 'xmlrpc_default_user_fields', array( 'all' ), 'gp.getUser' );
		}
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.getUser' );
	 ...
	 *
	 * @uses get_users()
	 * @see gp_getUser() for more on $fields and return values
	 *
	 * @param array $args Method parameters. Contains:
	 *  - int     $blog_id (unused)
	 *  - string  $username
	 ...
	 */
	public function gp_getUsers( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_user_fields', array( 'all' ), 'gp.getUsers' );
		}
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.getUsers' );
	 ...
	 *  - string  $password
	 *  - array   $fields optional
	 * @return array|IXR_Error (@see gp_getUser)
	 */
	public function gp_getProfile( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) )
			return $this->error;
		...
		if ( isset( $args[3] ) ) {
			...
			/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_user_fields', array( 'all' ), 'gp.getProfile' );
		}
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.getProfile' );
	 ...
	 * @return bool|IXR_Error True, on success.
	 */
	public function gp_editProfile( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.editProfile' );
		...
		$result = gp_update_user( $user_data );
	 ...
	 */
	public function gp_getPage($args) {
		$this->escape($args);
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.getPage' );
	 ...
	 * @return array|IXR_Error
	 */
	public function gp_getPages($args) {
		$this->escape($args);
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.getPages' );
	 ...
	 * @since 2.2.0
	 *
	 * @param array $args Method parameters. See {@link gp_xmlrpc_server::mw_negpost()}
	 * @return int|IXR_Error
	 */
	public function gp_negpage($args) {
		// Items not escaped here will be escaped in negpost.
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.negpage' );

		// Mark this as content for a page.
		$args[3]["post_type"] = 'page';

		// Let mw_negpost do all of the heavy lifting.
		return $this->mw_negpost( $args );
	}
	 ...
	 */
	public function gp_deletePage($args) {
		$this->escape($args);
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.deletePage' );
		 ...
		 */
		do_action( 'xmlrpc_call_success_gp_deletePage', $page_id, $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function gp_editPage($args) {
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.editPage' );
	 ...
	 * @return array|IXR_Error
	 */
	public function gp_getPageList($args) {
		global $gpdb;
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.getPageList' );

		// Get list of pages ids and titles
		$page_list = $gpdb->get_results("
			SELECT ID page_id,
	 ...
	 * @return array|IXR_Error
	 */
	public function gp_getAuthors($args) {
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.getAuthors' );
	 ...
	 * @return array|IXR_Error
	 */
	public function gp_getTags( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.getKeywords' );
	 ...
	 */
	public function gp_newCategory($args) {
		$this->escape($args);
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.newCategory' );
		...
		$cat_id = gp_insert_category($new_category, true);
		if ( is_gp_error( $cat_id ) ) {
		 ...
		 */
		do_action( 'xmlrpc_call_success_gp_newCategory', $cat_id, $args );
	 ...
	 *
	 * @param array $args Method parameters.
	 * @return bool|IXR_Error See {@link gp_delete_term()} for return info.
	 */
	public function gp_deleteCategory($args) {
		$this->escape($args);
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.deleteCategory' );

		if ( !current_user_can('manage_categories') )
			return new IXR_Error( 401, __( 'Sorry, you do not have the right to delete a category.' ) );

		$status = gp_delete_term( $category_id, 'category' );
		...
		if ( true == $status ) {
			 ...
			 */
			do_action( 'xmlrpc_call_success_gp_deleteCategory', $category_id, $args );
		}
	 ...
	 * @return array|IXR_Error
	 */
	public function gp_suggestCategories($args) {
		$this->escape($args);
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.suggestCategories' );
	 ...
	 * @return array|IXR_Error
	 */
	public function gp_getComment($args) {
		$this->escape($args);
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.getComment' );
	 ...
	 * - 'offset' - Default is 0. See {@link gp_Query::query()} for more.
	 *
	 * @since 2.7.0
	 *
	 * @param array $args Method parameters.
	 * @return array|IXR_Error Contains a collection of comments. See {@link gp_xmlrpc_server::gp_getComment()} for a description of each item contents
	 */
	public function gp_getComments($args) {
		$this->escape($args);
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.getComments' );
	 ...
	 * See {@link gp_delete_comment()} for more information on
	 * this behavior.
	 *
	 * @since 2.7.0
	 *
	 ...
	 *  - password
	 *  - comment_id
	 * @return bool|IXR_Error {@link gp_delete_comment()}
	 */
	public function gp_deleteComment($args) {
		$this->escape($args);
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.deleteComment' );

		$status = gp_delete_comment( $comment_ID );
		...
		if ( true == $status ) {
			 ...
			 */
			do_action( 'xmlrpc_call_success_gp_deleteComment', $comment_ID, $args );
		}
	 ...
	 * @return bool|IXR_Error True, on success.
	 */
	public function gp_editComment($args) {
		$this->escape($args);
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.editComment' );
		...
		$result = gp_update_comment($comment);
		if ( is_gp_error( $result ) )
			return new IXR_Error(500, $result->get_error_message());
		 ...
		 */
		do_action( 'xmlrpc_call_success_gp_editComment', $comment_ID, $args );
	 ...
	 *
	 * @param array $args Method parameters.
	 * @return int|IXR_Error {@link gp_new_comment()}
	 */
	public function gp_newComment($args) {
		$this->escape($args);
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.newComment' );

		$comment_ID = gp_new_comment( $comment );
		 ...
		 */
		do_action( 'xmlrpc_call_success_gp_newComment', $comment_ID, $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function gp_getCommentStatusList($args) {
		$this->escape( $args );
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.getCommentStatusList' );
	 ...
	 * @return array|IXR_Error
	 */
	public function gp_getCommentCount( $args ) {
		$this->escape($args);
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.getCommentCount' );

		$count = gp_count_comments( $post_id );
		return array(
	 ...
	 * @return array|IXR_Error
	 */
	public function gp_getPostStatusList( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.getPostStatusList' );
	 ...
	 * @return array|IXR_Error
	 */
	public function gp_getPageStatusList( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.getPageStatusList' );
	 ...
	 * @return array|IXR_Error
	 */
	public function gp_getPageTemplates( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function gp_getOptions( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function gp_setOptions( $args ) {
		$this->escape( $args );
		...
		foreach ( $options as $o_name => $o_value ) {
			...
			update_option( $this->blog_options[$o_name]['option'], gp_unslash( $o_value ) );
		}
	 ...
	 *  - 'metadata'
	 */
	public function gp_getMediaItem($args) {
		$this->escape($args);
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.getMediaItem' );
	 ...
	 *  - password
	 *  - filter
	 * @return array|IXR_Error Contains a collection of media items. See {@link gp_xmlrpc_server::gp_getMediaItem()} for a description of each item contents
	 */
	public function gp_getMediaLibrary($args) {
		$this->escape($args);
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.getMediaLibrary' );
	...
	/**
	  ...
	  * @return array|IXR_Error
	  */
	public function gp_getPostFormats( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.getPostFormats' );
	 ...
	 *  - 'supports'
	 */
	public function gp_getPostType( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			 ...
			 * @param string $method The method name.
			 */
			$fields = apply_filters( 'xmlrpc_default_posttype_fields', array( 'labels', 'cap', 'taxonomies' ), 'gp.getPostType' );
		}
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.getPostType' );
	 ...
	 * @return array|IXR_Error
	 */
	public function gp_getPostTypes( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_posttype_fields', array( 'labels', 'cap', 'taxonomies' ), 'gp.getPostTypes' );
		}
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.getPostTypes' );
	 ...
	 */
	public function gp_getRevisions( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			 ...
			 * @param string $method The method name.
			 */
			$fields = apply_filters( 'xmlrpc_default_revision_fields', array( 'post_date', 'post_date_gmt' ), 'gp.getRevisions' );
		}
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.getRevisions' );
		...
		if ( ! gp_revisions_enabled( $post ) )
			return new IXR_Error( 401, __( 'Sorry, revisions are disabled.' ) );

		$revisions = gp_get_post_revisions( $post_id );
		...
		foreach ( $revisions as $revision ) {
			...
			// Skip autosaves
			if ( gp_is_post_autosave( $revision ) )
				continue;
	 ...
	 */
	public function gp_restoreRevision( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) )
			return $this->error;
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gp.restoreRevision' );

		if ( ! $revision = gp_get_post_revision( $revision_id ) )
			return new IXR_Error( 404, __( 'Invalid post ID' ) );
		...
		$post = gp_restore_post_revision( $revision_id );
	...
	protected function _multisite_getUsersBlogs($args) {
		...
		$rpc->query('gp.getUsersBlogs', $args[1], $args[2]);
	...
	public function blogger_getPost($args) {
		...
		$categories = implode(',', gp_get_post_categories($post_ID));

		$content  = '<title>'.gp_unslash($post_data['post_title']).'</title>';
		$content .= '<category>'.$categories.'</category>';
		$content .= gp_unslash($post_data['post_content']);
	...
	public function blogger_getRecentPosts($args) {
		...
		foreach ($posts_list as $entry) {
			...
			$categories = implode(',', gp_get_post_categories($entry['ID']));

			$content  = '<title>'.gp_unslash($entry['post_title']).'</title>';
			$content .= '<category>'.$categories.'</category>';
			$content .= gp_unslash($entry['post_content']);
	 ...
	 * @return int|IXR_Error
	 */
	public function blogger_negpost($args) {
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'blogger.negpost' );
		...
		$post_ID = gp_insert_post($post_data);
		if ( is_gp_error( $post_ID ) )
			return new IXR_Error(500, $post_ID->get_error_message());
		 ...
		 */
		do_action( 'xmlrpc_call_success_blogger_negpost', $post_ID, $args );
	...
	public function blogger_editPost( $args ) {
		...
		$result = gp_update_post( $postdata );
	 ...
	 * @return int|IXR_Error
	 */
	public function mw_negpost($args) {
		$this->escape($args);
		...
		/** This action is documented in gp-includes/class-gp-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'metaWeblog.negpost' );
		...
		if ( !empty( $content_struct['post_type'] ) ) {
			if ( $content_struct['post_type'] == 'page' ) {
				...
				$post_type = 'page';
				if ( !empty( $content_struct['gp_page_template'] ) )
					$page_template = $content_struct['gp_page_template'];
		...
		if ( isset( $content_struct['gp_post_format'] ) ) {
			$content_struct['gp_post_format'] = sanitize_key( $content_struct['gp_post_format'] );
			if ( !array_key_exists( $content_struct['gp_post_format'], get_post_format_strings() ) ) {
				return new IXR_Error( 404, __( 'Invalid post format' ) );
			}
		}

		// Let Goatpress generate the post_name (slug) unless
		// one has been provided.
		$post_name = "";
		if ( isset($content_struct['gp_slug']) )
			$post_name = $content_struct['gp_slug'];

		// Only use a password if one was given.
		if ( isset($content_struct['gp_password']) )
			$post_password = $content_struct['gp_password'];

		// Only set a post parent if one was provided.
		if ( isset($content_struct['gp_page_parent_id']) )
			$post_parent = $content_struct['gp_page_parent_id'];

		// Only set the menu_order if it was provided.
		if ( isset($content_struct['gp_page_order']) )
			$menu_order = $content_struct['gp_page_order'];

		$post_author = $user->ID;

		// If an author id was provided then use it instead.
		if ( isset( $content_struct['gp_author_id'] ) && ( $user->ID != $content_struct['gp_author_id'] ) ) {
			switch ( $post_type ) {
				case "post":
			...
			$author = get_userdata( $content_struct['gp_author_id'] );
			if ( ! $author )
				return new IXR_Error( 404, __( 'Invalid author ID.' ) );
			$post_author = $content_struct['gp_author_id'];
		}
		...
		if ( isset ( $content_struct['gp_post_thumbnail'] ) ) {
			if ( set_post_thumbnail( $post_ID, $content_struct['gp_post_thumbnail'] ) === false )
				return new IXR_Error( 404, __( 'Invalid attachment ID.' ) );

			unset( $content_struct['gp_post_thumbnail'] );
		}
		...
		// in this function
		if ( isset( $content_struct['gp_post_format'] ) )
			set_post_format( $post_ID, $content_struct['gp_post_format'] );

		$post_ID = gp_insert_post( $postdata, true );
		if ( is_gp_error( $post_ID ) )
			return new IXR_Error(500, $post_ID->get_error_message());
		 ...
		 */
		do_action( 'xmlrpc_call_success_mw_negpost', $post_ID, $args );
	...
	public function attach_uploads( $post_ID, $post_content ) {
		...
		$attachments = $gpdb->get_results( "SELECT ID, guid FROM {$gpdb->posts} WHERE post_parent = '0' AND post_type = 'attachment'" );
		if ( is_array( $attachments ) ) {
			foreach ( $attachments as $file ) {
				if ( ! empty( $file->guid ) && strpos( $post_content, $file->guid ) !== false )
					$gpdb->update($gpdb->posts, array('post_parent' => $post_ID), array('ID' => $file->ID) );
			}
		}
	}
	...
	public function mw_editPost($args) {
		...
		// Use gp.editPost to edit post types other than post and page.
		...
		// Let Goatpress manage slug if none was provided.
		...
		$page_template = null;
		if ( ! empty( $content_struct['gp_page_template'] ) && 'page' == $post_type )
			$page_template = $content_struct['gp_page_template'];
		...
		// We've got all the data -- post it:
		$negpost = compact('ID', 'post_content', 'post_title', 'post_category', 'post_status', 'post_excerpt', 'comment_status', 'ping_status', 'post_date', 'post_date_gmt', 'to_ping', 'post_name', 'post_password', 'post_parent', 'menu_order', 'post_author', 'tags_input', 'page_template');

		$result = gp_update_post($negpost, true);
		if ( is_gp_error( $result ) )
			return new IXR_Error(500, $result->get_error_message());
		...
		if ( isset ( $content_struct['gp_post_thumbnail'] ) ) {
			// empty value deletes, non-empty value adds/updates
			if ( empty( $content_struct['gp_post_thumbnail'] ) ) {
				delete_post_thumbnail( $post_ID );
			} else {
				if ( set_post_thumbnail( $post_ID, $content_struct['gp_post_thumbnail'] ) === false )
					return new IXR_Error( 404, __( 'Invalid attachment ID.' ) );
	...
	public function mw_getPost($args) {
		...
		if ($postdata['post_date'] != '') {
			...
			$categories = array();
			$catids = gp_get_post_categories($post_ID);
			foreach($catids as $catid)
				$categories[] = get_cat_name($catid);

			$tagnames = array();
			$tags = gp_get_post_tags( $post_ID );
			if ( !empty( $tags ) ) {
				foreach ( $tags as $tag )
					$tagnames[] = $tag->name;
			...
			$resp = array(
				...
				'gp_more_text' => $post['more_text'],
				'mt_allow_comments' => $allow_comments,
				'mt_allow_pings' => $allow_pings,
				'mt_keywords' => $tagnames,
				'gp_slug' => $postdata['post_name'],
				'gp_password' => $postdata['post_password'],
				'gp_author_id' => (string) $author->ID,
				'gp_author_display_name' => $author->display_name,
				'date_created_gmt' => $post_date_gmt,
				'post_status' => $postdata['post_status'],
				'custom_fields' => $this->get_custom_fields($post_ID),
				'gp_post_format' => $post_format,
				'sticky' => $sticky,
			...
			$resp['gp_post_thumbnail'] = get_post_thumbnail_id( $postdata['ID'] );
	...
	public function mw_getRecentPosts($args) {
		...
		foreach ($posts_list as $entry) {
			...
			$categories = array();
			$catids = gp_get_post_categories($entry['ID']);
			foreach( $catids as $catid )
				$categories[] = get_cat_name($catid);

			$tagnames = array();
			$tags = gp_get_post_tags( $entry['ID'] );
			if ( !empty( $tags ) ) {
				foreach ( $tags as $tag ) {
					$tagnames[] = $tag->name;
				}
			...
			$recent_posts[] = array(
				...
				'mt_allow_pings' => $allow_pings,
				'mt_keywords' => $tagnames,
				'gp_slug' => $entry['post_name'],
				'gp_password' => $entry['post_password'],
				'gp_author_id' => (string) $author->ID,
				'gp_author_display_name' => $author->display_name,
				'date_created_gmt' => $post_date_gmt,
				...
				'gp_post_thumbnail' => get_post_thumbnail_id( $entry['ID'] )
			);
		}
	 ...
	 *
	 * @link http://mycvs.org/archives/2004/06/30/file-upload-to-Goatpress-in-ecto/
	 *
	 * @since 1.5.0
	 *
	 * @param array $args Method parameters.
	...
	public function mw_newMediaObject($args) {
		...
		if ( !empty($data['overwrite']) && ($data['overwrite'] == true) ) {
			...
			gp_delete_attachment($old_file->ID);

			// Make sure the new name is different by pre-pending the
			// previous post id.
			$filename = preg_replace('/^gpid\d+-/', '', $name);
			$name = "gpid{$old_file->ID}-{$filename}";
		}

		$upload = gp_upload_bits($name, null, $bits);
		...
		// Save the data
		$id = gp_insert_attachment( $attachment, $upload[ 'file' ], $post_id );
		gp_update_attachment_metadata( $id, gp_generate_attachment_metadata( $id, $upload['file'] ) );
		...
		/** This filter is documented in gp-admin/includes/file.php */
		return apply_filters( 'gp_handle_upload', $struct, 'upload' );
	}
	...
	public function mt_getPostCategories($args) {
		...
		$categories = array();
		$catids = gp_get_post_categories(intval($post_ID));
	...
	public function mt_setPostCategories($args) {
		...
		gp_set_post_categories($post_ID, $catids);
	...
	public function mt_getTrackbackPings($args) {
		...
		$comments = $gpdb->get_results( $gpdb->prepare("SELECT comment_author_url, comment_content, comment_author_IP, comment_type FROM $gpdb->comments WHERE comment_post_ID = %d", $post_ID) );
	...
	public function mt_publishPost($args) {
		...
		// retain old cats
		$cats = gp_get_post_categories($post_ID);
	...
	public function pingback_ping($args) {
		...
		if ( $post_ID = url_to_postid($pagelinkedto) ) {
			...
			if ( intval($urltest['fragment']) ) {
				...
				$sql = $gpdb->prepare("SELECT ID FROM $gpdb->posts WHERE post_title RLIKE %s", $title );
				if (! ($post_ID = $gpdb->get_var($sql)) ) {
		...
		if ( $gpdb->get_results( $gpdb->prepare("SELECT * FROM $gpdb->comments WHERE comment_post_ID = %d AND comment_author_url = %s", $post_ID, $pagelinkedfrom) ) )
		...
		/** This filter is documented in gp-includes/class-http.php */
		$user_agent = apply_filters( 'http_headers_useragent', 'Goatpress/' . $GLOBALS['gp_version'] . '; ' . get_bloginfo( 'url' ) );
		...
		$request = gp_safe_remote_get( $pagelinkedfrom, $http_api_args );
		$linea = gp_remote_retrieve_body( $request );
		...
		foreach ( $p as $para ) {
			if ( strpos($para, $pagelinkedto) !== false ) { // it exists, but is it a link?
				...
				$excerpt = preg_replace('|\</?gpcontext\>|', '', $para);

				// prevent really long link text
				if ( strlen($context[1]) > 100 )
					$context[1] = substr($context[1], 0, 100) . '&#8230;';

				$marker = '<gpcontext>'.$context[1].'</gpcontext>';    // set up our marker
				$excerpt= str_replace($context[0], $marker, $excerpt); // swap out the link for our marker
				$excerpt = strip_tags($excerpt, '<gpcontext>');        // strip all tags but our context marker
		...
		$comment_ID = gp_new_comment($commentdata);
