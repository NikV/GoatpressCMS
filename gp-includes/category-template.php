 ...
 * @return string|gp_Error A list of category parents on success, gp_Error on failure.
 */
...
function get_the_category( $id = false ) {
	...
	if ( ! $categories || is_gp_error( $categories ) )
		$categories = array();
 ...
 * @return string|gp_Error Category name on success, gp_Error on failure.
 */
...
function get_the_category_list( $separator = '', $parents='', $post_id = false ) {
	...
	$rel = ( is_object( $gp_rewrite ) && $gp_rewrite->using_permalinks() ) ? 'rel="category tag"' : 'rel="category"';
 ...
 *
 * Prior to v2.5 of Goatpress, category names were not supported.
 * Prior to v2.7, category slugs were not supported.
 * Prior to v2.7, only one category could be compared: in_category( $single_category ).
 * Prior to v2.7, this function could only be used in the Goatpress Loop.
 ...
 */
function gp_dropdown_categories( $args = '' ) {
	$defaults = array(
	 ...
	 */
	$output = apply_filters( 'gp_dropdown_cats', $output, $r );
 ...
 */
function gp_list_categories( $args = '' ) {
	$defaults = array(
	 ...
	 */
	$html = apply_filters( 'gp_list_categories', $output, $args );
...
function gp_tag_cloud( $args = '' ) {
	...
	if ( empty( $tags ) || is_gp_error( $tags ) )
		return;
	...
	$return = gp_generate_tag_cloud( $tags, $args ); // Here's where those top tags get sorted according to $args
	 ...
	 */
	$return = apply_filters( 'gp_tag_cloud', $return, $args );
 ...
 */
function gp_generate_tag_cloud( $tags, $args = '' ) {
	$defaults = array(
	...
	if ( $tags_sorted !== $tags ) {
		...
		if ( 'RAND' === $args['order'] ) {
			...
			if ( 'name' === $args['orderby'] ) {
				uasort( $tags, '_gp_object_name_sort_cb' );
			} else {
				uasort( $tags, '_gp_object_count_sort_cb' );
			}
	...
	if ( $args['filter'] ) {
		 ...
		 * to the $filter argument in gp_generate_tag_cloud().
		 *
		 * @since 2.3.0
		 *
		 * @see gp_generate_tag_cloud()
		 *
		 * @param array|string $return String containing the generated HTML tag cloud output
		 *                             or an array of tag links if the 'format' argument
		 *                             equals 'array'.
		 * @param array        $tags   An array of terms used in the tag cloud.
		 * @param array        $args   An array of gp_generate_tag_cloud() arguments.
		 */
		return apply_filters( 'gp_generate_tag_cloud', $return, $tags, $args );
	}
 ...
 * @access private
 */
function _gp_object_name_sort_cb( $a, $b ) {
 ...
 * @access private
 */
function _gp_object_count_sort_cb( $a, $b ) {
...
class Walker_Category extends Walker {
	 ...
	 * @param array  $args     An array of arguments. @see gp_list_categories()
	 ...
	 * @param array  $args   An array of arguments. Only uses 'list' for whether should append to output. @see gp_list_categories()
	 */
...
class Walker_CategoryDropdown extends Walker {
	 ...
	 *                         See {@see gp_dropdown_categories()}.
	 */
	public function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 ) {
 ...
 * @return string|bool|gp_Error A list of tags on success, false if there are no terms, gp_Error on failure.
 */
...
function term_description( $term = 0, $taxonomy = 'post_tag' ) {
	...
	return is_gp_error( $description ) ? '' : $description;
}
 ...
 * @return array|bool|gp_Error Array of term objects on success, false if there are no terms
 *                             or the post does not exist, gp_Error on failure.
 */
function get_the_terms( $post, $taxonomy ) {
	...
	if ( false === $terms ) {
		$terms = gp_get_object_terms( $post->ID, $taxonomy );
		gp_cache_add($post->ID, $terms, $taxonomy . '_relationships');
	}
	 ...
	 * @since 3.1.0
	 *
	 * @param array|gp_Error $terms    List of attached terms, or gp_Error on failure.
 ...
 * @param string $sep Optional. Separate items using this.
 * @param string $after Optional. After list.
 * @return string|bool|gp_Error A list of terms on success, false if there are no terms, gp_Error on failure.
 */
 ...
 * @param string $sep Optional. Separate items using this.
 * @param string $after Optional. After list.
 * @return false|null False on Goatpress error. Returns null when displaying.
 */
function the_terms( $id, $taxonomy, $before = '', $sep = ', ', $after = '' ) {
	$term_list = get_the_term_list( $id, $taxonomy, $before, $sep, $after );

	if ( is_gp_error( $term_list ) )
		return false;
 ...
 *
 * Prior to v2.7 of Goatpress, tags given as integers would also be checked against the post's tags' names and slugs (in addition to term_ids)
 * Prior to v2.7, this function could only be used in the Goatpress Loop.
 * As of 2.7, the function can be used anywhere if it is provided a post ID or post object.
 *
 * @since 2.6.0
