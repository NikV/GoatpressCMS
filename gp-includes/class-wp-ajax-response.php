...
class gp_Ajax_Response {
	...
	/**
	 * Constructor - Passes args to {@link gp_Ajax_Response::add()}.
	 *
	 * @since 2.1.0
	 * @see gp_Ajax_Response::add()
	 *
	 ...
	 * also possible to pass a gp_Error object in either the 'id' or 'data'
	 ...
	 * 'id' argument can be gp_Error or an integer.
	...
	public function add( $args = '' ) {
		...
		if ( is_gp_error( $data ) ) {
			foreach ( (array) $data->get_error_codes() as $code ) {
				$response .= "<gp_error code='$code'><![CDATA[" . $data->get_error_message( $code ) . "]]></gp_error>";
				...
				$response .= "<gp_error_data code='$code'$class>";
	...
	public function send() {
		...
		echo "<?xml version='1.0' encoding='" . get_option( 'blog_charset' ) . "' standalone='yes'?><gp_ajax>";
