 ...
 * @since 3.9.0
 */
function gp_underscore_audio_template() {
	$audio_types = gp_get_audio_extensions();
?>
<audio style="visibility: hidden"
	controls
	class="gp-audio-shortcode"
...
>
	...
	<source src="{{ data.model.src }}" type="{{ gp.media.view.settings.embedMimes[ data.model.src.split('.').pop() ] }}" />
	<# } #>

	<?php foreach ( $audio_types as $type ):
	?><# if ( ! _.isEmpty( data.model.<?php echo $type ?> ) ) { #>
	<source src="{{ data.model.<?php echo $type ?> }}" type="{{ gp.media.view.settings.embedMimes[ '<?php echo $type ?>' ] }}" />
	<# } #>
	<?php endforeach;
?></audio>
<?php
 ...
 * @since 3.9.0
 */
function gp_underscore_video_template() {
	$video_types = gp_get_video_extensions();
?>
<#  var w_rule = h_rule = '', classes = [],
		w, h, settings = gp.media.view.settings,
...
#>
<div style="{{ w_rule }}{{ h_rule }}" class="gp-video">
<video controls
	class="gp-video-shortcode {{ classes.join( ' ' ) }}"
	<# if ( w ) { #>width="{{ w }}"<# } #>
	<# if ( h ) { #>height="{{ h }}"<# } #>
	<?php
 ...
 * @since 3.5.0
 */
function gp_print_media_templates() {
	global $is_IE;
	$class = 'media-modal gp-core-ui';
	...
	<script type="text/html" id="tmpl-uploader-inline">
		...
		<?php if ( ! _device_can_upload() ) : ?>
			<h3 class="upload-instructions"><?php printf( __('The web browser on your device cannot be used to upload files. You may be able to use the <a href="%s">native app for your device</a> instead.'), 'https://apps.Goatpress.org/' ); ?></h3>
		<?php elseif ( is_multisite() && ! is_upload_space_available() ) : ?>
			<h3 class="upload-instructions"><?php _e( 'Upload Limit Exceeded' ); ?></h3>
			<?php
			/** This action is documented in gp-admin/includes/media.php */
		...
		<div class="attachment-media-view {{ data.orientation }}">
			<div class="thumbnail thumbnail-{{ data.type }}">
				...
				<# } else if ( 'video' === data.type ) {
					...
					} else if ( gp.media.view.settings.contentWidth ) {
						w_rule = 'width: ' + gp.media.view.settings.contentWidth + 'px;';
					}
					if ( data.height ) {
						h_rule = 'height: ' + data.height + 'px;';
					}
				#>
				<div style="{{ w_rule }}{{ h_rule }}" class="gp-media-wrapper gp-video">
	...
	<script type="text/html" id="tmpl-gallery-settings">
		...
		<label class="setting">
			...
			<select class="link-to"
				...
				<option value="post" <# if ( ! gp.media.galleryDefaults.link || 'post' == gp.media.galleryDefaults.link ) {
					#>selected="selected"<# }
				#>>
					<?php esc_attr_e('Attachment Page'); ?>
				</option>
				<option value="file" <# if ( 'file' == gp.media.galleryDefaults.link ) { #>selected="selected"<# } #>>
					<?php esc_attr_e('Media File'); ?>
				</option>
				<option value="none" <# if ( 'none' == gp.media.galleryDefaults.link ) { #>selected="selected"<# } #>>
					<?php esc_attr_e('None'); ?>
				</option>
			</select>
		</label>
			...
			<select class="columns" name="columns"
				...
				<?php for ( $i = 1; $i <= 9; $i++ ) : ?>
					<option value="<?php echo esc_attr( $i ); ?>" <#
						if ( <?php echo $i ?> == gp.media.galleryDefaults.columns ) { #>selected="selected"<# }
					#>>
						<?php echo esc_html( $i ); ?>
					</option>
				<?php endfor; ?>
	...
	<script type="text/html" id="tmpl-audio-details">
		<# var ext, html5types = {
			mp3: gp.media.view.settings.embedMimes.mp3,
			ogg: gp.media.view.settings.embedMimes.ogg
		}; #>

		<?php $audio_types = gp_get_audio_extensions(); ?>
		<div class="media-embed media-embed-details">
			<div class="embed-media-settings embed-audio-settings">
				<?php gp_underscore_audio_template() ?>
	...
	<script type="text/html" id="tmpl-video-details">
		<# var ext, html5types = {
			mp4: gp.media.view.settings.embedMimes.mp4,
			ogv: gp.media.view.settings.embedMimes.ogv,
			webm: gp.media.view.settings.embedMimes.webm
		}; #>

		<?php $video_types = gp_get_video_extensions(); ?>
		<div class="media-embed media-embed-details">
			<div class="embed-media-settings embed-video-settings">
				<div class="gp-video-holder">
				<#
				...
				<?php gp_underscore_video_template() ?>
				...
				<# _.each( data.attachments, function( attachment, index ) { #>
					<dl class="gallery-item">
						...
						<# if ( attachment.caption ) { #>
							<dd class="gp-caption-text gallery-caption">
								{{ attachment.caption }}
							</dd>
						<# } #>
					</dl>
