<?php
/**
 * Facilitates adding of the Goatpress editor as used on the Write and Edit screens.
 *
 * @package Goatpress
 * @since 3.3.0
 *
 * Private, not included by default. See gp_editor() in gp-includes/general-template.php.
 */

final class _gp_Editors {
	 ...
	 *
	 *     @type bool       $gpautop           Whether to use gpautop(). Default true.
	...
	public static function parse_settings( $editor_id, $settings ) {
		...
		/**
		 * Filter the gp_editor() settings.
		 *
		 * @since 4.0.0
		 *
		 * @see _gp_Editors()::parse_settings()
		 *
		 * @param array  $settings  Array of editor arguments.
		 * @param string $editor_id ID for the current editor instance.
		 */
		$settings = apply_filters( 'gp_editor_settings', $settings, $editor_id );

		$set = gp_parse_args( $settings, array(
			'gpautop'             => true,
			'media_buttons'       => true,
			'default_editor'      => '',
		...
		if ( self::$this_tinymce ) {
			if ( false !== strpos( $editor_id, '[' ) ) {
				...
				_deprecated_argument( 'gp_editor()', '3.9', 'TinyMCE editor IDs cannot have brackets.' );
			}
		}
		...
		if ( 'content' === $editor_id && empty( $set['tinymce']['gp_autoresize_on'] ) ) {
	...
	public static function editor( $content, $editor_id, $settings = array() ) {
		...
		$editor_class = ' class="' . trim( $set['editor_class'] . ' gp-editor-area' ) . '"';
		...
		if ( ! self::$this_quicktags && self::$this_tinymce ) {
			...
			$default_editor = $set['default_editor'] ? $set['default_editor'] : gp_default_editor();
			$autocomplete = ' autocomplete="off"';

			// 'html' is used for the "Text" editor tab.
			if ( 'html' === $default_editor ) {
				add_filter('the_editor_content', 'gp_htmledit_pre');
				$switch_class = 'html-active';
			} else {
				add_filter('the_editor_content', 'gp_richedit_pre');
				$switch_class = 'tmce-active';
			}

			$buttons .= '<button type="button" id="' . $editor_id . '-tmce" class="gp-switch-editor switch-tmce" onclick="switchEditors.switchto(this);">' . __('Visual') . "</button>\n";
			$buttons .= '<button type="button" id="' . $editor_id . '-html" class="gp-switch-editor switch-html" onclick="switchEditors.switchto(this);">' . _x( 'Text', 'Name for the Text editor tab (formerly HTML)' ) . "</button>\n";
		}

		$wrap_class = 'gp-core-ui gp-editor-wrap ' . $switch_class;

		if ( $set['_content_editor_dfw'] ) {
			$wrap_class .= ' has-dfw';
		}

		echo '<div id="gp-' . $editor_id . '-wrap" class="' . $wrap_class . '">';

		if ( self::$editor_buttons_css ) {
			gp_print_styles('editor-buttons');
		...
		if ( !empty($buttons) || $set['media_buttons'] ) {
			echo '<div id="gp-' . $editor_id . '-editor-tools" class="gp-editor-tools hide-if-no-js">';

			if ( $set['media_buttons'] ) {
				self::$has_medialib = true;

				if ( !function_exists('media_buttons') )
					include(ABSPATH . 'gp-admin/includes/media.php');

				echo '<div id="gp-' . $editor_id . '-media-buttons" class="gp-media-buttons">';
			...
			echo '<div class="gp-editor-tabs">' . $buttons . "</div>\n";
			echo "</div>\n";
		}
		 ...
		 */
		$the_editor = apply_filters( 'the_editor', '<div id="gp-' . $editor_id . '-editor-container" class="gp-editor-container">' .
	...
	public static function editor_settings($editor_id, $set) {
		...
		if ( empty(self::$first_init) ) {
			if ( is_admin() ) {
				...
				add_action( 'gp_print_footer_scripts', array( __CLASS__, 'editor_js' ), 50 );
				add_action( 'gp_print_footer_scripts', array( __CLASS__, 'enqueue_scripts' ), 1 );
			}
		}
		...
		if ( self::$this_tinymce ) {
			...
			if ( empty( self::$first_init ) ) {
				...
				if ( $set['teeny'] ) {
					 ...
					 * @param string $editor_id Unique editor identifier, e.g. 'content'.
					 */
					self::$plugins = $plugins = apply_filters( 'teeny_mce_plugins', array( 'colorpicker', 'lists', 'fullscreen', 'image', 'Goatpress', 'gpeditimage', 'gplink' ), $editor_id );
				} else {
					 ...
					 * 'myplugin' => 'http://mysite.com/gp-content/plugins/myfolder/mce_plugin.js'.
					 *
					 ...
					 * in Goatpress should be added to the TinyMCE instance.
					 *
					 * @since 3.3.0
					 *
					...
					if ( ! empty( $mce_external_plugins ) ) {
						 ...
						 *
						 * The language file should follow the same format as gp_mce_translation(),
						...
						foreach ( $mce_external_plugins as $name => $url ) {
							...
							if ( ! in_array( $name, $loaded_langs, true ) ) {
								...
								$path = gp_CONTENT_DIR . $path . '/langs/';
				...
				self::$first_init = array(
					...
					'formats' => "{
						alignleft: [
							...
							{selector: 'img,table,dl.gp-caption', classes: 'alignleft'}
						],
						aligncenter: [
							{selector: 'p,h1,h2,h3,h4,h5,h6,td,th,div,ul,ol,li', styles: {textAlign:'center'}},
							{selector: 'img,table,dl.gp-caption', classes: 'aligncenter'}
						],
						alignright: [
							{selector: 'p,h1,h2,h3,h4,h5,h6,td,th,div,ul,ol,li', styles: {textAlign:'right'}},
							{selector: 'img,table,dl.gp-caption', classes: 'alignright'}
						],
					...
					'entity_encoding' => 'raw',
					'keep_styles' => false,
					'cache_suffix' => 'gp-mce-' . $GLOBALS['tinymce_version'],

					// Limit the preview styles in the menu/toolbar
					'preview_styles' => 'font-family font-size font-weight font-style text-decoration text-transform',

					'end_container_on_empty_block' => true,
					'gpeditimage_disable_captions' => $no_captions,
					'gpeditimage_html5_captions' => current_theme_supports( 'html5', 'caption' ),
					'plugins' => implode( ',', $plugins ),
				);

				if ( ! empty( $mce_external_plugins ) ) {
					self::$first_init['external_plugins'] = gp_json_encode( $mce_external_plugins );
				}
				...
				// Goatpress default stylesheet and dashicons
				$mce_css = array(
					$dashicons,
					self::$baseurl . '/skins/Goatpress/gp-content.css?' . $version
				);
			...
			if ( $set['teeny'] ) {
				...
				$mce_buttons = array( 'bold', 'italic', 'strikethrough', 'bullist', 'numlist', 'blockquote', 'hr', 'alignleft', 'aligncenter', 'alignright', 'link', 'unlink', 'gp_more', 'spellchecker' );
			...
			if ( $post = get_post() ) {
				...
				if ( post_type_supports( $post->post_type, 'post-formats' ) ) {
					...
					if ( $post_format && ! is_gp_error( $post_format ) )
	...
	public static function enqueue_scripts() {
		gp_enqueue_script('word-count');

		if ( self::$has_tinymce )
			gp_enqueue_script('editor');

		if ( self::$has_quicktags ) {
			gp_enqueue_script( 'quicktags' );
			gp_enqueue_style( 'buttons' );
		}

		if ( in_array('gplink', self::$plugins, true) || in_array('link', self::$qt_buttons, true) ) {
			gp_enqueue_script('gplink');
		}

		if ( in_array('gpfullscreen', self::$plugins, true) || in_array('fullscreen', self::$qt_buttons, true) )
			gp_enqueue_script('gp-fullscreen');
		 ...
		 */
		do_action( 'gp_enqueue_editor', array(
	 ...
	 * Can be used directly (_gp_Editors::gp_mce_translation()) by passing the same locale as set in the TinyMCE init object.
	 *
	 ...
	 */
	public static function gp_mce_translation( $mce_locale = '', $json_only = false ) {
		 ...
		 */
		$mce_translation = apply_filters( 'gp_mce_translation', $mce_translation, $mce_locale );
		...
		if ( $json_only ) {
			return gp_json_encode( $mce_translation );
		}

		$baseurl = self::$baseurl ? self::$baseurl : includes_url( 'js/tinymce' );

		return "tinymce.addI18n( '$mce_locale', " . gp_json_encode( $mce_translation ) . ");\n" .
	...
	public static function editor_js() {
		 ...
		 */
		do_action( 'before_gp_tiny_mce', self::$mce_settings );
		?>
		...
		// Load tinymce.js when running from /src, else load gp-tinymce.js.gz (production) or tinymce.min.js (SCRIPT_DEBUG)
		$mce_suffix = false !== strpos( $GLOBALS['gp_version'], '-src' ) ? '' : '.min';

		if ( $tmce_on ) {
			if ( $compressed ) {
				echo "<script type='text/javascript' src='{$baseurl}/gp-tinymce.php?c=1&amp;$version'></script>\n";
			} else {
			...
			echo "<script type='text/javascript'>\n" . self::gp_mce_translation() . "</script>\n";

			if ( self::$ext_plugins ) {
				// Load the old-format English strings to prevent unsightly labels in old style popups
				echo "<script type='text/javascript' src='{$baseurl}/langs/gp-langs-en.js?$version'></script>\n";
			}
		}
		 ...
		 * @param array $mce_settings TinyMCE settings array.
		 */
		do_action( 'gp_tiny_mce_init', self::$mce_settings );
		...
		( function() {
			...
			if ( typeof tinymce !== 'undefined' ) {
				for ( edId in tinyMCEPreInit.mceInit ) {
					...
					wrapper = tinymce.DOM.select( '#gp-' + edId + '-wrap' )[0];
			...
			if ( typeof jQuery !== 'undefined' ) {
				jQuery('.gp-editor-wrap').on( 'click.gp-editor', function() {
					if ( this.id ) {
						window.gpActiveEditor = this.id.slice( 3, -5 );
					}
				});
			} else {
				for ( qtId in tinyMCEPreInit.qtInit ) {
					document.getElementById( 'gp-' + qtId + '-wrap' ).onclick = function() {
						window.gpActiveEditor = this.id.slice( 3, -5 );
					}
				}
			}
		 ...
		 * @param array $mce_settings TinyMCE settings array.
		 */
		do_action( 'after_gp_tiny_mce', self::$mce_settings );
	}

	public static function gp_fullscreen_html() {
		...
		?>
		<div id="gp-fullscreen-body" class="gp-core-ui<?php if ( is_rtl() ) echo ' rtl'; ?>" data-theme-width="<?php echo (int) $width; ?>" data-dfw-width="<?php echo (int) $dfw_width; ?>">
		<div id="fullscreen-topbar">
			<div id="gp-fullscreen-toolbar">
			<div id="gp-fullscreen-close"><a href="#" onclick="gp.editor.fullscreen.off();return false;"><?php _e('Exit fullscreen'); ?></a></div>
			<div id="gp-fullscreen-central-toolbar" style="width:<?php echo $width; ?>px;">

			<div id="gp-fullscreen-mode-bar">
				<div id="gp-fullscreen-modes" class="button-group">
					<a class="button gp-fullscreen-mode-tinymce" href="#" onclick="gp.editor.fullscreen.switchmode( 'tinymce' ); return false;"><?php _e( 'Visual' ); ?></a>
					<a class="button gp-fullscreen-mode-html" href="#" onclick="gp.editor.fullscreen.switchmode( 'html' ); return false;"><?php _ex( 'Text', 'Name for the Text editor tab (formerly HTML)' ); ?></a>
				</div>
			</div>

			<div id="gp-fullscreen-button-bar"><div id="gp-fullscreen-buttons" class="mce-toolbar">
		<?php
		...
		$buttons = array(
			...
			'gp-media-library' => array( 'title' => __('Media library (Alt + Shift + M)'), 'both' => true ),
		 ...
		 */
		$buttons = apply_filters( 'gp_fullscreen_buttons', $buttons );
		...
		foreach ( $buttons as $button => $args ) {
			...
			<div class="mce-widget mce-btn<?php if ( $args['both'] ) { ?> gp-fullscreen-both<?php } ?>">
			<button type="button" aria-label="<?php echo $title; ?>" title="<?php echo $title; ?>"<?php echo $onclick; ?> id="gp_fs_<?php echo $button; ?>">
		...
		<div id="gp-fullscreen-save">
			<input type="button" class="button button-primary right" value="<?php echo $save; ?>" onclick="gp.editor.fullscreen.save();" />
			<span class="gp-fullscreen-saved-message"><?php if ( $post && $post->post_status == 'publish' ) _e('Updated.'); else _e('Saved.'); ?></span>
			<span class="gp-fullscreen-error-message"><?php _e('Save failed.'); ?></span>
	...
	<div id="gp-fullscreen-statusbar">
		<div id="gp-fullscreen-status">
			<div id="gp-fullscreen-count"><?php printf( __( 'Word count: %s' ), '<span class="word-count">0</span>' ); ?></div>
			<div id="gp-fullscreen-tagline"><?php _e('Just write.'); ?></div>
		</div>
	</div>
	</div>
	 ...
	 */
	public static function gp_link_query( $args = array() ) {
		 ...
		 *
		 * @see gp_Query for a full list of arguments
		 *
		 * @since 3.7.0
		 *
		 * @param array $query An array of gp_Query arguments.
		 */
		$query = apply_filters( 'gp_link_query_args', $query );
		 ...
		 * @param array $query  An array of gp_Query arguments.
		 */
		return apply_filters( 'gp_link_query', $results, $query );
	}
	 ...
	 * @since 3.1.0
	 */
	public static function gp_link_dialog() {
		$search_panel_visible = '1' == get_user_setting( 'gplink', '0' ) ? ' search-panel-visible' : '';

		// display: none is required here, see #gp27605
		?>
		<div id="gp-link-backdrop" style="display: none"></div>
		<div id="gp-link-wrap" class="gp-core-ui<?php echo $search_panel_visible; ?>" style="display: none">
		<form id="gp-link" tabindex="-1">
		<?php gp_nonce_field( 'internal-linking', '_ajax_linking_nonce', false ); ?>
		<div id="link-modal-title">
			<?php _e( 'Insert/edit link' ) ?>
			<button type="button" id="gp-link-close"><span class="screen-reader-text"><?php _e( 'Close' ); ?></span></button>
	 	</div>
		<div id="link-selector">
			<div id="link-options">
				<p class="howto"><?php _e( 'Enter the destination URL' ); ?></p>
				<div class="gp-link-text-field">
					<label><span><?php _e( 'Link Text' ); ?></span><input id="gp-link-text" type="text" /></label>
				</div>
				<div>
					<label><span><?php _e( 'URL' ); ?></span><input id="gp-link-url" type="text" /></label>
				</div>
				<div class="link-target">
					<label><span>&nbsp;</span><input type="checkbox" id="gp-link-target" /> <?php _e( 'Open link in a new window/tab' ); ?></label>
				</div>
			</div>
			<p class="howto"><a href="#" id="gp-link-search-toggle"><?php _e( 'Or link to existing content' ); ?></a></p>
			<div id="search-panel">
				<div class="link-search-wrapper">
					<label>
						<span class="search-label"><?php _e( 'Search' ); ?></span>
						<input type="search" id="gp-link-search" class="link-search-field" autocomplete="off" />
						<span class="spinner"></span>
					</label>
				</div>
		...
		<div class="submitbox">
			<div id="gp-link-cancel">
				<a class="submitdelete deletion" href="#"><?php _e( 'Cancel' ); ?></a>
			</div>
			<div id="gp-link-update">
				<input type="submit" value="<?php esc_attr_e( 'Add Link' ); ?>" class="button button-primary" id="gp-link-submit" name="gp-link-submit">
			</div>
		</div>
		</form>
		</div>
