/* global tinymce, gpCookies, autosaveL10n, switchEditors */
// Back-compat
	...
	function autosave() {
		...
		function autosaveLocal() {
			...
			function checkStorage() {
				...
				try {
					window.sessionStorage.setItem( 'gp-test', test );
					result = window.sessionStorage.getItem( 'gp-test' ) === test;
					window.sessionStorage.removeItem( 'gp-test' );
				} catch(e) {}
			...
			function getStorage() {
				...
				if ( hasStorage && blog_id ) {
					stored_obj = sessionStorage.getItem( 'gp-autosave-' + blog_id );
			...
			function run() {
				...
				if ( $( '#gp-content-wrap' ).hasClass( 'tmce-active' ) ) {
				...
				$( 'form#post' ).on( 'submit.autosave-local', function() {
					...
					gpCookies.set( 'gp-saving-post', post_id + '-check', 24 * 60 * 60 );
				});
			}
			...
			function checkPost() {
				var content, post_title, excerpt, $notice,
					postData = getSavedPostData(),
					cookie = gpCookies.get( 'gp-saving-post' );

				if ( cookie === post_id + '-saved' ) {
					gpCookies.remove( 'gp-saving-post' );
			...
			function restorePost( postData ) {
				...
				if ( postData ) {
					...
					if ( editor && ! editor.isHidden() && typeof switchEditors !== 'undefined' ) {
						...
						editor.setContent( postData.content ? switchEditors.gpautop( postData.content ) : '' );
					} else {
		...
		function autosaveServer() {
			...
			function save() {
				...
				$document.trigger( 'gpcountwords', [ postData.content ] )
					.trigger( 'before-autosave', [ postData ] );

				postData._gpnonce = $( '#_gpnonce' ).val() || '';
			...
			$document.on( 'heartbeat-send.autosave', function( event, data ) {
				...
				if ( autosaveData ) {
					data.gp_autosave = autosaveData;
				}
			}).on( 'heartbeat-tick.autosave', function( event, data ) {
				if ( data.gp_autosave ) {
					response( data.gp_autosave );
				}
				...
				if ( 'timeout' === error || 603 === status ) {
					...
					if ( ! gp.autosave.local.hasStorage ) {
