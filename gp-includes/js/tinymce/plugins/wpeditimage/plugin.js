/* global tinymce */
tinymce.PluginManager.add( 'gpeditimage', function( editor ) {
	...
	editor.addButton( 'gp_img_remove', {
		tooltip: 'Remove',
	...
	each( {
		...
		editor.addButton( 'gp_img_' + name, {
			tooltip: tooltip,
			icon: 'dashicon dashicons-align-' + direction,
			cmd: 'alignnone' === name ? 'gpAlignNone' : 'Justify' + direction.slice( 0, 1 ).toUpperCase() + direction.slice( 1 ),
			onPostRender: function() {
				...
				editor.on( 'NodeChange', function( event ) {
					...
					node = editor.dom.getParent( event.element, '.gp-caption' ) || event.element;
	...
	editor.once( 'preinit', function() {
		toolbar = editor.gp._createToolbar( [
			'gp_img_alignleft',
			'gp_img_aligncenter',
			'gp_img_alignright',
			'gp_img_alignnone',
	...
	editor.on( 'gptoolbar', function( event ) {
	...
	function parseShortcode( content ) {
		return content.replace( /(?:<p>)?\[(?:gp_)?caption([^\]]+)\]([\s\S]+?)\[\/(?:gp_)?caption\](?:<\/p>)?/g, function( a, b, c ) {
			...
			if ( ! editor.getParam( 'gpeditimage_html5_captions' ) ) {
				width += 10;
			}

			return '<div class="mceTemp"><dl id="' + id + '" class="gp-caption ' + align + classes + '" style="width: ' + width + 'px">' +
				'<dt class="gp-caption-dt">'+ img +'</dt><dd class="gp-caption-dd">'+ caption +'</dd></dl></div>';
		});
	}
	...
	function getShortcode( content ) {
		return content.replace( /<div (?:id="attachment_|class="mceTemp)[^>]*>([\s\S]+?)<\/div>/g, function( a, b ) {
			...
			out = b.replace( /\s*<dl ([^>]+)>\s*<dt [^>]+>([\s\S]+?)<\/dt>\s*<dd [^>]+>([\s\S]*?)<\/dd>\s*<\/dl>\s*/gi, function( a, b, c, caption ) {
				...
				classes = classes.replace( /gp-caption ?|align[a-z]+ ?/gi, '' );
	...
	function extractImageData( imageNode ) {
		...
		tinymce.each( classes, function( name ) {
			...
			if ( /^gp-image/.test( name ) ) {
				metadata.attachment_id = parseInt( name.replace( 'gp-image-', '' ), 10 );
		...
		// Extract caption
		captionBlock = dom.getParents( imageNode, '.gp-caption' );
		...
		if ( captionBlock.length ) {
			...
			caption = dom.select( 'dd.gp-caption-dd', captionBlock );
			if ( caption.length ) {
				caption = caption[0];
	...
	function updateImage( imageNode, imageData ) {
		...
		if ( imageData.attachment_id ) {
			classes.push( 'gp-image-' + imageData.attachment_id );
		...
		if ( imageData.caption ) {
			...
			if ( captionNode ) {
				dl = dom.select( 'dl.gp-caption', captionNode );
				...
				dd = dom.select( '.gp-caption-dd', captionNode );
				...
				html =  '<dl ' + id + 'class="' + className +'" style="width: '+ width +'px">' +
					'<dt class="gp-caption-dt"></dt><dd class="gp-caption-dd">'+ imageData.caption +'</dd></dl>';
				...
				editor.$( wrap ).find( 'dt.gp-caption-dt' ).append( node );
			}
		} else if ( captionNode ) {
		...
		if ( gp.media.events ) {
			gp.media.events.trigger( 'editor:image-update', {
				editor: editor,
				metadata: imageData,
				image: imageNode
			} );
	...
	function editImage( img ) {
		...
		gp.media.events.trigger( 'editor:image-edit', {
			editor: editor,
			metadata: metadata,
			image: img
		} );
		...
		gp.media.events.trigger( 'editor:frame-create', { frame: frame } );
	...
	editor.on( 'init', function() {
		var dom = editor.dom,
			captionClass = editor.getParam( 'gpeditimage_html5_captions' ) ? 'html5-captions' : 'html4-captions';

		dom.addClass( editor.getBody(), captionClass );

		// Add caption field to the default image dialog
		editor.on( 'gpLoadImageForm', function( event ) {
			if ( editor.getParam( 'gpeditimage_disable_captions' ) ) {
				return;
			}
		...
		editor.on( 'gpNewImageRefresh', function( event ) {
			var parent, captionWidth;

			if ( parent = dom.getParent( event.node, 'dl.gp-caption' ) ) {
		...
		editor.on( 'gpImageFormSubmit', function( event ) {
			...
			if ( ! imgNode ) {
				...
				if ( caption ) {
					...
					html = '<dl class="gp-caption alignnone"' + captionWidth + '>' +
						'<dt class="gp-caption-dt">'+ html +'</dt><dd class="gp-caption-dd">'+ caption +'</dd></dl>';
				...
				wrap = dom.getParent( imgNode, 'dl.gp-caption' );

				if ( caption ) {
					if ( wrap ) {
						if ( parent = dom.select( 'dd.gp-caption-dd', wrap )[0] ) {
							parent.innerHTML = caption;
						}
					} else {
						if ( imgNode.className ) {
							captionId = imgNode.className.match( /gp-image-([0-9]+)/ );
						...
						captionAlign = ' class="gp-caption ' + captionAlign + '"';
						...
						html = '<dl ' + captionId + captionAlign + captionWidth + '>' +
							'<dt class="gp-caption-dt"></dt><dd class="gp-caption-dd">'+ caption +'</dd></dl>';
			...
			imgNode = dom.get('__gp-temp-img-id');
			dom.setAttrib( imgNode, 'id', imgId );
			event.imgData.node = imgNode;
		});

		editor.on( 'gpLoadImageData', function( event ) {
			var parent,
				data = event.imgData.data,
				imgNode = event.imgData.node;

			if ( parent = dom.getParent( imgNode, 'dl.gp-caption' ) ) {
				parent = dom.select( 'dd.gp-caption-dd', parent )[0];
		...
		dom.bind( editor.getDoc(), 'dragstart', function( event ) {
			...
			if ( node.nodeName === 'IMG' && dom.getParent( node, '.gp-caption' ) ) {
				event.preventDefault();
			}
		});

		// Prevent IE11 from making dl.gp-caption resizable
		if ( tinymce.Env.ie && tinymce.Env.ie > 10 ) {
			// The 'mscontrolselect' event is supported only in IE11+
			dom.bind( editor.getBody(), 'mscontrolselect', function( event ) {
				if ( event.target.nodeName === 'IMG' && dom.getParent( event.target, '.gp-caption' ) ) {
					// Hide the thick border with resize handles around dl.gp-caption
					editor.getBody().focus(); // :(
				} else if ( event.target.nodeName === 'DL' && dom.hasClass( event.target, 'gp-caption' ) ) {
	...
	editor.on( 'ObjectResized', function( event ) {
		...
		if ( node.nodeName === 'IMG' ) {
			editor.undoManager.transact( function() {
				...
				if ( parent = dom.getParent( node, '.gp-caption' ) ) {
	...
	editor.on( 'BeforeExecCommand', function( event ) {
		...
		} else if ( cmd === 'JustifyLeft' || cmd === 'JustifyRight' || cmd === 'JustifyCenter' || cmd === 'gpAlignNone' ) {
			node = editor.selection.getNode();
			align = 'align' + cmd.slice( 7 ).toLowerCase();
			DL = editor.dom.getParent( node, '.gp-caption' );
	...
	editor.gpSetImgCaption = function( content ) {
		return parseShortcode( content );
	};

	editor.gpGetImgCaption = function( content ) {
		return getShortcode( content );
	};

	editor.on( 'BeforeSetContent', function( event ) {
		if ( event.format !== 'raw' ) {
			event.content = editor.gpSetImgCaption( event.content );
		}
	});

	editor.on( 'PostProcess', function( event ) {
		if ( event.get ) {
			event.content = editor.gpGetImgCaption( event.content );
		}
	});

	// Add to editor.gp
	editor.gp = editor.gp || {};
	editor.gp.isPlaceholder = isPlaceholder;
