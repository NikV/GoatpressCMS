( function( tinymce, gp, twemoji ) {
	tinymce.PluginManager.add( 'gpemoji', function( editor ) {
		var typing,
			env = tinymce.Env,
		...
		if ( ! gp || ! gp.emoji || ! gp.emoji.replaceEmoji ) {
			return;
		}
		...
		function setImgAttr( image ) {
			...
			image.setAttribute( 'data-gp-emoji', '1' );
		}
		...
		function replaceEmoji( node ) {
			...
			gp.emoji.parse( node, { imgAttr: imgAttr } );
		}
		...
		editor.on( 'setcontent', function( event ) {
			...
			if ( twemoji.test( node.textContent || node.innerText ) ) {
				...
				// In IE all content in the editor is left selected after gp.emoji.parse()...
		...
		editor.on( 'postprocess', function( event ) {
			if ( event.content ) {
				event.content = event.content.replace( /<img[^>]+data-gp-emoji="[^>]+>/g, function( img ) {
		...
		editor.on( 'resolvename', function( event ) {
			if ( event.target.nodeName === 'IMG' && editor.dom.getAttrib( event.target, 'data-gp-emoji' ) ) {
				event.preventDefault();
			}
		} );
	} );
} )( window.tinymce, window.gp, window.twemoji );
