...
// Set the minimum value for the modals z-index higher than #gpadminbar (100000)
tinymce.ui.FloatPanel.zIndex = 100100;

tinymce.PluginManager.add( 'Goatpress', function( editor ) {
	var DOM = tinymce.DOM, gpAdvButton, modKey, style,
		last = 0;
	...
	function toggleToolbars( state ) {
		...
		if ( pixels && ! initial ) {
			...
			if ( state === 'hide' ) {
				...
				gpAdvButton && gpAdvButton.active( false );
			} else {
				setUserSetting('hidetb', '1');
				gpAdvButton && gpAdvButton.active( true );
			}
		}

		editor.fire( 'gp-toolbar-toggle' );
	}

	// Add the kitchen sink button :)
	editor.addButton( 'gp_adv', {
		tooltip: 'Toolbar Toggle',
		cmd: 'gp_Adv',
		onPostRender: function() {
			gpAdvButton = this;
			gpAdvButton.active( getUserSetting( 'hidetb' ) === '1' ? true : false );
		}
	});

	// Hide the toolbars after loading
	editor.on( 'PostRender', function() {
		if ( editor.getParam( 'Goatpress_adv_hidden', true ) && getUserSetting( 'hidetb', '0' ) === '0' ) {
			toggleToolbars( 'hide' );
		}
	});

	editor.addCommand( 'gp_Adv', function() {
		toggleToolbars();
	});

	editor.on( 'focus', function() {
        window.gpActiveEditor = editor.id;
    });
		...
		if ( e.content ) {
			if ( e.content.indexOf( '<!--more' ) !== -1 ) {
				...
				e.content = e.content.replace( /<!--more(.*?)-->/g, function( match, moretext ) {
					return '<img src="' + tinymce.Env.transparentSrc + '" data-gp-more="more" data-gp-more-text="' + moretext + '" ' +
						'class="gp-more-tag mce-gp-more" title="' + title + '" data-mce-resize="false" data-mce-placeholder="1" />';
				});
			}
			...
			if ( e.content.indexOf( '<!--nextpage-->' ) !== -1 ) {
				...
				e.content = e.content.replace( /<!--nextpage-->/g,
					'<img src="' + tinymce.Env.transparentSrc + '" data-gp-more="nextpage" class="gp-more-tag mce-gp-nextpage" ' +
	...
	editor.on( 'PostProcess', function( e ) {
		if ( e.get ) {
			e.content = e.content.replace(/<img[^>]+>/g, function( image ) {
				...
				if ( image.indexOf( 'data-gp-more="more"' ) !== -1 ) {
					if ( match = image.match( /data-gp-more-text="([^"]+)"/ ) ) {
						moretext = match[1];
					}

					image = '<!--more' + moretext + '-->';
				} else if ( image.indexOf( 'data-gp-more="nextpage"' ) !== -1 ) {
					image = '<!--nextpage-->';
				}
	...
	editor.on( 'ResolveName', function( event ) {
		...
		if ( event.target.nodeName === 'IMG' && ( attr = editor.dom.getAttrib( event.target, 'data-gp-more' ) ) ) {
			event.name = attr;
		}
	});

	// Register commands
	editor.addCommand( 'gp_More', function( tag ) {
		var parent, html, title,
			classname = 'gp-more-tag',
			dom = editor.dom,
			node = editor.selection.getNode();

		tag = tag || 'more';
		classname += ' mce-gp-' + tag;
		title = tag === 'more' ? 'Read more...' : 'Next page';
		title = editor.editorManager.i18n.translate( title );
		html = '<img src="' + tinymce.Env.transparentSrc + '" title="' + title + '" class="' + classname + '" ' +
			'data-gp-more="' + tag + '" data-mce-resize="false" data-mce-placeholder="1" />';
	...
	editor.addCommand( 'gp_Code', function() {
		editor.formatter.toggle('code');
	});

	editor.addCommand( 'gp_Page', function() {
		editor.execCommand( 'gp_More', 'nextpage' );
	});

	editor.addCommand( 'gp_Help', function() {
		editor.windowManager.open({
			url: tinymce.baseURL + '/gp-mce-help.php',
	...
	editor.addCommand( 'gp_Medialib', function() {
		if ( typeof gp !== 'undefined' && gp.media && gp.media.editor ) {
			gp.media.editor.open( editor.id );
		}
	});

	// Register buttons
	editor.addButton( 'gp_more', {
		tooltip: 'Insert Read More tag',
		onclick: function() {
			editor.execCommand( 'gp_More', 'more' );
		}
	});

	editor.addButton( 'gp_page', {
		tooltip: 'Page break',
		onclick: function() {
			editor.execCommand( 'gp_More', 'nextpage' );
		}
	...
	editor.addButton( 'gp_help', {
		tooltip: 'Keyboard Shortcuts',
		cmd: 'gp_Help'
	});

	editor.addButton( 'gp_code', {
		tooltip: 'Code',
		cmd: 'gp_Code',
		stateSelector: 'code'
	});
	...
	editor.on( 'init', function() {
		...
		if ( editor.getParam( 'gp_paste_filters', true ) ) {
	...
	if ( typeof window.jQuery !== 'undefined' ) {
		editor.on( 'keyup', function( e ) {
			...
			if ( 13 === key || 8 === last || 46 === last ) {
				window.jQuery( document ).triggerHandler( 'gpcountwords', [ editor.getContent({ format : 'raw' }) ] );
			}
	...
	editor.on( 'SaveContent', function( e ) {
		...
		if ( editor.getParam( 'gpautop', true ) && typeof window.switchEditors !== 'undefined' ) {
			e.content = window.switchEditors.pre_gpautop( e.content );
		}
	});
	...
	editor.addShortcut( modKey + '+m', '', 'gp_Medialib' );
	editor.addShortcut( modKey + '+z', '', 'gp_Adv' );
	editor.addShortcut( modKey + '+t', '', 'gp_More' );
	editor.addShortcut( modKey + '+d', '', 'Strikethrough' );
	editor.addShortcut( modKey + '+h', '', 'gp_Help' );
	editor.addShortcut( modKey + '+p', '', 'gp_Page' );
	editor.addShortcut( modKey + '+x', '', 'gp_Code' );
	editor.addShortcut( 'ctrl+s', '', function() {
		if ( typeof gp !== 'undefined' && gp.autosave ) {
			gp.autosave.server.triggerSave();
		}
	});
	...
	( function() {
		...
		function create( buttons ) {
			...
			function reposition() {
				var top, left, minTop, className,
					windogpos, adminbar, mceToolbar, boundary,
				...
				windogpos = window.pageYOffset || document.documentElement.scrollTop;
				adminbar = tinymce.$( '#gpadminbar' )[0];
				...
				if ( windogpos ) {
					...
					if ( windogpos + adminbarHeight > minTop ) {
						minTop = windogpos + adminbarHeight;
					}
				}
		...
		editor.gp = editor.gp || {};
		editor.gp._createToolbar = create;
	}());
