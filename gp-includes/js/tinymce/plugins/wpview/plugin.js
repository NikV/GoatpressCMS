...
/**
 * Goatpress View plugin.
 */
tinymce.PluginManager.add( 'gpview', function( editor ) {
	var selected,
		Env = tinymce.Env,
		VK = tinymce.util.VK,
	...
	function getView( node ) {
		return getParent( node, 'gpview-wrap' );
	}
	...
	function setViewCursor( before, view ) {
		var location = before ? 'before' : 'after',
			offset = before ? 0 : 1;
		deselect();
		editor.selection.setCursorLocation( editor.dom.select( '.gpview-selection-' + location, view )[0], offset );
		editor.nodeChanged();
	}
	...
	function removeView( view ) {
		editor.undoManager.transact( function() {
			handleEnter( view );
			gp.mce.views.remove( editor, view );
		});
	}
	...
	function select( viewNode ) {
		...
		if ( viewNode !== selected ) {
			...
			clipboard = dom.create( 'div', {
				'class': 'gpview-clipboard',
				'contenteditable': 'true'
			}, gp.mce.views.getText( viewNode ) );

			editor.dom.select( '.gpview-body', viewNode )[0].appendChild( clipboard );
		...
		editor.nodeChanged();
		editor.fire( 'gpview-selected', viewNode );
	}
	...
	function deselect() {
		...
		if ( selected ) {
			clipboard = editor.dom.select( '.gpview-clipboard', selected )[0];
	...
	// Check if the `gp.mce` API exists.
	if ( typeof gp === 'undefined' || ! gp.mce ) {
		return {
			getView: _noop
		};
	}
	...
	function emptyViews( content ) {
		return content.replace(/<div[^>]+data-gpview-text=\"([^"]+)"[^>]*>[\s\S]+?gpview-selection-after[^>]+>(?:&nbsp;|\u00a0)*<\/p><\/div>/g, '$1' );
	}
	...
	editor.on( 'BeforeSetContent', function( event ) {
		...
		event.content = gp.mce.views.setMarkers( event.content );
	});
	...
	editor.on( 'click', function( event ) {
		...
		if ( y < firstRect.top && ( view = getView( first ) ) ) {
			...
			tinymce.each( editor.dom.select( '.gpview-wrap' ), function( view ) {
	...
	editor.on( 'init', function() {
		...
		if ( MutationObserver ) {
			new MutationObserver( function() {
				editor.fire( 'gp-body-class-change' );
			} )
	...
	editor.on( 'PreProcess', function( event ) {
		// Empty the gpview wrap nodes
		tinymce.each( editor.dom.select( 'div[data-gpview-text]', event.node ), function( node ) {
    ...
    editor.on( 'PostProcess', function( event ) {
		if ( event.content ) {
			event.content = event.content.replace( /<div [^>]*?data-gpview-text="([^"]*)"[^>]*>[\s\S]*?<\/div>/g, function( match, shortcode ) {
				if ( shortcode ) {
	...
	editor.on( 'keydown', function( event ) {
		...
		if ( selected ) {
			...
			if ( ! ( ( cursorBefore = dom.hasClass( view, 'gpview-selection-before' ) ) ||
					( cursorAfter = dom.hasClass( view, 'gpview-selection-after' ) ) ) ) {
				return;
			}
	...
	editor.on( 'NodeChange', function( event ) {
		var dom = editor.dom,
			views = editor.dom.select( '.gpview-wrap' ),
		...
		tinymce.each( views, function ( view ) {
			if ( view.className ) {
				view.className = view.className.replace( / ?\bgpview-(?:selection-before|selection-after|cursor-hide)\b/g, '' );
			}
		});

		if ( focus && view ) {
			if ( ( className === 'gpview-selection-before' || className === 'gpview-selection-after' ) &&
				...
				cursorInterval = setInterval( function() {
					if ( dom.hasClass( view, 'gpview-cursor-hide' ) ) {
						dom.removeClass( view, 'gpview-cursor-hide' );
					} else {
						dom.addClass( view, 'gpview-cursor-hide' );
					}
				}, 500 );
			...
			} else if ( ! getParent( event.element, 'gpview-clipboard' ) && ! setViewCursorTries ) {
				deselect();
				setViewCursorTries++;
				setViewCursor( true, view );
			}
	...
	editor.on( 'BeforeExecCommand', function() {
		...
		if ( node && ( ( execCommandBefore = node.className === 'gpview-selection-before' ) || node.className === 'gpview-selection-after' ) && ( view = getView( node ) ) ) {
	...
	editor.on( 'ResolveName', function( event ) {
		if ( editor.dom.hasClass( event.target, 'gpview-wrap' ) ) {
			event.name = editor.dom.getAttrib( event.target, 'data-gpview-type' ) || 'gpview';
			event.stopPropagation();
	...
	editor.addButton( 'gp_view_edit', {
		tooltip: 'Edit ', // trailing space is needed, used for context
		icon: 'dashicon dashicons-edit',
		onclick: function() {
			selected && gp.mce.views.edit( editor, selected );
		}
	} );

	editor.addButton( 'gp_view_remove', {
		tooltip: 'Remove',
