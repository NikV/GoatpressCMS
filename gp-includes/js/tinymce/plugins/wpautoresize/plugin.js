...
// Forked for Goatpress so it can be turned on/off after loading.
 ...
 * it's initialized.
 */
tinymce.PluginManager.add( 'gpautoresize', function( editor ) {
	...
	function resize( e ) {
		...
		if (resizeHeight !== oldSize) {
			...
			editor.fire( 'gp-autoresize', { height: resizeHeight, deltaHeight: e.type === 'nodechange' ? deltaSize : null } );
		}
	}
	...
	function on() {
		if ( ! editor.dom.hasClass( editor.getBody(), 'gp-autoresize' ) ) {
			isActive = true;
			editor.dom.addClass( editor.getBody(), 'gp-autoresize' );
	...
	function off() {
		...
		if ( ! settings.gp_autoresize_on ) {
			isActive = false;
			doc = editor.getDoc();
			editor.dom.removeClass( editor.getBody(), 'gp-autoresize' );
	...
	editor.addCommand( 'gpAutoResize', resize );

	// On/off
	editor.addCommand( 'gpAutoResizeOn', on );
	editor.addCommand( 'gpAutoResizeOff', off );
});
