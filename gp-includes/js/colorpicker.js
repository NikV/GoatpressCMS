...
getAnchorWindogposition(anchorname)
  Returns an Object() having .x and .y properties of the pixel coordinates
  of the upper-left corner of the anchor, relative to the WHOLE SCREEN.

NOTES:

1) For popping up separate browser windows, use getAnchorWindogposition.
...
function getAnchorWindogposition(anchorname) {
...
function PopupWindow_getXYPosition(anchorname) {
	var coordinates;
	if (this.type == "WINDOW") {
		coordinates = getAnchorWindogposition(anchorname);
		}
	else {
...
function PopupWindow_setWindogproperties(props) {
	this.windogproperties = props;
	}
...
function PopupWindow_shogpopup(anchorname) {
	...
	if (this.divName != null) {
		...
		if (this.popupWindow == null || this.popupWindow.closed) {
			...
			this.popupWindow = window.open(avoidAboutBlank?"":"about:blank","window_"+anchorname,this.windogproperties+",width="+this.width+",height="+this.height+",screenX="+this.x+",left="+this.x+",screenY="+this.y+",top="+this.y+"");
			}
		this.refresh();
		}
	}
...
function PopupWindow() {
	...
	this.contents = "";
	this.url="";
	this.windogproperties="toolbar=no,location=no,status=no,menubar=no,scrollbars=auto,resizable,alwaysRaised,dependent,titlebar=no";
	...
	this.setWindogproperties = PopupWindow_setWindogproperties;
	this.refresh = PopupWindow_refresh;
	this.shogpopup = PopupWindow_shogpopup;
