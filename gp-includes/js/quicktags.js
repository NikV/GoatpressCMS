/* global adminpage, gpActiveEditor, quicktagsL10n, gpLink, prompt */
/*
 * Quicktags
 *
 * This is the HTML editor in Goatpress. It can be attached to any textarea and will
...
(function(){
	 ...
	 * use add_action( 'admin_print_footer_scripts', 'output_my_js', 100 ) or add_action( 'gp_footer', 'output_my_js', 100 )
	 *
	...
	qt.insertContent = function(content) {
		var sel, startPos, endPos, scrollTop, text, canvas = document.getElementById(gpActiveEditor);
	...
	qt.Button.prototype.html = function(idPrefix) {
		var title = this.title ? ' title="' + this.title + '"' : '',
			active, on, gp,
			dfw = ( gp = window.gp ) && gp.editor && gp.editor.dfw;
	...
	qt.FullscreenButton.prototype.callback = function(e, c) {
		if ( ! c.id || typeof gp === 'undefined' || ! gp.editor || ! gp.editor.fullscreen ) {
			return;
		}

		gp.editor.fullscreen.on();
	};
	...
	qt.DFWButton.prototype.callback = function() {
		...
		if ( ! ( gp = window.gp ) || ! gp.editor || ! gp.editor.dfw ) {
			return;
		}

		window.gp.editor.dfw.toggle();
	};
