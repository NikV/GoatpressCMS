...
var media = gp.media,
	baseSettings = window._gpmejsSettings || {},
	l10n = window._gpMediaViewsL10n || {};
	...
	unsetPlayers : function() {
		if ( this.players && this.players.length ) {
			_.each( this.players, function (player) {
				player.pause();
				gp.media.mixin.removePlayer( player );
			} );
			this.players = [];
		}
	}
 ...
 */
gp.media.playlist = new gp.media.collection({
	tag: 'playlist',
	editTitle : l10n.editPlaylistTitle,
	defaults : {
		id: gp.media.view.settings.post.id,
		style: 'light',
		tracklist: true,
		tracknumbers: true,
		images: true,
...
gp.media.audio = {
	...
	edit : function( data ) {
		var frame, shortcode = gp.shortcode.next( 'audio', data ).shortcode;
...
gp.media.video = {
	...
	edit : function( data ) {
		var frame,
			shortcode = gp.shortcode.next( 'video', data ).shortcode,
			attrs;
...
/*globals gp, Backbone, _ */

/**
 * gp.media.model.PostMedia
 *
...
var PostMedia = Backbone.Model.extend({
	...
	setSource: function( attachment ) {
		...
		if ( _.contains( gp.media.view.settings.embedExts, this.extension ) ) {
	...
	changeAttachment: function( attachment ) {
		...
		this.unset( 'src' );
		_.each( _.without( gp.media.view.settings.embedExts, this.extension ), function( ext ) {
			this.unset( ext );
		}, this );
	}
});
...
/**
 * gp.media.view.MediaFrame.MediaDetails
 *
 * @class
 * @augments gp.media.view.MediaFrame.Select
 * @augments gp.media.view.MediaFrame
...
MediaDetails = Select.extend({
	...
	initialize: function( options ) {
		...
		this.media = new gp.media.model.PostMedia( options.metadata );
		this.options.selection = new gp.media.model.Selection( this.media.attachment, { multiple: false } );
...
/**
 * gp.media.view.MediaFrame.VideoDetails
 *
 * @class
 * @augments gp.media.view.MediaFrame.MediaDetails
 * @augments gp.media.view.MediaFrame.Select
...
VideoDetails = MediaDetails.extend({
	...
	initialize: function( options ) {
		options.DetailsView = gp.media.view.VideoDetails;
	...
	createStates: function() {
		this.states.add([
			new gp.media.controller.VideoDetails({
				media: this.media
			}),
	...
	renderSelectPosterImageToolbar: function() {
		this.setPrimaryButton( l10n.videoSelectPosterImageTitle, function( controller, state ) {
			...
			_.each( gp.media.view.settings.embedExts, function (ext) {
				if ( controller.media.get( ext ) ) {
					urls.push( controller.media.get( ext ) );
				}
			} );

			gp.ajax.send( 'set-attachment-thumbnail', {
				data : {
					urls: urls,
...
/*global gp, jQuery, _, MediaElementPlayer */

/**
 * gp.media.view.MediaDetails
 *
 * @class
 * @augments gp.media.view.Settings.AttachmentDisplay
 * @augments gp.media.view.Settings
...
MediaDetails = AttachmentDisplay.extend({
	initialize: function() {
		...
		this.players = [];
		this.listenTo( this.controller, 'close', gp.media.mixin.unsetPlayers );
		this.on( 'ready', this.setPlayer );
		this.on( 'media:setting:remove', gp.media.mixin.unsetPlayers, this );
	 ...
	 *
	 * @fires gp.media.view.MediaDetails#media:setting:remove
	 *
	 * @param {Event} e
	 */
	...
	setPlayer : function() {
		...
		if ( this.model.get( 'src' ).indexOf( 'vimeo' ) > -1 && ! ( 'Froogaloop' in window ) ) {
			baseSettings = gp.media.mixin.mejsSettings;
