...
/**
 * gp.media.controller.CollectionAdd
 *
 ...
 * @param {gp.media.model.Attachments} [attributes.library]                 The attachments collection to browse.
 ...
 */
var Selection = gp.media.model.Selection,
...
CollectionAdd = Library.extend({
	...
	initialize: function() {
		...
		if ( ! this.get('library') ) {
			this.set( 'library', gp.media.query({ type: this.get('type') }) );
		}
...
/**
 * gp.media.controller.CollectionEdit
 *
 ...
 * @param {gp.media.model.Attachments} [attributes.library]              The attachments collection to edit.
 ...
 *                                                                       If none supplied, defaults to gp.media.view.Attachment.EditLibrary.
 * @param {string}                     attributes.type                   The collection's media type. (e.g. 'video').
 * @param {string}                     attributes.collectionType         The collection type. (e.g. 'playlist').
 */
...
CollectionEdit = Library.extend({
	...
	initialize: function() {
		...
		// If we haven't been provided a `library`, create a `Selection`.
		if ( ! this.get('library') ) {
			this.set( 'library', new gp.media.model.Selection() );
		}
		// The single `Attachment` view to be used in the `Attachments` view.
		if ( ! this.get('AttachmentView') ) {
			this.set( 'AttachmentView', gp.media.view.Attachment.EditLibrary );
		}
		Library.prototype.initialize.apply( this, arguments );
	},
	...
	activate: function() {
		...
		this.get('library').observe( gp.Uploader.queue );
	...
	deactivate: function() {
		...
		this.get('library').unobserve( gp.Uploader.queue );
	 ...
	 * @since 3.9.0
	 *
	 * @param {gp.media.view.attachmentsBrowser} The attachments browser view.
	 */
	renderSettings: function( attachmentsBrowserView ) {
		...
		if ( dragInfoText ) {
			attachmentsBrowserView.toolbar.set( 'dragInfo', new gp.media.View({
...
Cropper = gp.media.controller.State.extend({
	defaults: {
		id:          'cropper',
	...
	createCropContent: function() {
		this.cropperView = new gp.media.view.Cropper({
			controller: this,
	...
	createCropToolbar: function() {
		...
		this.frame.toolbar.set( new gp.media.view.Toolbar(toolbarOptions) );
	},

	doCrop: function( attachment ) {
		return gp.ajax.post( 'custom-header-crop', {
 ...
 *
 * @param {object}                    attributes                      The attributes hash passed to the state.
 * @param {gp.media.model.Attachment} attributes.model                The attachment.
...
EditImage = gp.media.controller.State.extend({
	defaults: {
		id:      'edit-image',
	...
	toolbar: function() {
		...
		frame.toolbar.set( new gp.media.view.Toolbar({
			controller: frame,
			items: {
				back: {
					style: 'primary',
...
Embed = gp.media.controller.State.extend({
	defaults: {
		id:       'embed',
	 ...
	 *
	 * @fires gp.media.controller.Embed#scan
	 */
	scan: function() {
		var scanners,
			embed = this,
...
/**
 * gp.media.controller.FeaturedImage
 *
...
FeaturedImage = Library.extend({
	...
	initialize: function() {
		...
		// If we haven't been provided a `library`, create a `Selection`.
		if ( ! this.get('library') ) {
			this.set( 'library', gp.media.query({ type: 'image' }) );
		}
	...
	updateSelection: function() {
		var selection = this.get('selection'),
			id = gp.media.view.settings.post.featuredImageId,
			attachment;
...
/**
 * gp.media.controller.GalleryAdd
 *
...
/**
 * gp.media.controller.GalleryEdit
 *
 ...
 * @param {gp.media.model.Attachments} [attributes.library]               The collection of attachments in the gallery.
 *                                                                        If one is not supplied, an empty media.model.Selection collection is created.
 * @param {boolean}                    [attributes.multiple=false]        Whether multi-select is enabled.
 * @param {boolean}                    [attributes.searchable=false]      Whether the library is searchable.
 * @param {boolean}                    [attributes.sortable=true]         Whether the Attachments should be sortable. Depends on the orderby property being set to menuOrder on the attachments collection.
...
GalleryEdit = Library.extend({
	...
	gallerySettings: function( browser ) {
		...
		browser.sidebar.set({
			gallery: new gp.media.view.Settings.Gallery({
				controller: this,
...
/**
 * gp.media.controller.ImageDetails
 *
 ...
 * @param {gp.media.model.Attachment} attributes.image                   The image's model.
 ...
 * @param {gp.media.model.Selection|object} [attributes.selection]               A collection to contain attachment selections within the state.
...
Library = gp.media.controller.State.extend({
	defaults: {
		id:                 'library',
	...
	initialize: function() {
		...
		if ( ! this.get('library') ) {
			this.set( 'library', gp.media.query() );
		}

		if ( ! ( selection instanceof gp.media.model.Selection ) ) {
			props = selection;
			...
			this.set( 'selection', new gp.media.model.Selection( null, {
	...
	activate: function() {
		...
		gp.Uploader.queue.on( 'add', this.uploading, this );
	...
	deactivate: function() {
		...
		gp.Uploader.queue.off( null, null, this );
	},
	...
	resetDisplays: function() {
		var defaultProps = gp.media.view.settings.defaultProps;
		this._displays = [];
	 ...
	 * @since 3.5.0
	 *
	 * @param {gp.media.model.Attachment} attachment
	...
	canEmbed: function( attachment ) {
		...
		return _.contains( gp.media.view.settings.embedExts, attachment.get('filename').split('.').pop() );
	},
...
_.extend( Library.prototype, gp.media.selectionSync );
...
_.extend( Region.prototype, {
	 ...
	 *
	 * @returns {gp.media.controller.Region} Returns itself to allow chaining.
	 */
	mode: function( mode ) {
		if ( ! mode ) {
			return this._mode;
	 ...
	 * @returns {gp.Backbone.Subviews} Subviews is returned to allow chaining
	 */
	 ...
	 *
	 * @param {string} event
	 * @returns {undefined|gp.media.controller.Region} Returns itself to allow chaining.
	 */
...
/**
 * gp.media.controller.ReplaceImage
 *
...
/**
 * gp.media.controller.StateMachine
 *
...
_.extend( StateMachine.prototype, Backbone.Events, {
	 ...
	 *
	 * @param {string} id
	 * @returns {gp.media.controller.State} Returns a State model
	 ...
	 * @param {string} id
	 *
	 * @fires gp.media.controller.State#deactivate
	 * @fires gp.media.controller.State#activate
	 *
	 * @returns {gp.media.controller.StateMachine} Returns itself to allow chaining
	 */
...
l10n = media.view.l10n = window._gpMediaViewsL10n || {};
...
/**
 * gp.media.view.AttachmentCompat
 *
...
AttachmentCompat = View.extend({
	...
	/**
	 * @returns {gp.media.view.AttachmentCompat} Returns itself to allow chaining
	 */
	dispose: function() {
...
/**
 * gp.media.view.AttachmentFilters
 *
 * @class
 * @augments gp.media.View
 * @augments gp.Backbone.View
...
AttachmentFilters = gp.media.View.extend({
	tagName:   'select',
...
Attachment = View.extend({
	...
	template:  gp.template('attachment'),
	...
	/**
	 * @returns {gp.media.view.Attachment} Returns itself to allow chaining
	 */
	dispose: function() {
		var selection = this.options.selection;
...
/**
 * gp.media.view.Attachment.Details
 *
 * @class
 ...
 * @augments Backbone.View
 */
var Attachment = gp.media.view.Attachment,
	l10n = gp.media.view.l10n,
	Details;
...
Details = Attachment.extend({
	...
	template:  gp.template('attachment-details'),
...
/**
 * gp.media.view.Attachment.EditLibrary
 *
 * @class
 * @augments gp.media.view.Attachment
 * @augments gp.media.View
 ...
 * @augments Backbone.View
 */
var EditLibrary = gp.media.view.Attachment.extend({
	buttons: {
		close: true
	}
});
...
/**
 * gp.media.view.Attachment.Library
 *
 * @class
 * @augments gp.media.view.Attachment
 * @augments gp.media.View
 ...
 * @augments Backbone.View
 */
var Library = gp.media.view.Attachment.extend({
	buttons: {
		check: true
	}
});
...
Attachments = View.extend({
	...
	initialize: function() {
		...
		_.defaults( this.options, {
			refreshSensitivity: gp.media.isTouchDevice ? 300 : 200,
			refreshThreshold:   3,
			AttachmentView:     gp.media.view.Attachment,
			sortable:           false,
			resize:             true,
	...
	initSortable: function() {
		...
		if ( gp.media.isTouchDevice || ! this.options.sortable || ! $.fn.sortable ) {
			return;
		}
...
/**
 * gp.media.view.AttachmentsBrowser
 *
 * @class
 * @augments gp.media.View
 * @augments gp.Backbone.View
 ...
 */
var View = gp.media.View,
	mediaTrash = gp.media.view.settings.mediaTrash,
	l10n = gp.media.view.l10n,
	$ = jQuery,
	AttachmentsBrowser;
...
AttachmentsBrowser = View.extend({
	...
	initialize: function() {
		_.defaults( this.options, {
			...
			display: false,
			sidebar: true,
			AttachmentView: gp.media.view.Attachment.Library
		});
	...
	/**
	 * @returns {gp.media.view.AttachmentsBrowser} Returns itself to allow chaining
	 */
	dispose: function() {
	...
	createToolbar: function() {
		...
		if ( this.controller.isModeActive( 'grid' ) ) {
			toolbarOptions.className = 'media-toolbar gp-filter';
		}

		/**
		* @member {gp.media.view.Toolbar}
		*/
		this.toolbar = new gp.media.view.Toolbar( toolbarOptions );

		this.views.add( this.toolbar );

		this.toolbar.set( 'spinner', new gp.media.view.Spinner({
			priority: -60
		}) );
		...
		if ( -1 !== $.inArray( this.options.filters, [ 'uploaded', 'all' ] ) ) {
			...
			this.toolbar.set( 'filtersLabel', new gp.media.view.Label({
			...
			if ( 'uploaded' === this.options.filters ) {
				this.toolbar.set( 'filters', new gp.media.view.AttachmentFilters.Uploaded({
					controller: this.controller,
					model:      this.collection.props,
					priority:   -80
				}).render() );
			} else {
				Filters = new gp.media.view.AttachmentFilters.All({
					controller: this.controller,
					model:      this.collection.props,
					priority:   -80
				});
		...
		if ( this.controller.isModeActive( 'grid' ) ) {
			LibraryViewSwitcher = View.extend({
				...
				template: gp.template( 'media-library-view-switcher')
			});
			...
			this.toolbar.set( 'dateFilterLabel', new gp.media.view.Label({
			...
			this.toolbar.set( 'dateFilter', new gp.media.view.DateFilter({
				controller: this.controller,
				model:      this.collection.props,
				priority: -75
			}).render() );
			...
			this.toolbar.set( 'selectModeToggleButton', new gp.media.view.SelectModeToggleButton({
				text: l10n.bulkSelect,
				controller: this.controller,
				priority: -70
			}).render() );

			this.toolbar.set( 'deleteSelectedButton', new gp.media.view.DeleteSelectedButton({
				filters: Filters,
				style: 'primary',
				disabled: true,
			...
			if ( mediaTrash ) {
				this.toolbar.set( 'deleteSelectedPermanentlyButton', new gp.media.view.DeleteSelectedPermanentlyButton({
					filters: Filters,
					style: 'primary',
					disabled: true,
		...
		if ( this.options.search ) {
			...
			this.toolbar.set( 'searchLabel', new gp.media.view.Label({
			...
			this.toolbar.set( 'search', new gp.media.view.Search({
				controller: this.controller,
				model:      this.collection.props,
				priority:   60
			}).render() );
	...
	createUploader: function() {
		this.uploader = new gp.media.view.UploaderInline({
			controller: this.controller,
			status:     false,
	...
	createAttachments: function() {
		this.attachments = new gp.media.view.Attachments({
			controller:           this.controller,
			collection:           this.collection,
			selection:            this.options.selection,
			model:                this.model,
	...
	createSidebar: function() {
		var options = this.options,
			...
			sidebar = this.sidebar = new gp.media.view.Sidebar({
				controller: this.controller
			});
		...
		if ( this.controller.uploader ) {
			sidebar.set( 'uploads', new gp.media.view.UploaderStatus({
				controller: this.controller,
				priority:   40
			}) );
		}
	...
	createSingle: function() {
		...
		sidebar.set( 'details', new gp.media.view.Attachment.Details({
			controller: this.controller,
			model:      single,
			priority:   80
		}) );

		sidebar.set( 'compat', new gp.media.view.AttachmentCompat({
			controller: this.controller,
			model:      single,
			priority:   120
		}) );
		...
		if ( this.options.display ) {
			sidebar.set( 'display', new gp.media.view.Settings.AttachmentDisplay({
				controller:   this.controller,
 ...
 * @augments Backbone.View
 */
var Button = gp.media.View.extend({
	tagName:    'a',
	...
	/**
	 * @returns {gp.media.view.Button} Returns itself to allow chaining
	 */
	render: function() {
...
EditImage = View.extend({
	...
	template: gp.template('image-editor'),
...
EmbedLink = gp.media.view.Settings.extend({
	className: 'embed-link-settings',
	template:  gp.template('embed-link-settings'),
	...
	fetch: function() {
		...
		embed = new gp.shortcode({
			tag: 'embed',
			attrs: _.pick( this.model.attributes, [ 'width', 'height', 'src' ] ),
			content: this.model.get('url')
		});

		gp.ajax.send( 'parse-embed', {
			data : {
				post_ID: gp.media.view.settings.post.id,
 ...
 * @augments Backbone.View
 */
var FocusManager = gp.media.View.extend({
 ...
 * states.
 *
 * @see gp.media.controller.State
 * @see gp.media.controller.Region
 *
 * @class
 * @augments gp.media.View
 * @augments gp.Backbone.View
 ...
 */
var Frame = gp.media.View.extend({
	initialize: function() {
		_.defaults( this.options, {
			mode: [ 'select' ]
		});
	...
	_createRegions: function() {
		...
		_.each( this.regions, function( region ) {
			this[ region ] = new gp.media.controller.Region({
				view:     this,
				id:       region,
	 ...
	 *
	 * @see gp.media.controller.State
	 * @see gp.media.controller.StateMachine
	 *
	 * @fires gp.media.controller.State#ready
	 */
	_createStates: function() {
		// Create the default `states` collection.
		this.states = new Backbone.Collection( null, {
			model: gp.media.controller.State
		});
	 ...
	 *
	 * @returns {gp.media.view.Frame} Returns itself to allow chaining
	 */
	reset: function() {
...
_.extend( Frame.prototype, gp.media.controller.StateMachine.prototype );
...
Post = Select.extend({
	initialize: function() {
		this.counts = {
			audio: {
				count: gp.media.view.settings.attachmentCounts.audio,
				state: 'playlist'
			},
			video: {
				count: gp.media.view.settings.attachmentCounts.video,
	...
	createStates: function() {
		...
		this.states.add([
			...
			new Library({
				...
				library:  gp.media.query( _.defaults({
					type: 'image'
				}, options.library ) )
			}),

			// Embed states.
			new gp.media.controller.Embed( { metadata: options.metadata } ),

			new gp.media.controller.EditImage( { model: options.editImage } ),

			// Gallery states.
			new gp.media.controller.GalleryEdit({
				library: options.selection,
				editing: options.editing,
				menu:    'gallery'
			}),

			new gp.media.controller.GalleryAdd(),
			...
			// Playlist states.
			new gp.media.controller.CollectionEdit({
				type: 'audio',
				collectionType: 'playlist',
				title:          l10n.editPlaylistTitle,
				SettingsView:   gp.media.view.Settings.Playlist,
				library:        options.selection,
				editing:        options.editing,
				menu:           'playlist',
			...
			new gp.media.controller.CollectionAdd({
				type: 'audio',
				collectionType: 'playlist',
		...
		if ( gp.media.view.settings.post.featuredImageId ) {
			this.states.add( new gp.media.controller.FeaturedImage() );
		}
	},
	...
	bindHandlers: function() {
		...
		if ( typeof checkCounts !== 'undefined' ) {
			this.listenTo( gp.media.model.Attachments.all, 'change:type', this.mediaTypeCounts );
		}
	...
	// Menus
	/**
	 * @param {gp.Backbone.View} view
	 */
	mainMenu: function( view ) {
		view.set({
			'library-separator': new gp.media.View({
				className: 'separator',
				priority: 100
			})
		});
	...
	galleryMenu: function( view ) {
		...
		view.set({
			...
			separateCancel: new gp.media.View({
				className: 'separator',
				priority: 40
			})
		});
	...
	embedContent: function() {
		var view = new gp.media.view.Embed({
			controller: this,
			model:      this.state()
		}).render();
	...
	editSelectionContent: function() {
		...
		view = new gp.media.view.AttachmentsBrowser({
			controller: this,
			...
			AttachmentView: gp.media.view.Attachments.EditSelection
		}).render();
	...
	editImageContent: function() {
		var image = this.state().get('image'),
			view = new gp.media.view.EditImage( { model: image, controller: this } ).render();
	...
	selectionStatusToolbar: function( view ) {
		...
		view.set( 'selection', new gp.media.view.Selection({
			controller: this,
	...
	mainInsertToolbar: function( view ) {
		...
		view.set( 'insert', {
			...
			/**
			 * @fires gp.media.controller.State#insert
			 */
			click: function() {
	...
	mainGalleryToolbar: function( view ) {
		...
		view.set( 'gallery', {
			...
			click: function() {
				...
				edit.set( 'library', new gp.media.model.Selection( models, {
	...
	mainEmbedToolbar: function( toolbar ) {
		toolbar.view = new gp.media.view.Toolbar.Embed({
			controller: this
		});
	},
		...
		this.toolbar.set( new gp.media.view.Toolbar({
			...
			items: {
				insert: {
					...
					/**
					 * @fires gp.media.controller.State#update
					 */
					click: function() {
						var controller = this.controller,
							state = controller.state();
		...
		this.toolbar.set( new gp.media.view.Toolbar({
			...
			items: {
				insert: {
					...
					/**
					 * @fires gp.media.controller.State#reset
					 */
					click: function() {
						var controller = this.controller,
							state = controller.state(),
...
/**
 * gp.media.view.MediaFrame.Select
 *
...
Select = MediaFrame.extend({
	...
	createSelection: function() {
		...
		if ( ! (selection instanceof gp.media.model.Selection) ) {
			this.options.selection = new gp.media.model.Selection( selection, {
				multiple: this.options.multiple
			});
		}

		this._selection = {
			attachments: new gp.media.model.Attachments(),
			difference: []
		};
	},
	...
	createStates: function() {
		...
		this.states.add([
			// Main states.
			new gp.media.controller.Library({
				library:   gp.media.query( options.library ),
	 ...
	 *
	 * @param {gp.media.view.Router} routerView
	 */
	 ...
	 *
	 * @param {gp.media.controller.Region} contentRegion
	 */
	browseContent: function( contentRegion ) {
		...
		// Browse our library of attachments.
		contentRegion.view = new gp.media.view.AttachmentsBrowser({
			controller: this,
	...
	uploadContent: function() {
		this.$el.removeClass( 'hide-toolbar' );
		this.content.set( new gp.media.view.UploaderInline({
			controller: this
		}) );
	},
	...
	createSelectToolbar: function( toolbar, options ) {
		...
		toolbar.view = new gp.media.view.Toolbar.Select( options );
	}
});
...
ImageDetails = AttachmentDisplay.extend({
	...
	template:  gp.template('image-details'),
 ...
 * @mixes gp.media.controller.StateMachine
 */
var Frame = gp.media.view.Frame,
	$ = jQuery,
	MediaFrame;

MediaFrame = Frame.extend({
	className: 'media-frame',
	template:  gp.template('media-frame'),
	...
	initialize: function() {
		...
		if ( this.options.modal ) {
			this.modal = new gp.media.view.Modal({
				controller: this,
	...
	/**
	 * @returns {gp.media.view.MediaFrame} Returns itself to allow chaining
	 */
	render: function() {
	...
	createMenu: function( menu ) {
		menu.view = new gp.media.view.Menu({
			controller: this
		});
	},
	...
	createToolbar: function( toolbar ) {
		toolbar.view = new gp.media.view.Toolbar({
			controller: this
		});
	},
	/**
	...
	createRouter: function( router ) {
		router.view = new gp.media.view.Router({
			controller: this
		});
	},
	/**
	...
	createIframeStates: function( options ) {
		var settings = gp.media.view.settings,
			tabs = settings.tabs,
	...
	iframeContent: function( content ) {
		...
		content.view = new gp.media.view.Iframe({
			controller: this
		});
	},
	...
	iframeMenu: function( view ) {
		...
		_.each( gp.media.view.settings.tabs, function( title, id ) {
			views[ 'iframe:' + id ] = {
 ...
 * @augments Backbone.View
 */
var MenuItem = gp.media.view.MenuItem,
	PriorityList = gp.media.view.PriorityList,
	Menu;
...
Modal = gp.media.View.extend({
	tagName:  'div',
	template: gp.template('media-modal'),
	...
	initialize: function() {
		...
		this.focusManager = new gp.media.view.FocusManager({
			el: this.el
		});
	},
	/**
	...
	/**
	 * @returns {gp.media.view.Modal} Returns itself to allow chaining
	 */
	attach: function() {
 ...
 * @augments Backbone.View
 */
var PriorityList = gp.media.View.extend({
	tagName:   'div',
	...
	/**
	 * @param {string} id
	 * @param {gp.media.View|Object} view
	 * @param {Object} options
	 * @returns {gp.media.view.PriorityList} Returns itself to allow chaining
	 */
	...
	/**
	 * @param {string} id
	 * @returns {gp.media.view.PriorityList}
	 */
	unset: function( id ) {
		var view = this.get( id );
	...
	toView: function( options ) {
		return new gp.media.View( options );
	}
});
...
Selection = gp.media.View.extend({
	tagName:   'div',
	className: 'media-selection',
	template:  gp.template('media-selection'),
	...
	initialize: function() {
		...
		/**
		 * @member {gp.media.view.Attachments.Selection}
		 */
		this.attachments = new gp.media.view.Attachments.Selection({
			controller: this.controller,
			collection: this.collection,
...
Settings = View.extend({
	...
	/**
	 * @returns {gp.media.view.Settings} Returns itself to allow chaining
	 */
	render: function() {
		View.prototype.render.apply( this, arguments );
...
/**
 * gp.media.view.Settings.AttachmentDisplay
 *
 * @class
 * @augments gp.media.view.Settings
 * @augments gp.media.View
...
AttachmentDisplay = Settings.extend({
	...
	template:  gp.template('attachment-display-settings'),
	...
	/**
	 * @returns {gp.media.view.AttachmentDisplay} Returns itself to allow chaining
	 */
	render: function() {
		var attachment = this.options.attachment;
		if ( attachment ) {
	...
	updateLinkTo: function() {
		...
		// If the input is visible, focus and select its contents.
		if ( ! gp.media.isTouchDevice && $input.is(':visible') ) {
			$input.focus()[0].select();
		}
	}
});
 ...
 * @augments Backbone.View
 */
var Sidebar = gp.media.view.PriorityList.extend({
...
Toolbar = View.extend({
	...
	initialize: function() {
		...
		this.primary   = new gp.media.view.PriorityList();
		this.secondary = new gp.media.view.PriorityList();
	...
	/**
	 * @returns {gp.media.view.Toolbar} Returns itsef to allow chaining
	 */
	dispose: function() {
		if ( this.selection ) {
	 ...
	 * @param {Object} [options={}]
	 * @returns {gp.media.view.Toolbar} Returns itself to allow chaining
	 */
	set: function( id, view, options ) {
		var list;
		options = options || {};
		...
		if ( _.isObject( id ) ) {
			...
			if ( ! ( view instanceof Backbone.View ) ) {
				...
				view = new gp.media.view.Button( view ).render();
			}
...
/**
 * Creates a dropzone on gp editor instances (elements with .gp-editor-wrap
 * or #gp-fullscreen-body) and relays drag'n'dropped files to a media workflow.
 *
 * gp.media.view.EditorUploader
 *
 * @class
 * @augments gp.media.View
 * @augments gp.Backbone.View
...
EditorUploader = View.extend({
	...
	template:  gp.template( 'uploader-editor' ),
	...
	render: function() {
		...
		View.prototype.render.apply( this, arguments );
		$( '.gp-editor-wrap, #gp-fullscreen-body' ).each( _.bind( this.attach, this ) );
		return this;
	},
	...
	drop: function( event ) {
		...
		$wrap = $( event.target ).parents( '.gp-editor-wrap' );
		if ( $wrap.length > 0 && $wrap[0].id ) {
			window.gpActiveEditor = $wrap[0].id.slice( 3, -5 );
		}

		if ( ! this.workflow ) {
			this.workflow = gp.media.editor.open( 'content', {
				frame:    'post',
				state:    'insert',
				title:    l10n.addMedia,
				multiple: true
...
UploaderInline = View.extend({
	...
	template:  gp.template('uploader-inline'),
	...
	initialize: function() {
		...
		if ( _.isUndefined( this.options.postId ) ) {
			this.options.postId = gp.media.view.settings.post.id;
		}

		if ( this.options.status ) {
			this.views.set( '.upload-inline-status', new gp.media.view.UploaderStatus({
				controller: this.controller
			}) );
		}
	},
	...
	/**
	 * @returns {gp.media.view.UploaderInline} Returns itself to allow chaining
	 */
	dispose: function() {
		if ( this.disposing ) {
			/**
	...
	/**
	 * @returns {gp.media.view.UploaderInline}
	 */
	ready: function() {
...
UploaderStatus = View.extend({
	...
	template:  gp.template('uploader-status'),
	...
	/**
	 * @global gp.Uploader
	 * @returns {gp.media.view.UploaderStatus}
	 */
	dispose: function() {
		gp.Uploader.queue.off( null, null, this );
		/**
	...
	filename: function( filename ) {
		return gp.media.truncate( _.escape( filename ), 24 );
	},
	/**
...
UploaderWindow = gp.media.View.extend({
	tagName:   'div',
	className: 'uploader-window',
	template:  gp.template('uploader-window'),
	...
	ready: function() {
		var postId = gp.media.view.settings.post.id,
			dropzone;
		...
		this.uploader = new gp.Uploader( this.options.uploader );
	...
	hide: function() {
		...
		gp.media.transition( $el ).done( function() {
		...
		// https://core.trac.Goatpress.org/ticket/27341
		_.delay( function() {
