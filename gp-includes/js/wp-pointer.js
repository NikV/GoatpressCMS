...
(function($){
	...
	$.widget('gp.pointer', {
		options: {
			pointerClass: 'gp-pointer',
			pointerWidth: 320,
			content: function() {
				return $(this).text();
			},
			buttons: function( event, t ) {
				var close  = ( gpPointerL10n ) ? gpPointerL10n.dismiss : 'Dismiss',
		...
		_create: function() {
			...
			this.content = $('<div class="gp-pointer-content"></div>');
			this.arrow   = $('<div class="gp-pointer-arrow"><div class="gp-pointer-arrow-inner"></div></div>');
		...
		_update: function( event, content ) {
			...
			if ( buttons ) {
				buttons.wrap('<div class="gp-pointer-buttons" />').parent().appendTo( this.content );
			}
		...
		repoint: function() {
			...
			this.pointer[0].className = this.pointer[0].className.replace( /gp-pointer-[^\s'"]*/, '' );

			// Add arrow class.
			this.pointer.addClass( 'gp-pointer-' + edge );
		},
