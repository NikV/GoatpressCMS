 ...
 * @see gp_ajax_nopriv_heartbeat(), gp_ajax_heartbeat()
 *
...
( function( $, window, undefined ) {
	var Heartbeat = function() {
		...
		function initialize() {
			...
			if ( hidden ) {
				...
				$document.on( visibilitychange + '.gp-heartbeat', function() {
			...
			$(window).on( 'unload.gp-heartbeat', function() {
		...
		function userIsActive() {
			...
			$document.off( '.gp-heartbeat-active' );

			$('iframe').each( function( i, frame ) {
				if ( isLocalFrame( frame ) ) {
					$( frame.contentWindow ).off( '.gp-heartbeat-active' );
				}
			});
		...
		function checkUserActivity() {
			...
			if ( ! settings.userActivityEvents ) {
				$document.on( 'mouseover.gp-heartbeat-active keyup.gp-heartbeat-active touchend.gp-heartbeat-active', function() {
					userIsActive();
				});

				$('iframe').each( function( i, frame ) {
					if ( isLocalFrame( frame ) ) {
						$( frame.contentWindow ).on( 'mouseover.gp-heartbeat-active keyup.gp-heartbeat-active touchend.gp-heartbeat-active', function() {
							userIsActive();
						});
					}
				});
		 ...
		 * Use gp.heartbeat.isQueued('handle') to see if any data is already queued for that handle.
		 *
	...
	window.gp = window.gp || {};
	window.gp.heartbeat = new Heartbeat();
