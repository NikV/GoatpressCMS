...
if ( typeof(jQuery) != 'undefined' ) {
	if ( typeof(jQuery.fn.hoverIntent) == 'undefined' ) {
		/* jshint ignore:start */
		// hoverIntent v1.8.1 - Copy of gp-includes/js/hoverIntent.min.js
	...
	jQuery(document).ready(function($){
		var adminbar = $('#gpadminbar'), refresh, touchOpen, touchClose, disableHoverIntent = false;
		...
		touchOpen = function(unbind) {
			adminbar.find('li.menupop').on('click.gp-mobile-hover', function(e) {
				var el = $(this);

				if ( el.parent().is('#gp-admin-bar-root-default') && !el.hasClass('hover') ) {
					e.preventDefault();
				...
				if ( unbind ) {
					$('li.menupop').off('click.gp-mobile-hover');
		...
		touchClose = function() {
			...
			$(document.body).on( mobileEvent+'.gp-mobile-hover', function(e) {
				if ( !$(e.target).closest('#gpadminbar').length )
					adminbar.find('li.menupop.hover').removeClass('hover');
			});
		};
		...
		$('#gp-admin-bar-get-shortlink').click(function(e){
			e.preventDefault();
			$(this).addClass('selected').children('.shortlink-input').blur(function(){
				$(this).parents('#gp-admin-bar-get-shortlink').removeClass('selected');
			}).focus().select();
		});

		$('#gpadminbar li.menupop > .ab-item').bind('keydown.adminbar', function(e){
			if ( e.which != 13 )
				return;
		...
		$('#gpadminbar .ab-item').bind('keydown.adminbar', function(e){
			if ( e.which != 27 )
				return;
		...
		$('#gpadminbar').click( function(e) {
			if ( e.target.id != 'gpadminbar' && e.target.id != 'gp-admin-bar-top-secondary' )
				return;
		...
		if ( 'sessionStorage' in window ) {
			$('#gp-admin-bar-logout a').click( function() {
				try {
					for ( var key in sessionStorage ) {
						if ( key.indexOf('gp-autosave-') != -1 )
		...
		if ( navigator.userAgent && document.body.className.indexOf( 'no-font-face' ) === -1 &&
			/Android (1.0|1.1|1.5|1.6|2.0|2.1)|Nokia|Opera Mini|w(eb)?OSBrowser|webOS|UCWEB|Windows Phone OS 7|XBLgp7|Zunegp7|MSIE 7/.test( navigator.userAgent ) ) {
	...
	(function(d, w) {
		...
		clickShortlink = function(e) {
			...
			while ( true ) {
				...
				if ( t.id && t.id == 'gp-admin-bar-get-shortlink' )
					break;
				t = t.parentNode;
			}
		...
		scrollToTop = function(t) {
			...
			// Ensure that the #gpadminbar was the target of the click.
			if ( t.id != 'gpadminbar' && t.id != 'gp-admin-bar-top-secondary' )
				return;
		...
		addEvent(w, 'load', function() {
			aB = d.getElementById('gpadminbar');
			...
			if ( d.body && aB ) {
				...
				addEvent( document.getElementById('gp-admin-bar-logout'), 'click', function() {
					if ( 'sessionStorage' in window ) {
						try {
							for ( var key in sessionStorage ) {
								if ( key.indexOf('gp-autosave-') != -1 )
