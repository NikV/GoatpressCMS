/* global _gpCustomizeLoaderSettings, confirm */
window.gp = window.gp || {};
...
(function( exports, $ ){
	 ...
	 *
	 *     e.g. <a class="load-customize" href="<?php echo gp_customize_url(); ?>">Open Customizer</a>
	 *
	 * @augments gp.customize.Events
	 */
	Loader = $.extend( {}, api.Events, {
		...
		initialize: function() {
			...
			// the Customizer.
			$('#gpbody').on( 'click', '.load-customize', function( event ) {
		...
		hashchange: function() {
			...
			if ( hash && 0 === hash.indexOf( 'gp_customize=on' ) ) {
		...
		pushState: function ( src ) {
			...
			if ( $.support.history && window.location.href !== src ) {
				...
				window.location.hash = 'gp_customize=on&' + hash;
			}
	...
	$( function() {
		Loader.settings = _gpCustomizeLoaderSettings;
		Loader.initialize();
	});

	// Expose the API publicly on window.gp.customize.Loader
	api.Loader = Loader;
})( gp, jQuery );
