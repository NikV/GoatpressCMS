...
( function( window, gp, $ ) {
	...
	gp.mce.views = {
		 ...
		 * @param {Object} extend An object to extend gp.mce.View.prototype with.
		 */
		register: function( type, extend ) {
			views[ type ] = gp.mce.View.extend( _.extend( extend, { type: type } ) );
		},
			...
			_.each( views, function( view, type ) {
				...
				_.each( current, function( piece ) {
					...
					while ( remaining && ( result = view.prototype.match( remaining ) ) ) {
						...
						pieces.push( {
							content: '<p data-gpview-marker="' + instance.encodedText + '">' + instance.text + '</p>',
							processed: true
						} );
		 ...
		 *
		 * @return {gp.mce.View} The view instance.
		 */
		 ...
		 *
		 * @return {gp.mce.View} The view instance or undefined.
		 */
		getInstance: function( object ) {
			...
			return instances[ $( object ).attr( 'data-gpview-text' ) ];
		},
		...
		getText: function( node ) {
			return decodeURIComponent( $( node ).attr( 'data-gpview-text' ) || '' );
		},
	 ...
	 * @param {Object} options Options.
	 */
	gp.mce.View = function( options ) {
		_.extend( this, options );
		this.initialize();
	};

	gp.mce.View.extend = Backbone.View.extend;

	_.extend( gp.mce.View.prototype, {
		...
		render: function( force ) {
			...
			if ( this.getContent() ) {
				this.setContent( this.getContent(), function( editor, node ) {
					$( node ).data( 'rendered', true ).trigger( 'gp-mce-view-bind' );
				}, force ? null : false );
			} else {
				this.setLoader();
			}
		...
		unbind: function() {
			this.getNodes( function( editor, node ) {
				$( node ).trigger( 'gp-mce-view-unbind' );
			}, true );
		},
		...
		getNodes: function( callback, rendered ) {
			this.getEditors( function( editor ) {
				...
				$( editor.getBody() )
					.find( '[data-gpview-text="' + self.encodedText + '"]' )
					.filter( function() {
						var data;
					...
					.each( function() {
						callback.call( self, editor, this, $( this ).find( '.gpview-content' ).get( 0 ) );
					} );
			} );
		},
		...
		getMarkers: function( callback ) {
			this.getEditors( function( editor ) {
				...
				$( editor.getBody() )
					.find( '[data-gpview-marker="' + this.encodedText + '"]' )
					.each( function() {
		...
		replaceMarkers: function() {
			this.getMarkers( function( editor, node ) {
				if ( $( node ).text() !== this.text ) {
					editor.dom.setAttrib( node, 'data-gpview-marker', null );
					return;
				}

				editor.dom.replace(
					editor.dom.createFragment(
						'<div class="gpview-wrap" data-gpview-text="' + this.encodedText + '" data-gpview-type="' + this.type + '">' +
							'<p class="gpview-selection-before">\u00a0</p>' +
							'<div class="gpview-body" contenteditable="false">' +
								'<div class="gpview-content gpview-type-' + this.type + '"></div>' +
							'</div>' +
							'<p class="gpview-selection-after">\u00a0</p>' +
						'</div>'
					),
					node
				);
			...
			if ( _.isObject( content ) && content.body.indexOf( '<script' ) !== -1 ) {
				...
				this.getNodes( function( editor, node, contentNode ) {
					...
					if ( content.indexOf( '<iframe' ) !== -1 ) {
						content += '<div class="gpview-overlay"></div>';
					}
				...
				setTimeout( function() {
					...
					tinymce.each( dom.$(
						...
						if (
							...
							link.href.indexOf( 'skins/Goatpress/gp-content.css' ) === -1
						) {
					...
					dom.add( content, 'div', { 'class': 'gpview-overlay' } );
							...
							'<head>' +
								...
								'<style>' +
									'html {' +
										...
										'margin: 0;' +
									'}' +
									'body#gpview-iframe-sandbox {' +
										'background: transparent;' +
										'padding: 1px 0 !important;' +
										'margin: -1px 0 0 !important;' +
									'}' +
									'body#gpview-iframe-sandbox:before,' +
									'body#gpview-iframe-sandbox:after {' +
										'display: none;' +
										'content: "";' +
									'}' +
								'</style>' +
							'</head>' +
							'<body id="gpview-iframe-sandbox" class="' + bodyClasses + '">' +
								body +
							'</body>' +
						'</html>'
					);
					...
					if ( MutationObserver ) {
						...
						$( node ).one( 'gp-mce-view-unbind', function() {
							observer.disconnect();
						} );
					} else {
						for ( i = 1; i < 6; i++ ) {
					...
					editor.on( 'gp-body-class-change', classChange );

					$( node ).one( 'gp-mce-view-unbind', function() {
						editor.off( 'gp-body-class-change', classChange );
					} );
		...
		setLoader: function() {
			this.setContent(
				'<div class="loading-placeholder">' +
					...
					'<div class="gpview-loading"><ins></ins></div>' +
				'</div>'
			);
		},
		 ...
		 * @param {String} dashicon A dashicon ID (optional). {@link https://developer.Goatpress.org/resource/dashicons/}
		 */
		setError: function( message, dashicon ) {
			this.setContent(
				'<div class="gpview-error">' +
		...
		match: function( content ) {
			var match = gp.shortcode.next( this.type, content );
		...
		update: function( text, editor, node ) {
			_.find( views, function( view, type ) {
				...
				if ( match ) {
					...
					editor.dom.setAttrib( node, 'data-gpview-text', encodeURIComponent( text ) );
					gp.mce.views.createInstance( type, text, match.options ).render();
...
} )( window, window.gp, window.jQuery );

/*
 * The Goatpress core TinyMCE views.
...
( function( window, views, $ ) {
	...
	gallery = _.extend( {}, media, {
		state: [ 'gallery-edit' ],
		template: gp.media.template( 'editor-gallery' ),

		initialize: function() {
			var attachments = gp.media.gallery.attachments( this.shortcode, postID ),
			...
			.done( function() {
				...
				self.content = self.template( {
					...
					columns: attrs.columns ? parseInt( attrs.columns, 10 ) : gp.media.galleryDefaults.columns
				} );
	...
	av = _.extend( {}, media, {
		...
		initialize: function() {
			...
			if ( this.url ) {
				this.loader = false;
				this.shortcode = gp.media.embed.shortcode( {
					url: this.text
				} );
			}

			gp.ajax.send( this.action, {
				data: {
					post_ID: postID,
			...
			this.getEditors( function( editor ) {
				editor.on( 'gpview-selected', function() {
					self.pausePlayers();
				} );
			} );
		},
		...
		pausePlayers: function() {
			this.getNodes( function( editor, node, content ) {
				var win = $( 'iframe.gpview-sandbox', content ).get( 0 );
...
} )( window, window.gp.mce.views, window.jQuery );
