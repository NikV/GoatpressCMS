...
class gp_Embed {
	...
	public function __construct() {
		// Hack to get the [embed] shortcode to run before gpautop()
	 ...
	 * an AJAX request that will call gp_Embed::cache_oembed().
	 */
...
<?php
	...
	/**
	 * Register an embed handler. Do not use this function directly, use {@link gp_embed_register_handler()} instead.
	...
	/**
	 * Unregister a previously registered embed handler. Do not use this function directly, use {@link gp_embed_unregister_handler()} instead.
	 *
	 ...
	 * If none of the regex matches and it's enabled, then the URL will be given to the {@link gp_oEmbed} class.
	 *
	 * @param array $attr {
	...
	public function shortcode( $attr, $url = '' ) {
		...
		$rawattr = $attr;
		$attr = gp_parse_args( $attr, gp_embed_defaults( $url ) );

		$this->last_attr = $attr;

		// kses converts & into &amp; and we need to undo this
		// See https://core.trac.Goatpress.org/ticket/11311
		$url = str_replace( '&amp;', '&', $url );
		...
		if ( ! empty( $this->post_ID ) ) // Potentially set by gp_Embed::cache_oembed()
		...
		if ( $post_ID ) {
			...
			if ( $html ) {
				/** This filter is documented in gp-includes/class-gp-embed.php */
	...
	/**
	 * Passes any unlinked URLs that are on their own line to {@link gp_Embed::shortcode()} for potential embedding.
	 *
	 * @uses gp_Embed::autoembed_callback()
	 *
	...
	/**
	 * Callback function for {@link gp_Embed::autoembed()}.
	 *
