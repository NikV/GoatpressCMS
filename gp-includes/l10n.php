 ...
 *
 * If the locale is not set already, then the gpLANG constant is used if it is
 ...
 */
function get_locale() {
	global $locale, $gp_local_package;

	if ( isset( $locale ) ) {
		/**
		 * Filter Goatpress install's locale ID.
		 *
		 * @since 1.5.0
		 *
	...
	if ( isset( $gp_local_package ) ) {
		$locale = $gp_local_package;
	}

	// gpLANG was defined in gp-config.
	if ( defined( 'gpLANG' ) ) {
		$locale = gpLANG;
	}
	...
	if ( is_multisite() ) {
		...
		if ( defined( 'gp_INSTALLING' ) || ( false === $ms_locale = get_option( 'gpLANG' ) ) ) {
			$ms_locale = get_site_option( 'gpLANG' );
		}
	...
	/** This filter is documented in gp-includes/l10n.php */
	return apply_filters( 'locale', $locale );
}
 ...
 *
 * Loads the .mo file in gp_LANG_DIR constant path from Goatpress root.
...
function load_default_textdomain( $locale = null ) {
	...
	$return = load_textdomain( 'default', gp_LANG_DIR . "/$locale.mo" );

	if ( ( is_multisite() || ( defined( 'gp_INSTALLING_NETWORK' ) && gp_INSTALLING_NETWORK ) ) && ! file_exists(  gp_LANG_DIR . "/admin-$locale.mo" ) ) {
		load_textdomain( 'default', gp_LANG_DIR . "/ms-$locale.mo" );
		return $return;
	}

	if ( is_admin() || defined( 'gp_INSTALLING' ) || ( defined( 'gp_REPAIRING' ) && gp_REPAIRING ) ) {
		load_textdomain( 'default', gp_LANG_DIR . "/admin-$locale.mo" );
	}

	if ( is_network_admin() || ( defined( 'gp_INSTALLING_NETWORK' ) && gp_INSTALLING_NETWORK ) )
		load_textdomain( 'default', gp_LANG_DIR . "/admin-network-$locale.mo" );
 ...
 * @param string $plugin_rel_path Optional. Relative path to gp_PLUGIN_DIR where the .mo file resides.
 *                                Default false.
...
function load_plugin_textdomain( $domain, $deprecated = false, $plugin_rel_path = false ) {
	...
	if ( false !== $plugin_rel_path	) {
		$path = gp_PLUGIN_DIR . '/' . trim( $plugin_rel_path, '/' );
	...
	$mofile = gp_LANG_DIR . '/plugins/' . $mofile;
 ...
 *
 * @param string $domain             Text domain. Unique identifier for retrieving translated strings.
 * @param string $mu_plugin_rel_path Relative to gpMU_PLUGIN_DIR directory in which the .mo file resides.
...
function load_muplugin_textdomain( $domain, $mu_plugin_rel_path = '' ) {
	...
	$path = trailingslashit( gpMU_PLUGIN_DIR . '/' . ltrim( $mu_plugin_rel_path, '/' ) );
...
function load_theme_textdomain( $domain, $path = false ) {
	...
	// Otherwise, load from the languages directory
	$mofile = gp_LANG_DIR . "/themes/{$domain}-{$locale}.mo";
	return load_textdomain( $domain, $mofile );
}
 ...
 *
 * The default directory is gp_LANG_DIR.
 *
 * @since 3.0.0
 *
...
function get_available_languages( $dir = null ) {
	...
	foreach( (array)glob( ( is_null( $dir) ? gp_LANG_DIR : $dir ) . '/*.mo' ) as $lang_file ) {
 ...
 *
 * Looks in the gp-content/languages directory for translations of
 * plugins or themes.
 *
 * @since 3.7.0
 *
 ...
 */
function gp_get_installed_translations( $type ) {
	if ( $type !== 'themes' && $type !== 'plugins' && $type !== 'core' )
		return array();

	$dir = 'core' === $type ? '' : "/$type";

	if ( ! is_dir( gp_LANG_DIR ) )
		return array();

	if ( $dir && ! is_dir( gp_LANG_DIR . $dir ) )
		return array();

	$files = scandir( gp_LANG_DIR . $dir );
	if ( ! $files )
		return array();
	...
	foreach ( $files as $file ) {
		...
		$language_data[ $textdomain ][ $language ] = gp_get_pomo_file_data( gp_LANG_DIR . "$dir/$file" );
	}
 ...
 */
function gp_get_pomo_file_data( $po_file ) {
 ...
 * }
 */
function gp_dropdown_languages( $args = array() ) {
