...
function get_bookmark($bookmark, $output = OBJECT, $filter = 'raw') {
	...
	if ( empty($bookmark) ) {
		...
		gp_cache_add($bookmark->link_id, $bookmark, 'bookmark');
		$_bookmark = $bookmark;
	} else {
		if ( isset($GLOBALS['link']) && ($GLOBALS['link']->link_id == $bookmark) ) {
			$_bookmark = & $GLOBALS['link'];
		} elseif ( ! $_bookmark = gp_cache_get($bookmark, 'bookmark') ) {
			$_bookmark = $gpdb->get_row($gpdb->prepare("SELECT * FROM $gpdb->links WHERE link_id = %d LIMIT 1", $bookmark));
			if ( $_bookmark ) {
				$_bookmark->link_category = array_unique( gp_get_object_terms( $_bookmark->link_id, 'link_category', array( 'fields' => 'ids' ) ) );
				gp_cache_add( $_bookmark->link_id, $_bookmark, 'bookmark' );
			}
		}
	}
...
function get_bookmarks( $args = '' ) {
	...
	$key = md5( serialize( $r ) );
	if ( $cache = gp_cache_get( 'get_bookmarks', 'bookmark' ) ) {
	...
	if ( ! empty( $r['category_name'] ) ) {
		if ( $r['category'] = get_term_by('name', $r['category_name'], 'link_category') ) {
			...
			$cache[ $key ] = array();
			gp_cache_set( 'get_bookmarks', $cache, 'bookmark' );
			/** This filter is documented in gp-includes/bookmark.php */
	...
	$search = '';
	if ( ! empty( $r['search'] ) ) {
		$like = '%' . $gpdb->esc_like( $r['search'] ) . '%';
		$search = $gpdb->prepare(" AND ( (link_url LIKE %s) OR (link_name LIKE %s) OR (link_description LIKE %s) ) ", $like, $like, $like );
	}
	...
	if ( ! empty( $category_query ) ) {
		...
		$join = " INNER JOIN $gpdb->term_relationships AS tr ON ($gpdb->links.link_id = tr.object_id) INNER JOIN $gpdb->term_taxonomy as tt ON tt.term_taxonomy_id = tr.term_taxonomy_id";
	}
	...
	$query = "SELECT * $length $recently_updated_test $get_updated FROM $gpdb->links $join WHERE 1=1 $visible $category_query";
...
function clean_bookmark_cache( $bookmark_id ) {
	gp_cache_delete( $bookmark_id, 'bookmark' );
	gp_cache_delete( 'get_bookmarks', 'bookmark' );
