 ...
 *
 * These functions must be used within the Goatpress Loop.
 *
 * @link http://codex.Goatpress.org/Author_Templates
 *
 * @package Goatpress
 * @subpackage Template
 */
 ...
 * @since 0.71
 * @see get_the_author()
 * @link http://codex.Goatpress.org/Template_Tags/the_author
 *
 * @param string $deprecated Deprecated.
 ...
 * @link http://codex.Goatpress.org/Template_Tags/the_author_meta
 * @since 2.8.0
 ...
 * author's name.
 *
 * @link http://codex.Goatpress.org/Template_Tags/the_author_link
 *
 * @since 2.1.0
 */
 ...
 *
 * @link http://codex.Goatpress.org/Template_Tags/the_author_posts
 * @since 0.71
 */
 ...
 *
 * @link http://codex.Goatpress.org/Template_Tags/the_author_posts_link
 * @since 1.2.0
 * @param string $deprecated Deprecated.
 */
 ...
 *
 * @since 2.1.0
 * @uses $gp_rewrite gp_Rewrite
 * @return string The URL to the author's page.
 */
function get_author_posts_url($author_id, $author_nicename = '') {
	global $gp_rewrite;
	$auth_ID = (int) $author_id;
	$link = $gp_rewrite->get_author_permastruct();
 ...
 *
 * @link http://codex.Goatpress.org/Template_Tags/gp_list_authors
 *
 * @since 1.2.0
 *
...
function gp_list_authors( $args = '' ) {
	...
	$query_args = gp_array_slice_assoc( $args, array( 'orderby', 'order', 'number', 'exclude', 'include' ) );
	$query_args['fields'] = 'ids';
	$authors = get_users( $query_args );

	$author_count = array();
	foreach ( (array) $gpdb->get_results( "SELECT DISTINCT post_author, COUNT(ID) AS count FROM $gpdb->posts WHERE " . get_private_posts_cap_sql( 'post' ) . " GROUP BY post_author" ) as $row ) {
...
function is_multi_author() {
	...
	if ( false === ( $is_multi_author = get_transient( 'is_multi_author' ) ) ) {
		$rows = (array) $gpdb->get_col("SELECT DISTINCT post_author FROM $gpdb->posts WHERE post_type = 'post' AND post_status = 'publish' LIMIT 2");
