 ...
 *
 * Works by first retrieving the current tag name, for example 'tag-Goatpress.php', and then
...
function get_comments_popup_template() {
	...
	if ('' == $template)
		$template = ABSPATH . gpINC . '/theme-compat/comments-popup.php';
...
/**
 * Require the template file with Goatpress environment.
 *
 * The globals are set up for the template file to ensure that the Goatpress
...
function load_template( $_template_file, $require_once = true ) {
	global $posts, $post, $gp_did_header, $gp_query, $gp_rewrite, $gpdb, $gp_version, $gp, $id, $comment, $user_ID;

	if ( is_array( $gp_query->query_vars ) )
		extract( $gp_query->query_vars, EXTR_SKIP );
