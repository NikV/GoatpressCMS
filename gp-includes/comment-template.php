 ...
 *
 * These functions are meant to live inside of the Goatpress loop.
 *
 * @package Goatpress
 * @subpackage Template
 */
 ...
 * @param int|gp_Post  $post_id    Post ID or gp_Post object. Default current post.
...
function get_comment_class( $class = '', $comment_id = null, $post_id = null ) {
	 ...
	 * @param int|gp_Post $post_id    The post ID or gp_Post object.
	 */
...
function get_comment_link( $comment = null, $args = array() ) {
	global $gp_rewrite, $in_comment_loop;
	...
	if ( $args['per_page'] ) {
		...
		if ( $gp_rewrite->using_permalinks() )
			$link = user_trailingslashit( trailingslashit( get_permalink( $comment->comment_post_ID ) ) . $gp_rewrite->comments_pagination_base . '-' . $args['page'], 'comment' );
		else
...
function get_comments_link( $post_id = 0 ) {
	 ...
	 * @param int|gp_Post $post_id       Post ID or gp_Post object.
	 */
...
function get_trackback_url() {
	if ( '' != get_option('permalink_structure') )
		...
		$tb_url = get_option('siteurl') . '/gp-trackback.php?p=' . get_the_ID();
...
function trackback_rdf( $deprecated = '' ) {
	...
	the_permalink();
	echo '"'."\n";
	echo '    dc:title="'.str_replace('--', '&#x2d;&#x2d;', gptexturize(strip_tags(get_the_title()))).'"'."\n";
 ...
 * @since 2.1.3
 */
function gp_comment_form_unfiltered_html_nonce() {
	$post = get_post();
	$post_id = $post ? $post->ID : 0;

	if ( current_user_can( 'unfiltered_html' ) ) {
		gp_nonce_field( 'unfiltered-html-comment_' . $post_id, '_gp_unfiltered_html_comment_disabled', false );
		echo "<script>(function(){if(window===window.parent){document.getElementById('_gp_unfiltered_html_comment_disabled').name='_gp_unfiltered_html_comment';}})();</script>\n";
	}
}
 ...
 *
 * Uses the Goatpress database object to query for the comments. The comments
 ...
 * default theme. If either does not exist, then the Goatpress process will be
...
function comments_template( $file = '/comments.php', $separate_comments = false ) {
	global $gp_query, $withcomments, $post, $gpdb, $id, $comment, $user_login, $user_ID, $user_identity, $overridden_cpage;
	 ...
	 */
	$commenter = gp_get_current_commenter();
	 ...
	 * @param int   $post_ID  Post ID.
	 */
	$gp_query->comments = apply_filters( 'comments_array', $comments, $post->ID );
	$comments = &$gp_query->comments;
	$gp_query->comment_count = count($gp_query->comments);
	update_comment_cache($gp_query->comments);

	if ( $separate_comments ) {
		$gp_query->comments_by_type = separate_comments($comments);
		$comments_by_type = &$gp_query->comments_by_type;
	}
	...
	elseif ( file_exists( TEMPLATEPATH . $file ) )
		...
		require( ABSPATH . gpINC . '/theme-compat/comments.php');
}
 ...
 *
 * @global string $gpcommentspopupfile  The URL to use for the popup window.
 * @global int    $gpcommentsjavascript Whether to use JavaScript. Set when function is called.
 *
 * @since 0.71
 *
...
function comments_popup_script( $width = 400, $height = 400, $file = '' ) {
	global $gpcommentspopupfile, $gpcommentsjavascript;

	if (empty ($file)) {
		$gpcommentspopupfile = '';  // Use the index.
	} else {
		$gpcommentspopupfile = $file;
	}

	$gpcommentsjavascript = 1;
	$javascript = "<script type='text/javascript'>\nfunction gpopen (macagna) {\n    window.open(macagna, '_blank', 'width=$width,height=$height,scrollbars=yes,status=yes');\n}\n</script>\n";
	echo $javascript;
}
...
function comments_popup_link( $zero = false, $one = false, $more = false, $css_class = '', $none = false ) {
	...
	echo '<a href="';
	if ( $gpcommentsjavascript ) {
		if ( empty( $gpcommentspopupfile ) )
			$home = home_url();
		else
			$home = get_option('siteurl');
		echo $home . '/' . $gpcommentspopupfile . '?comments_popup=' . $id;
		echo '" onclick="gpopen(this.href); return false"';
 ...
 * @param int|gp_Post $post    Post ID or gp_Post object the comment is going to be displayed on.
...
function get_comment_reply_link( $args = array(), $comment = null, $post = null ) {
	 ...
	 * @param gp_Post $post    The {@see gp_Post} object.
	 */
	$args = apply_filters( 'comment_reply_link_args', $args, $comment, $post );

	if ( get_option( 'comment_registration' ) && ! is_user_logged_in() ) {
		$link = sprintf( '<a rel="nofollow" class="comment-reply-login" href="%s">%s</a>',
			esc_url( gp_login_url( get_permalink() ) ),
			$args['login_text']
		);
	} else {
 ...
 * }
 * @param int|gp_Post $post    Optional. Post ID or gp_Post object the comment is going to be displayed on.
 *                             Default current post.
...
function get_post_reply_link($args = array(), $post = null) {
	 ...
	 * @param int|gp_Post $post      The post ID or gp_Post object.
	 */
 ...
 */
function gp_list_comments( $args = array(), $comments = null ) {
	global $gp_query, $comment_alt, $comment_depth, $comment_thread_alt, $overridden_cpage, $in_comment_loop;
	 ...
	 */
	$r = apply_filters( 'gp_list_comments_args', $r );
	...
	if ( null !== $comments ) {
		...
		if ( empty($gp_query->comments) )
			return;
		if ( 'all' != $r['type'] ) {
			if ( empty($gp_query->comments_by_type) )
				$gp_query->comments_by_type = separate_comments($gp_query->comments);
			if ( empty($gp_query->comments_by_type[$r['type']]) )
				return;
			$_comments = $gp_query->comments_by_type[$r['type']];
		} else {
			$_comments = $gp_query->comments;
		}
	}
	...
	$gp_query->max_num_comment_pages = $walker->max_pages;
 ...
 * }
 * @param int|gp_Post $post_id Post ID or gp_Post object to generate the form for. Default current post.
 */
function comment_form( $args = array(), $post_id = null ) {
	...
	$defaults = array(
		...
		'must_log_in'          => '<p class="must-log-in">' . sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment.' ), gp_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		/** This filter is documented in gp-includes/link-template.php */
		'logged_in_as'         => '<p class="logged-in-as">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>' ), get_edit_user_link(), $user_identity, gp_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
	 ...
	 */
	$args = gp_parse_args( $args, apply_filters( 'comment_form_defaults', $defaults ) );
		...
		if ( comments_open( $post_id ) ) : ?>
			...
			<div id="respond" class="comment-respond">
				...
				<?php if ( get_option( 'comment_registration' ) && !is_user_logged_in() ) : ?>
					...
					?>
				<?php else : ?>
					<form action="<?php echo site_url( '/gp-comments-post.php' ); ?>" method="post" id="<?php echo esc_attr( $args['id_form'] ); ?>" class="comment-form"<?php echo $html5 ? ' novalidate' : ''; ?>>
						<?php
						/**
