 ...
 * @since 2.8.0
 */
class gp_Widget_Pages extends gp_Widget {
	...
	public function widget( $args, $instance ) {
		 ...
		 */
		$out = gp_list_pages( apply_filters( 'widget_pages_args', array(
			'title_li'    => '',
			'echo'        => 0,
			'sort_column' => $sortby,
			'exclude'     => $exclude
	...
	public function form( $instance ) {
		//Defaults
		$instance = gp_parse_args( (array) $instance, array( 'sortby' => 'post_title', 'title' => '', 'exclude' => '') );
 ...
 * @since 2.8.0
 */
class gp_Widget_Links extends gp_Widget {
	...
	public function widget( $args, $instance ) {
		 ...
		 */
		gp_list_bookmarks( apply_filters( 'widget_links_args', array(
	...
	public function form( $instance ) {
		...
		//Defaults
		$instance = gp_parse_args( (array) $instance, array( 'images' => true, 'name' => true, 'description' => false, 'rating' => false, 'category' => false, 'orderby' => 'name', 'limit' => -1 ) );
 ...
 * @since 2.8.0
 */
class gp_Widget_Search extends gp_Widget {
	...
	public function widget( $args, $instance ) {
		...
		/** This filter is documented in gp-includes/default-widgets.php */
	...
	public function form( $instance ) {
		$instance = gp_parse_args( (array) $instance, array( 'title' => '') );
...
<?php
	...
	public function update( $new_instance, $old_instance ) {
		$instance = $old_instance;
		$new_instance = gp_parse_args((array) $new_instance, array( 'title' => ''));
		$instance['title'] = strip_tags($new_instance['title']);
		return $instance;
	}
 ...
 * @since 2.8.0
 */
class gp_Widget_Archives extends gp_Widget {
...
?>
		...
		<select id="<?php echo esc_attr( $dropdown_id ); ?>" name="archive-dropdown" onchange='document.location.href=this.options[this.selectedIndex].value;'>
			...
			<?php gp_get_archives( $dropdown_args ); ?>
...
<?php
		 ...
		 */
		gp_get_archives( apply_filters( 'widget_archives_args', array(
			'type'            => 'monthly',
			'show_post_count' => $c
		) ) );
?>
...
<?php
	...
	public function update( $new_instance, $old_instance ) {
		$instance = $old_instance;
		$new_instance = gp_parse_args( (array) $new_instance, array( 'title' => '', 'count' => 0, 'dropdown' => '') );
		$instance['title'] = strip_tags($new_instance['title']);
	...
	public function form( $instance ) {
		$instance = gp_parse_args( (array) $instance, array( 'title' => '', 'count' => 0, 'dropdown' => '') );
 ...
 * @since 2.8.0
 */
class gp_Widget_Meta extends gp_Widget {

	public function __construct() {
		$widget_ops = array('classname' => 'widget_meta', 'description' => __( "Login, RSS, &amp; Goatpress.org links.") );
...
<?php
			/**
			 * Filter the "Powered by Goatpress" text in the Meta widget.
			 *
			 * @since 3.6.0
			 *
			 * @param string $title_text Default title text for the Goatpress.org link.
			 */
			echo apply_filters( 'widget_meta_poweredby', sprintf( '<li><a href="%s" title="%s">%s</a></li>',
				esc_url( __( 'https://Goatpress.org/' ) ),
				esc_attr__( 'Powered by Goatpress, state-of-the-art semantic personal publishing platform.' ),
				_x( 'Goatpress.org', 'meta widget link text' )
			) );
 ...
 * @since 2.8.0
 */
class gp_Widget_Calendar extends gp_Widget {
 ...
 * @since 2.8.0
 */
class gp_Widget_Text extends gp_Widget {
	...
	public function widget( $args, $instance ) {
		 ...
		 * @param gp_Widget $instance    gp_Widget instance.
		 */
		...
		if ( ! empty( $title ) ) {
			...
			<div class="textwidget"><?php echo !empty( $instance['filter'] ) ? gpautop( $text ) : $text; ?></div>
		<?php
		echo $args['after_widget'];
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		if ( current_user_can('unfiltered_html') )
			...
			$instance['text'] = stripslashes( gp_filter_post_kses( addslashes($new_instance['text']) ) ); // gp_filter_post_kses() expects slashed
		$instance['filter'] = ! empty( $new_instance['filter'] );
		return $instance;
	}

	public function form( $instance ) {
		$instance = gp_parse_args( (array) $instance, array( 'title' => '', 'text' => '' ) );
		$title = strip_tags($instance['title']);
 ...
 * @since 2.8.0
 */
class gp_Widget_Categories extends gp_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if ( $d ) {
			 ...
			 */
			gp_dropdown_categories( apply_filters( 'widget_categories_dropdown_args', $cat_args ) );
?>
...
<?php
		 ...
		 */
		gp_list_categories( apply_filters( 'widget_categories_args', $cat_args ) );
?>
		</ul>
<?php
		}
 ...
 * @since 2.8.0
 */
class gp_Widget_Recent_Posts extends gp_Widget {
	...
	public function widget($args, $instance) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = gp_cache_get( 'widget_recent_posts', 'widget' );
		}
		 ...
		 * @since 3.4.0
		 *
		 * @see gp_Query::get_posts()
		 *
		 * @param array $args An array of arguments used to retrieve the recent posts.
		 */
		$r = new gp_Query( apply_filters( 'widget_posts_args', array(
...
<?php
		...
		if ( ! $this->is_preview() ) {
			...
			gp_cache_set( 'widget_recent_posts', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		$alloptions = gp_cache_get( 'alloptions', 'options' );
	...
	public function flush_widget_cache() {
		gp_cache_delete('widget_recent_posts', 'widget');
	}
 ...
 * @since 2.8.0
 */
class gp_Widget_Recent_Comments extends gp_Widget {
	...
	public function __construct() {
		...
		if ( is_active_widget(false, false, $this->id_base) )
			add_action( 'gp_head', array($this, 'recent_comments_style') );
...
<?php
	...
	public function flush_widget_cache() {
		gp_cache_delete('widget_recent_comments', 'widget');
	}
	...
	public function widget( $args, $instance ) {
		...
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = gp_cache_get('widget_recent_comments', 'widget');
		}
		if ( ! is_array( $cache ) ) {
			$cache = array();
		}
		 ...
		 * @since 3.4.0
		 *
		 * @see gp_Comment_Query::query() for information on accepted arguments.
		 *
		...
		if ( $comments ) {
			...
			$post_ids = array_unique( gp_list_pluck( $comments, 'comment_post_ID' ) );
		...
		if ( ! $this->is_preview() ) {
			...
			gp_cache_set( 'widget_recent_comments', $cache, 'widget' );
		}
	}
 ...
 * @since 2.8.0
 */
class gp_Widget_RSS extends gp_Widget {
	...
	public function widget($args, $instance) {
		...
		gp_widget_rss_output( $rss, $instance );
		echo $args['after_widget'];
	...
	public function update($new_instance, $old_instance) {
		...
		return gp_widget_rss_process( $new_instance, $testurl );
	}
 ...
 */
function gp_widget_rss_output( $rss, $args = array() ) {
	if ( is_string( $rss ) ) {
		$rss = fetch_feed($rss);
	...
	foreach ( $rss->get_items( 0, $items ) as $item ) {
		...
		$desc = esc_attr( gp_trim_words( $desc, 55, ' [&hellip;]' ) );
 ...
 */
function gp_widget_rss_form( $args, $inputs = null ) {
	$default_inputs = array( 'url' => true, 'title' => true, 'items' => true, 'show_summary' => true, 'show_author' => true, 'show_date' => true );
	$inputs = gp_parse_args( $inputs, $default_inputs );
 ...
 * @return array
 */
function gp_widget_rss_process( $widget_rss, $check_feed = true ) {
 ...
 * @since 2.8.0
 */
class gp_Widget_Tag_Cloud extends gp_Widget {
	...
	public function widget( $args, $instance ) {
		 ...
		 */
		gp_tag_cloud( apply_filters( 'widget_tag_cloud_args', array(
 ...
 * @since 3.0.0
 */
 class gp_Nav_Menu_Widget extends gp_Widget {
	...
	public function widget($args, $instance) {
		// Get menu
		$nav_menu = ! empty( $instance['nav_menu'] ) ? gp_get_nav_menu_object( $instance['nav_menu'] ) : false;
		 ...
		 *     An array of arguments passed to gp_nav_menu() to retrieve a custom menu.
		 *
		 ...
		 */
		gp_nav_menu( apply_filters( 'widget_nav_menu_args', $nav_menu_args, $nav_menu, $args ) );
...
/**
 * Register all of the default Goatpress widgets on startup.
 *
 * Calls 'widgets_init' action after all of the Goatpress widgets have been
 * registered.
 *
 * @since 2.2.0
 */
function gp_widgets_init() {
	...
	register_widget('gp_Widget_Pages');

	register_widget('gp_Widget_Calendar');

	register_widget('gp_Widget_Archives');

	if ( get_option( 'link_manager_enabled' ) )
		register_widget('gp_Widget_Links');

	register_widget('gp_Widget_Meta');

	register_widget('gp_Widget_Search');

	register_widget('gp_Widget_Text');

	register_widget('gp_Widget_Categories');

	register_widget('gp_Widget_Recent_Posts');

	register_widget('gp_Widget_Recent_Comments');

	register_widget('gp_Widget_RSS');

	register_widget('gp_Widget_Tag_Cloud');

	register_widget('gp_Nav_Menu_Widget');

	/**
	 * Fires after all default Goatpress widgets have been registered.
	 *
	 * @since 2.2.0
	 */
...
add_action('init', 'gp_widgets_init', 1);
