 ...
 *
 * @see gp_Rewrite::add_rule() for long description.
 * @since 2.1.0
 *
...
function add_rewrite_rule($regex, $redirect, $after = 'bottom') {
	global $gp_rewrite;
	$gp_rewrite->add_rule($regex, $redirect, $after);
}
 ...
 *
 * @see gp_Rewrite::add_rewrite_tag()
 * @since 2.1.0
 *
...
function add_rewrite_tag( $tag, $regex, $query = '' ) {
	...
	$gp_rewrite->add_rewrite_tag( $tag, $regex, $query );
}

/**
 * Add permalink structure.
 *
 * @see gp_Rewrite::add_permastruct()
 * @since 3.0.0
 *
 * @param string $name Name for permalink structure.
 * @param string $struct Permalink structure.
 * @param array $args Optional configuration for building the rules from the permalink structure,
 *     see {@link gp_Rewrite::add_permastruct()} for full details.
 */
function add_permastruct( $name, $struct, $args = array() ) {
	...
	return $gp_rewrite->add_permastruct( $name, $struct, $args );
}
...
function add_feed($feedname, $function) {
	global $gp_rewrite;
	if ( ! in_array($feedname, $gp_rewrite->feeds) ) //override the file if it is
		$gp_rewrite->feeds[] = $feedname;
...
function add_rewrite_endpoint( $name, $places, $query_var = null ) {
	global $gp_rewrite;
	$gp_rewrite->add_endpoint( $name, $places, $query_var );
}
 ...
 * @return string
 */
function _gp_filter_taxonomy_base( $base ) {
	if ( !empty( $base ) ) {
...
function url_to_postid($url) {
	...
	$rewrite = $gp_rewrite->gp_rewrite_rules();
	...
	if ( !$gp_rewrite->using_index_permalinks() )
		$url = str_replace( $gp_rewrite->index . '/', '', $url );
	...
	foreach ( (array)$rewrite as $match => $query) {
		...
		if ( preg_match("#^$match#", $request_match, $matches) ) {
			...
			if ( $gp_rewrite->use_verbose_page_rules && preg_match( '/pagename=\$matches\[([0-9]+)\]/', $query, $varmatch ) ) {
			...
			$query = addslashes(gp_MatchesMapRegex::apply($query, $matches));
			...
			foreach ( (array) $query_vars as $key => $value ) {
				if ( in_array( $key, $gp->public_query_vars ) ){
					$query[$key] = $value;
...
/**
 * Goatpress Rewrite Component.
 *
 * The Goatpress Rewrite class writes the rewrite module rules to the .htaccess
 * file. It also handles parsing the request to get the correct setup for the
 * Goatpress Query class.
 *
 * The Rewrite along with gp class function as a front controller for Goatpress.
...
class gp_Rewrite {
	 ...
	 * {@link gp_Rewrite::$index} with a trailing slash appended. Otherwise
	 * the root will be empty.
	 *
	 * @see gp_Rewrite::init()
	 * @see gp_Rewrite::using_index_permalinks()
	 * @since 1.5.0
	 * @var string
	 */
	public $root = '';
	...
	/**
	 * Rules that don't redirect to Goatpress' index.php.
	 *
	...
	/**
	 * Whether to write every mod_rewrite rule for Goatpress into the .htaccess file.
	 *
	 * This is off by default, turning it on might print a lot of rewrite rules
	 * to the .htaccess file.
	 *
	 * @see gp_Rewrite::mod_rewrite_rules()
	 * @since 2.0.0
	 * @access public
	 * @var bool
	 */
	 ...
	 * set to true. Prior to Goatpress 3.3 this flag indicated that every page
	 * would have a set of rules added to the top of the rewrite rules array.
	 * Now it tells {@link gp::parse_request()} to check if a URL matching the
	 * page permastruct is actually a page before accepting it.
	 *
	 * @link https://core.trac.Goatpress.org/ticket/16687
	 * @see gp_Rewrite::init()
	 * @since 2.5.0
	 * @access public
	 * @var bool
	 ...
	 * {@link gp_Rewrite::$rewritereplace} and are rewritten to the
	 * query variables listed in {@link gp_Rewrite::$queryreplace}.
	 *
	 ...
	 * of rewrite tags, see {@link gp_Rewrite::$rewritecode}.
	 *
	 * @since 1.5.0
	 * @access private
	 * @var array
	...
	/**
	 * Query variables that rewrite tags map to, see {@link gp_Rewrite::$rewritecode}.
	 *
	 * @since 1.5.0
	 * @access private
	 * @var array
	...
	public function page_uri_index() {
		...
		$pages = $gpdb->get_results("SELECT ID, post_name, post_parent FROM $gpdb->posts WHERE post_type = 'page' AND post_status != 'auto-draft'");
		...
		foreach ( $posts as $id => $post ) {
			// URL => page name
			$uri = get_page_uri($id);
			$attachments = $gpdb->get_results( $gpdb->prepare( "SELECT ID, post_name, post_parent FROM $gpdb->posts WHERE post_type = 'attachment' AND post_parent = %d", $id ));
	 ...
	 *
	 * @see gp_Rewrite::$rewritecode
	 * @see gp_Rewrite::$rewritereplace
	 * @see gp_Rewrite::$queryreplace
	 * @since 1.5.0
	 * @access public
	 *
	 ...
	 *
	 * The main gp_Rewrite function for building the rewrite rule list. The
	 ...
	 *
	 * Shorten version of {@link gp_Rewrite::generate_rewrite_rules()} that
	 * allows for shorter list of parameters. See the method for longer
	 * description of what generating rewrite rules does.
	 *
	 * @uses gp_Rewrite::generate_rewrite_rules() See for long description and rest of parameters.
	 * @since 1.5.0
	 * @access public
	 *
	 ...
	 * reference to the current gp_Rewrite instance to further manipulate the
	...
	public function rewrite_rules() {
		...
		$deprecated_files = array(
			'.*gp-(atom|rdf|rss|rss2|feed|commentsrss2)\.php$' => $this->index . '?feed=old',
			'.*gp-app\.php(/.*)?$' => $this->index . '?error=403',
		);

		// Registration rules
		$registration_pages = array();
		if ( is_multisite() && is_main_site() ) {
			$registration_pages['.*gp-signup.php$'] = $this->index . '?signup=true';
			$registration_pages['.*gp-activate.php$'] = $this->index . '?activate=true';
		}
		$registration_pages['.*gp-register.php$'] = $this->index . '?register=true'; // Deprecated
		 ...
		 * @since 1.5.0
		 *
		 * @param gp_Rewrite $this Current gp_Rewrite instance, passed by reference.
		 */
	 ...
	 * gp_Rewrite::rewrite_rules()} is that this method stores the rewrite rules
	 ...
	 */
	public function gp_rewrite_rules() {
	 ...
	 *
	 * Will add the non_gp_rules property rules to the .htaccess file before
	 * the Goatpress rewrite rules one.
	 *
	 * @since 1.5.0
	 * @access public
	 *
	...
	public function mod_rewrite_rules() {
		...
		//add in the rules that don't redirect to gp's index.php (and thus shouldn't be handled by gp at all)
		foreach ( (array) $this->non_gp_rules as $match => $query) {
	...
	<system.webServer>
		...
		$rules .= '
			<rule name="Goatpress" patternSyntax="Wildcard">
				<match url="*" />
					<conditions>
	...
	public function add_rule($regex, $redirect, $after = 'bottom') {
		...
		if ( $front != $this->index ) { //it doesn't redirect to gp's index.php
	...
	public function add_external_rule($regex, $redirect) {
		$this->non_gp_rules[$regex] = $redirect;
	}
	 ...
	 * The new permastruct is added to the {@link gp_Rewrite::$extra_permastructs} array. When the
	 * rewrite rules are built by {@link gp_Rewrite::rewrite_rules()} all of these extra permastructs
	 * are passed to {@link gp_Rewrite::generate_rewrite_rules()} which transforms them into the
	 * regular expressions that many love to hate.
	 *
	 * The $args parameter gives you control over how {@link gp_Rewrite::generate_rewrite_rules()}
	 ...
	 * @param array $args Optional configuration for building the rules from the permalink structure:
	 *     - with_front (bool) - Should the structure be prepended with gp_Rewrite::$front? Default is true.
	 ...
	 * Remove rewrite rules and then recreate rewrite rules.
	 *
	 * Calls {@link gp_Rewrite::gp_rewrite_rules()} after removing the
	...
	public function flush_rules($hard = true) {
		...
		if ( ! did_action( 'gp_loaded' ) ) {
			add_action( 'gp_loaded', array( $this, 'flush_rules' ) );
			return;
		}

		delete_option('rewrite_rules');
		$this->gp_rewrite_rules();
		/**
	...
	public function init() {
		$this->extra_rules = $this->non_gp_rules = $this->endpoints = array();
	 ...
	 * {@link gp_Rewrite::init()} after the option is updated.
	 *
