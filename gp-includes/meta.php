 ...
 * Metadata API
 *
 * Functions for retrieving and manipulating metadata of various Goatpress object types. Metadata
...
function add_metadata($meta_type, $object_id, $meta_key, $meta_value, $unique = false) {
	...
	if ( $unique && $gpdb->get_var( $gpdb->prepare(
	...
	$result = $gpdb->insert( $table, array(
		$column => $object_id,
		'meta_key' => $meta_key,
	...
	$mid = (int) $gpdb->insert_id;

	gp_cache_delete($object_id, $meta_type . '_meta');
...
function update_metadata($meta_type, $object_id, $meta_key, $meta_value, $prev_value = '') {
	...
	$meta_ids = $gpdb->get_col( $gpdb->prepare( "SELECT $id_column FROM $table WHERE meta_key = %s AND $column = %d", $meta_key, $object_id ) );
	if ( empty( $meta_ids ) ) {
	...
	$result = $gpdb->update( $table, $data, $where );
	if ( ! $result )
		return false;
...
function delete_metadata($meta_type, $object_id, $meta_key, $meta_value = '', $delete_all = false) {
	...
	$query = $gpdb->prepare( "SELECT $id_column FROM $table WHERE meta_key = %s", $meta_key );

	if ( !$delete_all )
		$query .= $gpdb->prepare(" AND $type_column = %d", $object_id );

	if ( $meta_value )
		$query .= $gpdb->prepare(" AND meta_value = %s", $meta_value );

	$meta_ids = $gpdb->get_col( $query );
	if ( !count( $meta_ids ) )
		return false;

	if ( $delete_all )
		$object_ids = $gpdb->get_col( $gpdb->prepare( "SELECT $type_column FROM $table WHERE meta_key = %s", $meta_key ) );
	...
	if ( $delete_all ) {
		foreach ( (array) $object_ids as $o_id ) {
			gp_cache_delete($o_id, $meta_type . '_meta');
		}
	} else {
		gp_cache_delete($object_id, $meta_type . '_meta');
	}
...
function get_metadata($meta_type, $object_id, $meta_key = '', $single = false) {
	...
	$meta_cache = gp_cache_get($object_id, $meta_type . '_meta');
...
function metadata_exists( $meta_type, $object_id, $meta_key ) {
	...
	/** This filter is documented in gp-includes/meta.php */
...
function get_metadata_by_mid( $meta_type, $meta_id ) {
	...
	$meta = $gpdb->get_row( $gpdb->prepare( "SELECT * FROM $table WHERE $id_column = %d", $meta_id ) );
...
function update_metadata_by_mid( $meta_type, $meta_id, $meta_value, $meta_key = false ) {
	...
	if ( $meta = get_metadata_by_mid( $meta_type, $meta_id ) ) {
		...
		/** This action is documented in gp-includes/meta.php */
		do_action( "update_{$meta_type}_meta", $meta_id, $object_id, $meta_key, $_meta_value );
		...
		$result = $gpdb->update( $table, $data, $where, '%s', '%d' );
		if ( ! $result )
			return false;
...
function delete_metadata_by_mid( $meta_type, $meta_id ) {
	...
	if ( $meta = get_metadata_by_mid( $meta_type, $meta_id ) ) {
		...
		$result = (bool) $gpdb->delete( $table, array( $id_column => $meta_id ) );
...
function update_meta_cache($meta_type, $object_ids) {
	...
	foreach ( $object_ids as $id ) {
		$cached_object = gp_cache_get( $id, $cache_key );
	...
	$id_list = join( ',', $ids );
	$id_column = 'user' == $meta_type ? 'umeta_id' : 'meta_id';
	$meta_list = $gpdb->get_results( "SELECT $column, meta_key, meta_value FROM $table WHERE $column IN ($id_list) ORDER BY $id_column ASC", ARRAY_A );
	...
	foreach ( $ids as $id ) {
		if ( ! isset($cache[$id]) )
			$cache[$id] = array();
		gp_cache_add( $id, $cache[$id], $cache_key );
	}
...
function get_meta_sql( $meta_query, $type, $primary_table, $primary_id_column, $context = null ) {
	$meta_query_obj = new gp_Meta_Query( $meta_query );
 ...
 *
 * `gp_Meta_Query` is a helper that allows primary query classes, such as {@see gp_Query} and {@see gp_User_Query},
...
class gp_Meta_Query {
	 ...
	 *
	 * See {@see gp_Meta_Query::__construct()} for information on meta query arguments.
	 *
	 * @since 3.2.0
	 * @access public
	 * @var array
	...
	public function parse_query_vars( $qv ) {
		...
		// gp_Query sets 'meta_value' = '' by default.
	 ...
	 *
	 * Called by the public {@see gp_Meta_Query::get_sql()}, this method
	...
	public function get_sql_for_clause( &$clause, $parent_query, $clause_key = '' ) {
		...
		if ( false === $alias ) {
			...
			if ( 'NOT EXISTS' === $meta_compare ) {
				...
				$join .= $i ? " AS $alias" : '';
				$join .= $gpdb->prepare( " ON ($this->primary_table.$this->primary_id_column = $alias.$this->meta_id_column AND $alias.meta_key = %s )", $clause['key'] );
		...
		if ( array_key_exists( 'key', $clause ) ) {
			if ( 'NOT EXISTS' === $meta_compare ) {
				...
				$sql_chunks['where'][] = $gpdb->prepare( "$alias.meta_key = %s", trim( $clause['key'] ) );
			}
		}
		...
		if ( array_key_exists( 'value', $clause ) ) {
			...
			switch ( $meta_compare ) {
				...
				case 'NOT IN' :
					...
					$where = $gpdb->prepare( $meta_compare_string, $meta_value );
					break;

				case 'BETWEEN' :
				case 'NOT BETWEEN' :
					$meta_value = array_slice( $meta_value, 0, 2 );
					$where = $gpdb->prepare( '%s AND %s', $meta_value );
					break;

				case 'LIKE' :
				case 'NOT LIKE' :
					$meta_value = '%' . $gpdb->esc_like( $meta_value ) . '%';
					$where = $gpdb->prepare( '%s', $meta_value );
					break;
	 ...
	 * In the case of {@see gp_Meta_Query}, this only applies to 'IN' clauses that
	...
	protected function find_compatible_table_alias( $clause, $parent_query ) {
		 ...
		 * @param object      $this         gp_Meta_Query object.
		 */
...
function _get_meta_table($type) {
	...
	if ( empty($gpdb->$table_name) )
		return false;

	return $gpdb->$table_name;
}
