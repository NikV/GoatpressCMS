 ...
 */
_deprecated_file( basename( __FILE__ ), '3.0', gpINC . '/class-simplepie.php' );
...
define('RSS', 'RSS');
define('ATOM', 'Atom');
define('MAGPIE_USER_AGENT', 'Goatpress/' . $GLOBALS['gp_version']);
...
/**
 * Retrieve URL headers and content using gp HTTP Request API.
 *
 * @since 1.5.0
 * @package External
 * @subpackage MagpieRSS
...
function _fetch_remote_file($url, $headers = "" ) {
	$resp = gp_safe_remote_request( $url, array( 'headers' => $headers, 'timeout' => MAGPIE_FETCH_TIME_OUT ) );
	if ( is_gp_error($resp) ) {
	...
	// Also note, gp_HTTP lowercases all keys, Snoopy did not.
	$return_headers = array();
	foreach ( gp_remote_retrieve_headers( $resp ) as $key => $value ) {
		if ( !is_array($value) ) {
	...
	$response->status = gp_remote_retrieve_response_code( $resp );
	$response->response_code = gp_remote_retrieve_response_code( $resp );
	$response->headers = $return_headers;
	$response->results = gp_remote_retrieve_body( $resp );
...
function init () {
	...
	if ( !defined('MAGPIE_USER_AGENT') ) {
		$ua = 'Goatpress/' . $GLOBALS['gp_version'];
...
class RSSCache {
	...
	function RSSCache ($base='', $age='') {
		$this->BASE_CACHE = gp_CONTENT_DIR . '/cache';
		if ( $base ) {
 ...
 */
function gp_rss( $url, $num_items = -1 ) {
 ...
 * to display. You can't display all of them like you can with gp_rss()
 * function.
 *
 * @since 1.5.0
 * @package External
