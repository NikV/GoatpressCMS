 ...
 * Used internally by the {@link gp_Embed} class, but is designed to be generic.
 *
 * @link http://codex.Goatpress.org/oEmbed oEmbed Codex Article
...
class gp_oEmbed {
	...
	public function __construct() {
		$providers = array(
			...
			'#https?://Goatpress.tv/.*#i'                         => array( 'http://Goatpress.tv/oembed/',                        true  ),
			...
			'#https?://(www\.)?issuu\.com/.+/docs/.+#i'           => array( 'http://issuu.com/oembed_gp',                         true  ),
		 ...
		 * | Goatpress.tv | Goatpress.tv         |  Yes  | 2.9.0     |
		...
		// Fix any embeds that contain new lines in the middle of the HTML which breaks gpautop().
	...
	/**
	 * Add an oEmbed provider just-in-time when gp_oembed_add_provider() is called
	...
	/**
	 * Remove an oEmbed provider just-in-time when gp_oembed_remove_provider() is called
	 * before the 'plugins_loaded' hook.
	 *
	...
	public function discover( $url ) {
		...
		// Fetch URL content
		$request = gp_safe_remote_get( $url, $args );
		if ( $html = gp_remote_retrieve_body( $request ) ) {
	...
	public function fetch( $provider, $url, $args = '' ) {
		$args = gp_parse_args( $args, gp_embed_defaults( $url ) );
		...
		foreach( array( 'json', 'xml' ) as $format ) {
			...
			if ( is_gp_error( $result ) && 'not-implemented' == $result->get_error_code() )
				continue;
			return ( $result && ! is_gp_error( $result ) ) ? $result : false;
		}
		return false;
	}
	...
	private function _fetch_with_format( $provider_url_with_args, $format ) {
		...
		/** This filter is documented in gp-includes/class-oembed.php */
		$args = apply_filters( 'oembed_remote_get_args', array(), $provider_url_with_args );

		$response = gp_safe_remote_get( $provider_url_with_args, $args );
		if ( 501 == gp_remote_retrieve_response_code( $response ) )
			return new gp_Error( 'not-implemented' );
		if ( ! $body = gp_remote_retrieve_body( $response ) )
			return false;
	...
	/**
	 * Converts a data object from {@link gp_oEmbed::fetch()} and returns the HTML.
	 *
	 ...
	 * @param object $data Data object from gp_oEmbed::data2html()
...
/**
 * Returns the initialized {@link gp_oEmbed} object
 *
 * @since 2.9.0
 * @access private
 *
 * @see gp_oEmbed
 *
 * @return gp_oEmbed object.
 */
function _gp_oembed_get_object() {
	static $gp_oembed;

	if ( is_null($gp_oembed) )
		$gp_oembed = new gp_oEmbed();
