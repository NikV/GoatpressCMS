<?php
/**
 * Goatpress Customize Widgets classes
 *
 * @package Goatpress
 ...
 * @since 3.9.0
 *
 * @see gp_Customize_Manager
 */
final class gp_Customize_Widgets {

	/**
	 * gp_Customize_Manager instance.
	 *
	 * @since 3.9.0
	 * @access public
	 * @var gp_Customize_Manager
	 */
	public $manager;
	 ...
	 * @access public
	 *
	 * @param gp_Customize_Manager $manager Customize manager bootstrap instance.
	 */
	public function __construct( $manager ) {
		...
		add_action( 'gp_loaded',                               array( $this, 'override_sidebars_widgets_for_theme_switch' ) );
	...
	public function register_settings() {
		...
		if ( $this->manager->doing_ajax( 'update-widget' ) && isset( $_REQUEST['widget-id'] ) ) {
			$widget_setting_ids[] = $this->get_setting_id( gp_unslash( $_REQUEST['widget-id'] ) );
		}
		 ...
		 * But don't do this if a customize_save because this will cause gp to think there is nothing
	...
	public function override_sidebars_widgets_for_theme_switch() {
		...
		$this->old_sidebars_widgets = gp_get_sidebars_widgets();
		...
		unset( $GLOBALS['_gp_sidebars_widgets'] ); // reset global cache var used by gp_get_sidebars_widgets()
	}
	 ...
	 * theme_mod.
	 *
	 * @see gp_Customize_Widgets::handle_theme_switch()
	 * @since 3.9.0
	 * @access public
	 *
	 ...
	 *
	 * Note: these actions are also fired in gp_ajax_update_widget().
	 *
	 * @since 3.9.0
	 * @access public
	 */
	public function customize_controls_init() {
		/** This action is documented in gp-admin/includes/ajax-actions.php */
	 ...
	 * after Goatpress is loaded so that all filters have been
	...
	public function schedule_customize_register() {
		if ( is_admin() ) {
			...
			add_action( 'gp', array( $this, 'customize_register' ) );
		}
	}
	...
	public function customize_register() {
		global $gp_registered_widgets, $gp_registered_widget_controls, $gp_registered_sidebars;

		$sidebars_widgets = array_merge(
			array( 'gp_inactive_widgets' => array() ),
			array_fill_keys( array_keys( $GLOBALS['gp_registered_sidebars'] ), array() ),
			gp_get_sidebars_widgets()
		);
		 ...
		 */
		foreach ( array_keys( $gp_registered_widgets ) as $widget_id ) {
		...
		foreach ( $sidebars_widgets as $sidebar_id => $sidebar_widget_ids ) {
			...
			$is_registered_sidebar = isset( $GLOBALS['gp_registered_sidebars'][$sidebar_id] );
			$is_inactive_widgets   = ( 'gp_inactive_widgets' === $sidebar_id );
			...
			if ( $is_registered_sidebar || $is_inactive_widgets ) {
				...
				if ( $is_active_sidebar ) {
					...
					$section_args = array(
						'title' => $GLOBALS['gp_registered_sidebars'][ $sidebar_id ]['name'],
						'description' => $GLOBALS['gp_registered_sidebars'][ $sidebar_id ]['description'],
						'priority' => array_search( $sidebar_id, array_keys( $gp_registered_sidebars ) ),
						'panel' => 'widgets',
					...
					$section = new gp_Customize_Sidebar_Section( $this->manager, $section_id, $section_args );
					$this->manager->add_section( $section );

					$control = new gp_Widget_Area_Customize_Control( $this->manager, $setting_id, array(
						'section'    => $section_id,
						'sidebar_id' => $sidebar_id,
			...
			foreach ( $sidebar_widget_ids as $i => $widget_id ) {
				...
				if ( ! $is_active_sidebar || ! isset( $GLOBALS['gp_registered_widgets'][$widget_id] ) ) {
					continue;
				}

				$registered_widget = $GLOBALS['gp_registered_widgets'][$widget_id];
				$setting_id        = $this->get_setting_id( $widget_id );
				$id_base           = $GLOBALS['gp_registered_widget_controls'][$widget_id]['id_base'];

				$control = new gp_Widget_Form_Customize_Control( $this->manager, $setting_id, array(
					...
					'priority'       => $i,
					'width'          => $gp_registered_widget_controls[$widget_id]['width'],
					'height'         => $gp_registered_widget_controls[$widget_id]['height'],
	 ...
	 * @return gp_Error|array Array containing a widget's id_base and number components,
	 *                        or a gp_Error object.
	 */
	public function parse_widget_setting_id( $setting_id ) {
		if ( ! preg_match( '/^(widget_(.+?))(?:\[(\d+)\])?$/', $setting_id, $matches ) ) {
			return new gp_Error( 'widget_setting_invalid_id' );
		}
	...
	public function enqueue_scripts() {
		gp_enqueue_style( 'customize-widgets' );
		gp_enqueue_script( 'customize-widgets' );
		...
		$settings = array(
			'nonce'                => gp_create_nonce( 'update-widget' ),
			'registeredSidebars'   => array_values( $GLOBALS['gp_registered_sidebars'] ),
			'registeredWidgets'    => $GLOBALS['gp_registered_widgets'],
		...
		$gp_scripts->add_data(
			'customize-widgets',
			'data',
			sprintf( 'var _gpCustomizeWidgetsSettings = %s;', gp_json_encode( $settings ) )
		);
	}
	...
	public function get_available_widgets() {
		...
		global $gp_registered_widgets, $gp_registered_widget_controls;
		require_once ABSPATH . '/gp-admin/includes/widgets.php'; // for next_widget_id_number()

		$sort = $gp_registered_widgets;
		...
		foreach ( $sort as $widget ) {
			...
			$is_disabled     = false;
			$is_multi_widget = ( isset( $gp_registered_widget_controls[$widget['id']]['id_base'] ) && isset( $widget['params'][0]['number'] ) );
			if ( $is_multi_widget ) {
				$id_base            = $gp_registered_widget_controls[$widget['id']]['id_base'];
				...
				if ( $sidebar && 'gp_inactive_widgets' !== $sidebar ) {
					$is_disabled = true;
				}
				$id_base = $widget['id'];
			}

			$list_widget_controls_args = gp_list_widget_controls_dynamic_sidebar( array( 0 => $args, 1 => $widget['params'][0] ) );
			...
			$available_widget = array_merge( $available_widget, array(
				...
				'id_base'      => $id_base,
				'transport'    => 'refresh',
				'width'        => $gp_registered_widget_controls[$widget['id']]['width'],
				'height'       => $gp_registered_widget_controls[$widget['id']]['height'],
	...
	public function get_widget_control( $args ) {
		...
		call_user_func_array( 'gp_widget_control', $args );
		$replacements = array(
	...
	public function customize_preview_init() {
		add_action( 'gp_enqueue_scripts', array( $this, 'customize_preview_enqueue' ) );
		add_action( 'gp_print_styles',    array( $this, 'print_preview_css' ), 1 );
		add_action( 'gp_footer',          array( $this, 'export_preview_data' ), 20 );
	}

	/**
	 * When previewing, make sure the proper previewing widgets are used.
	 *
	 * Because gp_get_sidebars_widgets() gets called early at init
	 * (via gp_convert_widget_settings()) and can set global variable
	 * $_gp_sidebars_widgets to the value of get_option( 'sidebars_widgets' )
	...
	public function customize_preview_enqueue() {
		gp_enqueue_script( 'customize-preview-widgets' );
	}
	...
	/**
	 * At the very end of the page, at the very end of the gp_footer,
	...
	public function export_preview_data() {
		...
		<script type="text/javascript">
			var _gpWidgetCustomizerPreviewSettings = <?php echo gp_json_encode( $settings ); ?>;
		</script>
		<?php
	}
	...
	public function tally_sidebars_via_is_active_sidebar_calls( $is_active, $sidebar_id ) {
		if ( isset( $GLOBALS['gp_registered_sidebars'][$sidebar_id] ) ) {
	...
	protected function get_instance_hash_key( $serialized_instance ) {
		return gp_hash( $serialized_instance );
	}
	...
	public function sanitize_sidebar_widgets_js_instance( $widget_ids ) {
		...
		$widget_ids = array_values( array_intersect( $widget_ids, array_keys( $gp_registered_widgets ) ) );
		return $widget_ids;
	}
	 ...
	 *
	 * @param  string $widget_id Widget ID.
	 * @return gp_Error|array Array containing the updated widget information.
	 *                        A gp_Error object, otherwise.
	 */
	public function call_widget_update( $widget_id ) {
		global $gp_registered_widget_updates, $gp_registered_widget_controls;
		...
		if ( ! empty( $_POST['sanitized_widget_setting'] ) ) {
			...
			if ( false === $sanitized_widget_setting ) {
				...
				return new gp_Error( 'widget_setting_malformed' );
			}

			$instance = $this->sanitize_widget_instance( $sanitized_widget_setting );
			if ( is_null( $instance ) ) {
				$this->stop_capturing_option_updates();
				return new gp_Error( 'widget_setting_unsanitized' );
			}
			...
			if ( ! is_null( $parsed_id['number'] ) ) {
				...
				$_REQUEST[$key] = $_POST[$key] = gp_slash( $value );
		...
		if ( 0 !== $this->count_captured_options() ) {
			if ( $this->count_captured_options() > 1 ) {
				$this->stop_capturing_option_updates();
				return new gp_Error( 'widget_setting_too_many_options' );
			}

			$updated_option_name = key( $this->get_captured_options() );
			if ( $updated_option_name !== $option_name ) {
				$this->stop_capturing_option_updates();
				return new gp_Error( 'widget_setting_unexpected_option' );
			}
		}
		 ...
		 * in place from gp_Customize_Setting::preview() will use this value
		...
		ob_start();
		$form = $gp_registered_widget_controls[ $widget_id ];
		if ( $form ) {
	 ...
	 *
	 * Most code here copied from gp_ajax_save_widget()
	 *
	 * @since 3.9.0
	 * @access public
	 *
	 * @see gp_ajax_save_widget()
	 *
	 */
	public function gp_ajax_update_widget() {
		...
		if ( empty( $_POST['widget-id'] ) ) {
			gp_send_json_error( 'missing_widget-id' );
		}
		...
		if ( $is_updating_widget_template ) {
			gp_send_json_error( 'template_widget_not_updatable' );
		}

		$updated_widget = $this->call_widget_update( $widget_id ); // => {instance,form}
		if ( is_gp_error( $updated_widget ) ) {
			gp_send_json_error( $updated_widget->get_error_message() );
		}

		$form = $updated_widget['form'];
		$instance = $this->sanitize_widget_js_instance( $updated_widget['instance'] );

		gp_send_json_success( compact( 'form', 'instance' ) );
	}
	...
	public function capture_filter_pre_get_option( $value ) {
		...
		if ( isset( $this->_captured_options[ $option_name ] ) ) {
			...
			/** This filter is documented in gp-includes/option.php */
