...
class Featured_Content {
	...
	/**
	 * Conditionally hook into Goatpress.
	 *
	 * Theme must declare that they support this module by adding
	 * add_theme_support( 'featured-content' ); during after_setup_theme.
	 *
	 * If no theme support is found there is no need to hook into Goatpress.
	...
	public static function init() {
		...
		add_action( 'gp_loaded',                          array( __CLASS__, 'gp_loaded'          )    );
	}

	/**
	 * Hide "featured" tag from the front-end.
	 *
	 * Has to run on gp_loaded so that the preview filters of the Customizer
	 ...
	 * @since Twenty Fourteen 1.0
	 */
	public static function gp_loaded() {
	 ...
	 * @since Twenty Fourteen 1.0
	 *
	 * @param gp_Query $query gp_Query object.
	 * @return gp_Query Possibly-modified gp_Query.
	 */
	 ...
	 */
	public static function customize_register( $gp_customize ) {
		...
		$gp_customize->add_setting( 'featured-content[tag-name]', array(
			'default'              => _x( 'featured', 'featured content default tag slug', 'twentyfourteen' ),
			'type'                 => 'option',
			'sanitize_js_callback' => array( __CLASS__, 'delete_transient' ),
		) );
		$gp_customize->add_setting( 'featured-content[hide-tag]', array(
			'default'              => true,
			'type'                 => 'option',
			'sanitize_js_callback' => array( __CLASS__, 'delete_transient' ),
		) );
		...
		$gp_customize->add_control( 'featured-content[tag-name]', array(
			'label'    => __( 'Tag Name', 'twentyfourteen' ),
			'section'  => 'featured_content',
			'priority' => 20,
		) );
		$gp_customize->add_control( 'featured-content[hide-tag]', array(
	...
	public static function enqueue_scripts() {
		gp_enqueue_script( 'featured-content-suggest', get_template_directory_uri() . '/js/featured-content-admin.js', array( 'jquery', 'suggest' ), '20131022', true );
	}
	...
	public static function get_setting( $key = 'all' ) {
		...
		$options = gp_parse_args( $saved, $defaults );
	...
	public static function validate_settings( $input ) {
		...
		if ( empty( $input['tag-name'] ) ) {
			...
			if ( $term ) {
				...
				$new_tag = gp_create_tag( $input['tag-name'] );

				if ( ! is_gp_error( $new_tag ) && isset( $new_tag['term_id'] ) ) {
