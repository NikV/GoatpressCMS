 ...
 * http://codex.Goatpress.org/The_Loop to understand it and
 * http://codex.Goatpress.org/Template_Tags to understand
 * the tags used in it.
 *
				...
				<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
					...
					<div class="entry-meta">
						...
						<?php
							...
							if ( gp_attachment_is_image() ) {
								echo ' <span class="meta-sep">|</span> ';
								$metadata = gp_get_attachment_metadata();
								printf( __( 'Full size is %s pixels', 'twentyten' ),
									sprintf( '<a href="%1$s" title="%2$s">%3$s &times; %4$s</a>',
										esc_url( gp_get_attachment_url() ),
...
?>
						<p class="attachment"><a href="<?php echo esc_url( $next_attachment_url ); ?>" title="<?php the_title_attribute(); ?>" rel="attachment"><?php
							...
							echo gp_get_attachment_image( $post->ID, array( $attachment_width, $attachment_height ) ); // filterable image width with, essentially, no limit for image height.
						?></a></p>
...
<?php else : ?>
						<a href="<?php echo esc_url( gp_get_attachment_url() ); ?>" title="<?php the_title_attribute(); ?>" rel="attachment"><?php echo esc_html( basename( get_permalink() ) ); ?></a>
<?php endif; ?>
						</div><!-- .entry-attachment -->
						<div class="entry-caption"><?php if ( ! empty( $post->post_excerpt ) ) the_excerpt(); ?></div>

<?php the_content( __( 'Continue reading <span class="meta-nav">&rarr;</span>', 'twentyten' ) ); ?>
<?php gp_link_pages( array( 'before' => '<div class="page-link">' . __( 'Pages:', 'twentyten' ), 'after' => '</div>' ) ); ?>
