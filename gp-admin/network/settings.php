...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __('<a href="http://codex.Goatpress.org/Network_Admin_Settings_Screen" target="_blank">Documentation on Network Settings</a>') . '</p>' .
...
if ( $_POST ) {
	...
	$options = array(
		...
		'illegal_names', 'limited_email_domains', 'banned_email_domains', 'gpLANG', 'admin_email',
	);

	// Handle translation install.
	if ( ! empty( $_POST['gpLANG'] ) && gp_can_install_language_pack() ) {  // @todo: Skip if already installed
	...
	foreach ( $options as $option_name ) {
		if ( ! isset($_POST[$option_name]) )
			continue;
		$value = gp_unslash( $_POST[$option_name] );
	 ...
	 * @since MU
	 */
	do_action( 'update_gpmu_options' );

	gp_redirect( add_query_arg( 'updated', 'true', network_admin_url( 'settings.php' ) ) );
	exit();
}
...
<div class="wrap">
	...
	<form method="post" action="settings.php" novalidate="novalidate">
		<?php gp_nonce_field( 'siteoptions' ); ?>
			...
			<tr>
				...
				<td>
					...
					<?php if ( is_subdomain_install() ) {
						echo '<p class="description">' . __( 'If registration is disabled, please set <code>NOBLOGREDIRECT</code> in <code>gp-config.php</code> to a URL you will redirect visitors to if they visit a non-existent site.' ) . '</p>';
					} ?>
					</fieldset>
				</td>
			</tr>
...
<?php echo esc_textarea( get_site_option( 'first_comment' ) ) ?></textarea>
		...
		if ( ! empty( $languages ) || ! empty( $translations ) ) {
			...
			<table class="form-table">
				<tr>
					<th><label for="gpLANG"><?php _e( 'Default Language' ); ?></label></th>
					<td>
						<?php
						$lang = get_site_option( 'gpLANG' );
						...
						gp_dropdown_languages( array(
							...
							'show_available_translations' => gp_can_install_language_pack(),
						) );
						?>
					</td>
				</tr>
