...
if ( ! current_user_can('manage_sites') )
	gp_die(__('You do not have sufficient permissions to edit this site.'));
...
get_current_screen()->add_help_tab( array(
	...
	'content' =>
		...
		'<p>' . __('<strong>Settings</strong> - This page shows a list of all settings associated with this site. Some are created by Goatpress and others are created by plugins you activate. Note that some fields are grayed out and say Serialized Data. You cannot modify these values due to the way the setting is stored in the database.') . '</p>'
) );
...
$referer = remove_query_arg( 'update', gp_get_referer() );
...
if ( $action ) {
	...
	switch ( $action ) {
		case 'newuser':
			check_admin_referer( 'add-user', '_gpnonce_add-new-user' );
			$user = $_POST['user'];
			if ( ! is_array( $_POST['user'] ) || empty( $user['username'] ) || empty( $user['email'] ) ) {
				$update = 'err_new';
			} else {
				$password = gp_generate_password( 12, false);
				$user_id = gpmu_create_user( esc_html( strtolower( $user['username'] ) ), $password, esc_html( $user['email'] ) );
	...
	gp_safe_redirect( add_query_arg( 'update', $update, $referer ) );
	exit();
}
 ...
 */
if ( ! gp_is_large_network( 'users' ) && apply_filters( 'show_network_site_users_add_existing_form', true ) )
...
/** This filter is documented in gp-admin/network/site-users.php */
...
<form action="site-users.php?action=adduser" id="adduser" method="post">
	...
	<table class="form-table">
		<tr>
			...
			<td><input type="text" class="regular-text gp-suggest-user" name="newuser" id="newuser" /></td>
		</tr>
		<tr>
...
<form action="<?php echo network_admin_url('site-users.php?action=newuser'); ?>" id="newuser" method="post">
	...
	<?php gp_nonce_field( 'add-user', '_gpnonce_add-new-user' ) ?>
