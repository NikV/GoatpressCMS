...
function confirm_delete_users( $users ) {
	...
	<?php
	gp_nonce_field( 'ms-users-delete' );
	...
	<?php foreach ( ( $allusers = (array) $_POST['allusers'] ) as $user_id ) {
		if ( $user_id != '' && $user_id != '0' ) {
			...
			if ( ! current_user_can( 'delete_user', $delete_user->ID ) ) {
				gp_die( sprintf( __( 'Warning! User %s cannot be deleted.' ), $delete_user->user_login ) );
			}

			if ( in_array( $delete_user->user_login, $site_admins ) ) {
				gp_die( sprintf( __( 'Warning! User cannot be deleted. The user %s is a network administrator.' ), '<em>' . $delete_user->user_login . '</em>' ) );
			}
			?>
			<tr>
	...
	<?php
	/** This action is documented in gp-admin/users.php */
...
if ( isset( $_GET['action'] ) ) {
	/** This action is documented in gp-admin/network/edit.php */
	...
	switch ( $_GET['action'] ) {
		case 'deleteuser':
			...
			if ( $id != '0' && $id != '1' ) {
				...
				gp_redirect( network_admin_url( 'users.php' ) );
			}
			exit();
		...
		case 'allusers':
			...
			if ( ( isset( $_POST['action']) || isset($_POST['action2'] ) ) && isset( $_POST['allusers'] ) ) {
						...
						switch ( $doaction ) {
							...
							case 'spam':
								...
								if ( is_super_admin( $user->ID ) )
									gp_die( sprintf( __( 'Warning! User cannot be modified. The user %s is a network administrator.' ), esc_html( $user->user_login ) ) );
				...
				gp_safe_redirect( add_query_arg( array( 'updated' => 'true', 'action' => $userfunction ), gp_get_referer() ) );
			} else {
		...
		case 'dodelete':
			...
			gp_redirect( add_query_arg( array( 'updated' => 'true', 'action' => $deletefunction ), network_admin_url( 'users.php' ) ) );
			exit();
	}
}

$gp_list_table = _get_list_table('gp_MS_Users_List_Table');
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __('<a href="http://codex.Goatpress.org/Network_Admin_Users_Screen" target="_blank">Documentation on Network Users</a>') . '</p>' .
...
<div class="wrap">
	...
	<form method="get" class="search-form">
		<?php $gp_list_table->search_box( __( 'Search Users' ), 'all-user' ); ?>
	</form>
...
<?php require_once( ABSPATH . 'gp-admin/admin-footer.php' ); ?>
