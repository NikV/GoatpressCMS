...
$gp_list_table = _get_list_table( 'gp_MS_Sites_List_Table' );
...
if ( isset( $_GET['action'] ) ) {
	...
	if ( 'confirm' === $_GET['action'] ) {
		...
		if ( $current_site->blog_id == $id ) {
			gp_die( __( 'You are not allowed to change the current site.' ) );
		}
		...
		?>
			<div class="wrap">
				...
				<form action="sites.php?action=<?php echo esc_attr( $_GET['action2'] ) ?>" method="post">
					...
					<input type="hidden" name="_gp_http_referer" value="<?php echo esc_attr( gp_get_referer() ); ?>" />
					<?php gp_nonce_field( $_GET['action2'], '_gpnonce', false ); ?>
					<p><?php echo esc_html( gp_unslash( $_GET['msg'] ) ); ?></p>
						...
						switch ( $doaction ) {
							case 'delete':
								if ( ! current_user_can( 'delete_site', $val ) )
									gp_die( __( 'You are not allowed to delete the site.' ) );

								$updated_action = 'all_delete';
								gpmu_delete_blog( $val, true );
							break;
	...
	if ( ! empty( $updated_action ) ) {
		gp_safe_redirect( add_query_arg( array( 'updated' => $updated_action ), gp_get_referer() ) );
		exit();
	}
}
...
<?php $gp_list_table->search_box( __( 'Search Sites' ), 'site' ); ?>
