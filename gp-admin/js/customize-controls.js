/* globals _gpCustomizeHeader, _gpCustomizeBackground, _gpMediaViewsL10n, MediaElementPlayer */
(function( exports, $ ){
	var Container, focus, api = gp.customize;

	/**
	 * @class
	 * @augments gp.customize.Value
	 * @augments gp.customize.Class
	 *
	 * @param options
	 ...
	 * @since 4.1.0
	 *
	 * @param {gp.customize.Class} instance
	 ...
	 * @since 4.1.0
	 *
	 * @param {(gp.customize.Panel|gp.customize.Section|gp.customize.Control)} a
	 * @param {(gp.customize.Panel|gp.customize.Section|gp.customize.Control)} b
	 * @returns {Number}
	 */
	 ...
	 *
	 * @constructor
	 * @augments gp.customize.Section
	 * @augments gp.customize.Container
	 */
	api.ThemesSection = api.Section.extend({
		...
		ready: function () {
			...
			section.template = gp.template( 'customize-themes-details-view' );
		...
		onChangeExpanded: function ( expanded, args ) {
			...
			var position, scroll,
				...
				overlay = section.closest( '.gp-full-overlay' ),
				container = section.closest( '.gp-full-overlay-sidebar-content' ),
	 ...
	 *
	 * @see PHP class gp_Customize_Control.
	 *
	 * @class
	 * @augments gp.customize.Class
	 *
	...
	api.Control = api.Class.extend({
		...
		renderContent: function () {
			...
			if ( 0 !== $( '#tmpl-' + control.templateSelector ).length ) {
				template = gp.template( control.templateSelector );
	...
	api.ColorControl = api.Control.extend({
		ready: function() {
			...
			picker.val( control.setting() ).gpColorPicker({
				change: function() {
					control.setting.set( picker.gpColorPicker('color') );
				},
				clear: function() {
			...
			this.setting.bind( function ( value ) {
				picker.val( value );
				picker.gpColorPicker( 'color', value );
			});
		}
	});
	...
	api.MediaControl = api.Control.extend({
		...
		cleanupPlayer: function () {
			this.player && gp.media.mixin.removePlayer( this.player );
		},
			...
			this.frame = gp.media({
				...
				states: [
					new gp.media.controller.Library({
						...
						library:   gp.media.query({ type: this.params.mime_type }),
						multiple:  false,
						date:      false
					})
				]
		...
		select: function() {
			...
			var node,
				...
				mejsSettings = window._gpmejsSettings || {};
	 ...
	 *
	 * @class
	 * @augments gp.customize.MediaControl
	 * @augments gp.customize.Control
	 * @augments gp.customize.Class
	 */
	 ...
	 *
	 * @class
	 * @augments gp.customize.UploadControl
	 * @augments gp.customize.MediaControl
	 * @augments gp.customize.Control
	 * @augments gp.customize.Class
	 */
	...
	api.BackgroundControl = api.UploadControl.extend({
		...
		select: function() {
			...
			gp.ajax.post( 'custom-background-add', {
				nonce: _gpCustomizeBackground.nonces.add,
				gp_customize: 'on',
	...
	api.HeaderControl = api.Control.extend({
		 ...
		 * @param {gp.media.controller.Cropper} controller
		 * @returns {Object} Options
		 */
		calculateImageSelectOptions: function(attachment, controller) {
			var xInit = parseInt(_gpCustomizeHeader.data.width, 10),
				yInit = parseInt(_gpCustomizeHeader.data.height, 10),
				flexWidth = !! parseInt(_gpCustomizeHeader.data['flex-width'], 10),
				flexHeight = !! parseInt(_gpCustomizeHeader.data['flex-height'], 10),
			...
			this.frame = gp.media({
				...
				states: [
					new gp.media.controller.Library({
						...
						date:      false,
						priority:  20,
						suggestedWidth: _gpCustomizeHeader.data.width,
						suggestedHeight: _gpCustomizeHeader.data.height
					}),
					new gp.media.controller.Cropper({
		...
		/**
		 * Creates a new gp.customize.HeaderTool.ImageModel from provided
	...
	api.ThemeControl = api.Control.extend({
		...
		ready: function() {
			...
			control.container.on( 'click keydown touchend', '.theme', function( event ) {
				...
				$( '.gp-full-overlay' ).addClass( 'customize-loading' );
	...
	/**
	 * @class
	 * @augments gp.customize.Messenger
	 * @augments gp.customize.Class
	 * @mixes gp.customize.Events
	 */
	...
	api.Previewer = api.Messenger.extend({
		...
		initialize: function( params, options ) {
			...
			this.add( 'previewUrl', params.previewUrl ).setter( function( to ) {
				...
				// Check for URLs that include "/gp-admin/" or end in "/gp-admin".
				// Strip hashes and query strings before testing.
				if ( /\/gp-admin(\/|$)/.test( to.replace( /[#?].*$/, '' ) ) )
					return null;
	...
	$( function() {
		api.settings = window._gpCustomizeSettings;
		api.l10n = window._gpCustomizeControlsL10n;
		...
		var parent, topFocus,
			body = $( document.body ),
			overlay = body.children( '.gp-full-overlay' ),
		...
		api.previewer = new api.Previewer({
			...
			signature:   'gp_CUSTOMIZER_SIGNATURE'
		}, {
			...
			save: function() {
				...
				submit = function () {
					...
					request = gp.ajax.post( 'customize_save', query );
		 ...
		 * @since 4.1.0
		 */
		api.reflogpaneContents = _.bind( function () {
		...
		api.bind( 'ready', api.reflogpaneContents );
		api.reflogpaneContents = _.debounce( api.reflogpaneContents, 100 );
		$( [ api.panel, api.section, api.control ] ).each( function ( i, values ) {
			values.bind( 'add', api.reflogpaneContents );
			values.bind( 'change', api.reflogpaneContents );
			values.bind( 'remove', api.reflogpaneContents );
		} );
