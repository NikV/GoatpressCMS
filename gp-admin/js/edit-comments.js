/* global adminCommentsL10n, thousandsSeparator, list_args, QTags, ajaxurl, gpAjax */
...
setCommentsList = function() {
	...
	delBefore = function( settings, list ) {
		var note, id, el, n, h, a, author,
			action = false,
			gpListsData = $( settings.target ).attr( 'data-gp-lists' );
		...
		if ( gpListsData.indexOf(':trash=1') != -1 )
			action = 'trash';
		else if ( gpListsData.indexOf(':spam=1') != -1 )
			action = 'spam';

		if ( action ) {
			id = gpListsData.replace(/.*?comment-([0-9]+).*/, '$1');
			el = $('#comment-' + id);
			...
			a = $('.undo a', '#undo-' + id);
			a.attr('href', 'comment.php?action=un' + action + 'comment&c=' + id + '&_gpnonce=' + settings.data._ajax_nonce);
			a.attr('data-gp-lists', 'delete:the-comment-list:comment-' + id + '::un' + action + '=1');
			a.attr('class', 'vim-z vim-destructive');
			$('.avatar', el).first().clone().prependTo('#undo-' + id + ' .' + action + '-undo-inside');

			a.click(function(){
				list.gpList.del(this);
	...
	delAfter = function( r, settings ) {
		...
		theList.get(0).gpList.add( theExtraList.children(':eq(0)').remove().clone() );
	...
	refillTheExtraList = function(ev) {
		...
		if (ev) {
			theExtraList.empty();
			args.number = Math.min(8, per_page); // see gp_Comments_List_Table::prepare_items() @ class-gp-comments-list-table.php
		} else {
			args.number = 1;
		...
		$.ajax({
			...
			success: function(response) {
				theExtraList.get(0).gpList.add( response.rows );
			}
		});
	};

	theExtraList = $('#the-extra-comment-list').gpList( { alt: '', delColor: 'none', addColor: 'none' } );
	theList = $('#the-comment-list').gpList( { alt: '', delBefore: delBefore, dimAfter: dimAfter, delAfter: delAfter, addColor: 'none' } )
		.bind('gpListDelEnd', function(e, s){
			var gpListsData = $(s.target).attr('data-gp-lists'), id = s.element.replace(/[^0-9]+/g, '');

			if ( gpListsData.indexOf(':trash=1') != -1 || gpListsData.indexOf(':spam=1') != -1 )
...
commentReply = {
	...
	show : function(xml) {
		...
		r = gpAjax.parseAjaxResponse(xml);
		if ( r.errors ) {
			t.error({'responseText': gpAjax.broken});
			return false;
		}
...
$(document).ready(function(){
	...
	if ( typeof $.table_hotkeys != 'undefined' ) {
		...
		toggle_all = function() {
			$('#cb-select-all-1').data( 'gp-toggle', 1 ).trigger( 'click' ).removeData( 'gp-toggle' );
		};
