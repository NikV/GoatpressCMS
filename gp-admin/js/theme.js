/* global _gpThemeSettings, confirm */
window.gp = window.gp || {};
...
// Setup app structure
_.extend( themes, { model: {}, view: {}, routes: {}, router: {}, template: gp.template });
...
themes.view.Appearance = gp.Backbone.View.extend({

	el: '#gpbody-content .wrap .theme-browser',
	...
	searchContainer: $( '#gpbody h2:first' ),
	...
	search: function() {
		...
		this.searchContainer
			.append( $.parseHTML( '<label class="screen-reader-text" for="gp-filter-search-input">' + l10n.search + '</label>' ) )
			.append( view.el );
	},
...
themes.Collection = Backbone.Collection.extend({
	...
	// Send request to api.Goatpress.org/themes
	apiCall: function( request, paginated ) {
		return gp.ajax.send( 'query-themes', {
			data: {
			// Request data
				request: _.extend({
					per_page: 100,
...
themes.view.Theme = gp.Backbone.View.extend({
...
themes.view.Details = gp.Backbone.View.extend({
	...
	containFocus: function( $el ) {
		...
		$el.on( 'keydown.gp-themes', function( event ) {
...
themes.view.Preview = themes.view.Details.extend({
	...
	className: 'gp-full-overlay expanded',
...
themes.view.Themes = gp.Backbone.View.extend({
...
themes.view.Search = gp.Backbone.View.extend({

	tagName: 'input',
	className: 'gp-filter-search',
	id: 'gp-filter-search-input',
	searching: false,
...
themes.Router = Backbone.Router.extend({
	...
	search: function( query ) {
		$( '.gp-filter-search' ).val( query );
	},

	themes: function() {
		$( '.gp-filter-search' ).val( '' );
	},
...
themes.Run = {
	...
	routes: function() {
		...
		themes.router.on( 'route:search', function() {
			$( '.gp-filter-search' ).trigger( 'keyup' );
		});
...
themes.view.InstallerSearch =  themes.view.Search.extend({
	...
	doSearch: _.debounce( function( value ) {
		...
		// Get the themes by sending Ajax POST request to api.Goatpress.org/themes
...
themes.view.Installer = themes.view.Appearance.extend({
	...
	// in new location
	searchContainer: $( '.gp-filter .search-form' ),
...
themes.RunInstaller = {
	...
	routes: function() {
		...
		// Bind to our global `gp.themes` object
		...
		themes.router.on( 'route:search', function() {
			$( '.gp-filter-search' ).focus().trigger( 'keyup' );
		});
...
$( document ).ready(function() {
	...
	$( '.broken-themes .delete-theme' ).on( 'click', function() {
		return confirm( _gpThemeSettings.settings.confirmDelete );
	});
});
...
jQuery(document).ready( function($) {
	tb_position = function() {
		...
		if ( $('#gpadminbar').length ) {
			adminbar_height = parseInt( $('#gpadminbar').css('height'), 10 );
		}
