...
(function($){
	...
	function check_pass_strength() {
		...
		strength = gp.passwordStrength.meter( pass1, gp.passwordStrength.userInputBlacklist(), pass2 );
	...
	$(document).ready( function() {
		...
		$colorpicker.on( 'click.colorpicker', '.color-option', function() {
			...
			if ( user_id === current_user_id ) {
				...
				// repaint icons
				if ( typeof gp !== 'undefined' && gp.svgPainter ) {
					try {
						colors = $.parseJSON( $this.children( '.icon_colors' ).val() );
					} catch ( error ) {}

					if ( colors ) {
						gp.svgPainter.setColors( colors );
						gp.svgPainter.paint();
					}
				}
