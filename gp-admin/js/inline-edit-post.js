...
$( document ).on( 'heartbeat-tick.gp-check-locked-posts', function( e, data ) {
	var locked = data['gp-check-locked-posts'] || {};

	$('#the-list tr').each( function(i, el) {
		var key = el.id, row = $(el), lock_data, avatar;

		if ( locked.hasOwnProperty( key ) ) {
			if ( ! row.hasClass('gp-locked') ) {
				...
				row.addClass('gp-locked');
			}
		} else if ( row.hasClass('gp-locked') ) {
			// Make room for the CSS animation
			row.removeClass('gp-locked').delay(1000).find('.locked-info span').empty();
		}
	});
}).on( 'heartbeat-send.gp-check-locked-posts', function( e, data ) {
	var check = [];
	...
	if ( check.length ) {
		data['gp-check-locked-posts'] = check;
	}
}).ready( function() {
	// Set the heartbeat interval to 15 sec.
	if ( typeof gp !== 'undefined' && gp.heartbeat ) {
