...
window.switchEditors = {
	...
	go: function( id, mode ) {
		...
		if ( 'tmce' === mode || 'tinymce' === mode ) {
			...
			if ( tinyMCEPreInit.mceInit[ id ] && tinyMCEPreInit.mceInit[ id ].gpautop ) {
				txtarea_el.value = t.gpautop( txtarea_el.value );
			}
			...
			if ( ed ) {
				...
				// The TinyMCE instance doesn't exist, run the content through 'pre_gpautop()' and show the textarea
				if ( tinyMCEPreInit.mceInit[ id ] && tinyMCEPreInit.mceInit[ id ].gpautop ) {
					txtarea_el.value = t.pre_gpautop( txtarea_el.value );
				}
	...
	_gp_Nop: function( content ) {
		...
		if ( content.indexOf( '[caption' ) !== -1 ) {
			...
			content = content.replace( /\[caption[\s\S]+?\[\/caption\]/g, function( a ) {
				return a.replace( /<br([^>]*)>/g, '<gp-temp-br$1>' ).replace( /[\r\n\t]+/, '' );
			});
		}
		...
		if ( preserve_linebreaks ) {
			content = content.replace( /<gp-line-break>/g, '\n' );
		}

		// and the <br> tags in captions
		if ( preserve_br ) {
			content = content.replace( /<gp-temp-br([^>]*)>/g, '<br$1>' );
		}

		return content;
	},

	_gp_Autop: function(pee) {
		...
		// put back the line breaks in pre|script
		if ( preserve_linebreaks ) {
			pee = pee.replace( /<gp-line-break>/g, '\n' );
		}

		if ( preserve_br ) {
			pee = pee.replace( /<gp-temp-br([^>]*)>/g, '<br$1>' );
		}

		return pee;
	},

	pre_gpautop: function( content ) {
		var t = this, o = { o: t, data: content, unfiltered: content },
			q = typeof( jQuery ) !== 'undefined';

		if ( q ) {
			jQuery( 'body' ).trigger( 'beforePregpautop', [ o ] );
		}

		o.data = t._gp_Nop( o.data );

		if ( q ) {
			jQuery('body').trigger('afterPregpautop', [ o ] );
		}

		return o.data;
	},

	gpautop: function( pee ) {
		var t = this, o = { o: t, data: pee, unfiltered: pee },
			q = typeof( jQuery ) !== 'undefined';

		if ( q ) {
			jQuery( 'body' ).trigger('beforegpautop', [ o ] );
		}

		o.data = t._gp_Autop( o.data );

		if ( q ) {
			jQuery( 'body' ).trigger('aftergpautop', [ o ] );
		}
