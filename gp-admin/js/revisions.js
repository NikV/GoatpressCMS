...
(function($) {
	var revisions;
	/**
	 * Expose the module in window.gp.revisions.
	 */
	revisions = gp.revisions = { model: {}, view: {}, controller: {} };

	// Link post revisions data served from the back-end.
	revisions.settings = window._gpRevisionsSettings || {};
	...
	revisions.model.Diffs = Backbone.Collection.extend({
		...
		load: function( comparisons ) {
			gp.revisions.log( 'load', comparisons );
			...
			return this.fetch({ data: { compare: comparisons }, remove: false }).done( function() {
				gp.revisions.log( 'load:complete', comparisons );
			});
		},
		...
		sync: function( method, model, options ) {
			if ( 'read' === method ) {
				...
				var deferred = gp.ajax.send( options ),
	...
	/**
	 * gp.revisions.model.FrameState
	 *
	 * The frame state.
	 *
	 * @see gp.revisions.view.Frame
	 *
	 ...
	 */
	revisions.view.Frame = gp.Backbone.View.extend({
		className: 'revisions',
		template: gp.template('revisions-frame'),
		...
		render: function() {
			gp.Backbone.View.prototype.render.apply( this, arguments );

			$('html').css( 'overflow-y', 'scroll' );
			$('#gpbody-content .wrap').append( this.el );
	...
	revisions.view.Controls = gp.Backbone.View.extend({
		...
		ready: function() {
			...
			this.window = $(window);
			this.window.on( 'scroll.gp.revisions', {controls: this}, function(e) {
				...
				if ( scrolled >= controls.top ) {
					if ( ! frame.$el.hasClass('pinned') ) {
						...
						controls.window.on('resize.gp.revisions.pinning click.gp.revisions.pinning', {controls: controls}, function(e) {
							e.data.controls.setWidth();
						});
					}
					frame.$el.addClass('pinned');
				} else if ( frame.$el.hasClass('pinned') ) {
					controls.window.off('.gp.revisions.pinning');
	...
	// The tickmarks view
	revisions.view.Tickmarks = gp.Backbone.View.extend({
	...
	// The metabox view
	revisions.view.Metabox = gp.Backbone.View.extend({
	...
	revisions.view.Meta = gp.Backbone.View.extend({
		template: gp.template('revisions-meta'),
	...
	// The checkbox view.
	revisions.view.Checkbox = gp.Backbone.View.extend({
		className: 'revisions-checkbox',
		template: gp.template('revisions-checkbox'),
	...
	revisions.view.Tooltip = gp.Backbone.View.extend({
	...
	revisions.view.Buttons = gp.Backbone.View.extend({
		className: 'revisions-buttons',
		template: gp.template('revisions-buttons'),
	...
	// The slider view.
	revisions.view.Slider = gp.Backbone.View.extend({
		className: 'gp-slider',
		direction: isRtl ? 'right' : 'left',
		...
		start: function( event, ui ) {
			...
			$( window ).on( 'mousemove.gp.revisions', { view: this }, function( e ) {
				var handles,
					view              = e.data.view,
		...
		stop: function() {
			$( window ).off('mousemove.gp.revisions');
	...
	revisions.view.Diff = gp.Backbone.View.extend({
		className: 'revisions-diff',
		template:  gp.template('revisions-diff'),
	...
	revisions.init = function() {
		...
		state = new revisions.model.FrameState({
			initialDiffState: {
				// gp_localize_script doesn't stringifies ints, so cast them.
				to: parseInt( revisions.settings.to, 10 ),
				from: parseInt( revisions.settings.from, 10 ),
				// gp_localize_script does not allow for top-level booleans so do a comparator here.
