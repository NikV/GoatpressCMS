( function( window, $, undefined ) {
	...
	$( function() {
		var $wrap = $( '#postdivrich' ),
			$contentWrap = $( '#gp-content-wrap' ),
			$tools = $( '#gp-content-editor-tools' ),
			$visualTop = $(),
			$visualEditor = $(),
			$textTop = $( '#ed_toolbar' ),
			$textEditor = $( '#content' ),
			...
			$postBody = $('#post-body'),
			fullscreen = window.gp.editor && window.gp.editor.fullscreen,
			mceEditor,
			mceBind = function(){},
			mceUnbind = function(){},
			fixedTop = false,
		...
		$document.on( 'tinymce-editor-init.editor-expand', function( event, editor ) {
			...
			// Set the minimum height to the initial viegport height.
			...
			function mceGetCursorOffset() {
				...
				if ( editor.gp && editor.gp.getView && ( view = editor.gp.getView( node ) ) ) {
			...
			// but also to scroll the window when the cursor moves out of the viegport to a gpview.
			// Setting a buffer > 0 will prevent the browser default.
			// Some browsers will scroll to the middle,
			// others to the top/bottom of the *window* when moving the cursor out of the viegport.
			...
			function mceShow() {
				...
				setTimeout( function() {
					editor.execCommand( 'gpAutoResize' );
					adjust();
				}, 300 );
			}
			...
			mceBind = function() {
				...
				editor.on( 'gp-toolbar-toggle', toggleAdvanced );
				// Adjust when the editor resizes.
				editor.on( 'setcontent gp-autoresize gp-toolbar-toggle', adjust );
			...
			mceUnbind = function() {
				...
				editor.off( 'gp-toolbar-toggle', toggleAdvanced );
				editor.off( 'setcontent gp-autoresize gp-toolbar-toggle', adjust );
			...
			if ( $wrap.hasClass( 'gp-editor-expand' ) ) {
				// Adjust "immediately"
				mceBind();
		...
		function adjust( event ) {
			...
			if ( ! canPin ) {
				...
				if ( ( ! fixedTop || resize ) &&
					...
					( windogpos >= ( topPos - heights.toolsHeight - heights.adminBarHeight ) &&
					// Handle scrolling up.
					windogpos <= ( topPos - heights.toolsHeight - heights.adminBarHeight + editorHeight - buffer ) ) ) {
					fixedTop = true;
				...
				// Maybe unpin the top.
					...
					// Handle scrolling up.
					if ( windogpos <= ( topPos - heights.toolsHeight - heights.adminBarHeight ) ) {
						fixedTop = false;
					...
					// Handle scrolling down.
					} else if ( windogpos >= ( topPos - heights.toolsHeight - heights.adminBarHeight + editorHeight - buffer ) ) {
						fixedTop = false;
				...
				if ( ( ! fixedBottom || ( resize && advanced ) ) &&
						// +[n] for the border around the .gp-editor-container.
						( windogpos + heights.windowHeight ) <= ( editorPos + editorHeight + heights.bottomHeight + heights.statusBarHeight + borderWidth ) ) {
			...
			if ( $postboxContainer.width() < 300 && heights.windowWidth > 600 && // sidebar position is changed with @media from CSS, make sure it is on the side
				...
				heights.windowHeight < editorHeight ) { // the editor is taller than the viegport
				...
				if ( ( heights.sideSortablesHeight + pinnedToolsTop + sidebarBottom ) > heights.windowHeight || fixedSideTop || fixedSideBottom ) {
					...
					if ( windogpos + pinnedToolsTop <= postBodyTop ) {
						...
						if ( windogpos > lastScrollPosition ) {
							...
							} else if ( ! fixedSideBottom && heights.sideSortablesHeight + $sideSortables.offset().top + sidebarBottom < windogpos + heights.windowHeight ) {
								// pin the bottom
							...
							} else if ( ! fixedSideTop && $sideSortables.offset().top >= windogpos + pinnedToolsTop ) {
								// pin the top
								fixedSideTop = true;
					...
					// if the sidebar container is smaller than the viegport, then pin/unpin the top when scrolling
					if ( windogpos >= ( postBodyTop - pinnedToolsTop ) ) {
		...
		function on() {
			...
			$wrap.addClass( 'gp-editor-expand' );
			...
			$document.on( 'gp-collapse-menu.editor-expand postboxes-columnchange.editor-expand editor-classchange.editor-expand', adjust )
				.on( 'postbox-toggled.editor-expand', function() {
					...
					adjust();
				}).on( 'gp-window-resized.editor-expand', function() {
					if ( mceEditor && ! mceEditor.isHidden() ) {
						mceEditor.execCommand( 'gpAutoResize' );
					} else {
						textEditorResize();
					}
				});
			...
			if ( mceEditor ) {
				mceEditor.settings.gp_autoresize_on = true;
				mceEditor.execCommand( 'gpAutoResizeOn' );
		...
		function off() {
			...
			$wrap.removeClass( 'gp-editor-expand' );
			...
			if ( mceEditor ) {
				mceEditor.settings.gp_autoresize_on = false;
				mceEditor.execCommand( 'gpAutoResizeOff' );
	...
	$( function() {
		...
		if ( $( '#postdivrich' ).hasClass( 'gp-editor-expand' ) ) {
			$content.on( 'keydown.focus-shortcut', toggleViaKeyboard );
		}
		...
		$document.on( 'tinymce-editor-setup.focus', function( event, editor ) {
			...
			editor.addCommand( 'gpToggleDFW', toggle );
			editor.addShortcut( 'alt+shift+w', '', 'gpToggleDFW' );
		} );
		...
		$document.on( 'tinymce-editor-init.focus', function( event, editor ) {
			...
			if ( editor.id === 'content' ) {
				...
				mceBind = function() {
					...
					editor.on( 'blur', blur );
					editor.on( 'gp-autoresize', recalcEditorRect );
				};
				...
				mceUnbind = function() {
					...
					editor.off( 'gp-autoresize', recalcEditorRect );
				};
