/* global _gpCustomizeWidgetsSettings */
(function( gp, $ ){
	...
	// Link settings
	api.Widgets.data = _gpCustomizeWidgetsSettings || {};
	l10n = api.Widgets.data.l10n;
	delete api.Widgets.data.l10n;

	/**
	 * gp.customize.Widgets.WidgetModel
	 *
	...
	/**
	 * gp.customize.Widgets.WidgetCollection
	 *
	...
	/**
	 * gp.customize.Widgets.SidebarModel
	 *
	...
	/**
	 * gp.customize.Widgets.SidebarCollection
	 *
	...
	/**
	 * gp.customize.Widgets.AvailableWidgetsPanelView
	 *
	 ...
	 */
	api.Widgets.AvailableWidgetsPanelView = gp.Backbone.View.extend({
	...
	/**
	 * gp.customize.Widgets.WidgetControl
	 *
	 * Customizer control for widgets.
	 * Note that 'widget_form' must match the gp_Widget_Form_Customize_Control::$type
	 *
	 * @constructor
	...
	api.Widgets.WidgetControl = api.Control.extend({
		...
		_setupWideWidget: function() {
			...
			$customizeSidebar = $( '.gp-full-overlay-sidebar-content:first' );
		...
		_setupReorderUI: function() {
			...
			$reorderNav.find( '.move-widget, .move-widget-down, .move-widget-up' ).each( function() {
				...
				if ( $( this ).is( '.move-widget' ) ) {
					...
					if ( isMoveUp ) {
						self.moveUp();
						gp.a11y.speak( l10n.widgetMovedUp );
					} else {
						self.moveDown();
						gp.a11y.speak( l10n.widgetMovedDown );
					}
		...
		_setupRemoveUI: function() {
			...
			replaceDeleteWithRemove = function() {
				$removeBtn.text( l10n.removeBtnLabel ); // gp_widget_control() outputs the link as "Delete"
		...
		/**
		 * @return {gp.customize.controlConstructor.sidebar_widgets[]}
		 */
		...
		updateWidget: function( args ) {
			...
			jqxhr = $.post( gp.ajax.settings.url, data );
	...
	/**
	 * gp.customize.Widgets.SidebarSection
	 *
	...
	/**
	 * gp.customize.Widgets.SidebarControl
	 *
	 * Customizer control for widgets.
	 * Note that 'sidebar_widgets' must match the gp_Widget_Area_Customize_Control::$type
	 *
	 * @since 3.9.0
	 *
	 * @constructor
	...
	api.Widgets.SidebarControl = api.Control.extend({
				...
				_( removedWidgetIds ).each( function( removedWidgetId ) {
					...
					setTimeout( function() {
						...
						api.each( function( otherSetting ) {
							if ( otherSetting.id === self.setting.id || 0 !== otherSetting.id.indexOf( 'sidebars_widgets[' ) || otherSetting.id === 'sidebars_widgets[gp_inactive_widgets]' ) {
								return;
							}
						...
						if ( api.Widgets.savedWidgetIds[removedWidgetId] ) {
							inactiveWidgets = api.value( 'sidebars_widgets[gp_inactive_widgets]' )().slice();
							inactiveWidgets.push( removedWidgetId );
							api.value( 'sidebars_widgets[gp_inactive_widgets]' )( _( inactiveWidgets ).unique() );
						}
		 ...
		 *
		 * @since 3.9
		 * @return {gp.customize.controlConstructor.widget_form[]}
		 */
	...
	api.Widgets.getSidebarWidgetControlContainingWidget = function( widgetId ) {
		...
		// @todo this can use widgetIdToSettingId(), then pass into gp.customize.control( x ).getSidebarWidgetsControl()
