...
gpWidgets = {
	...
	init : function() {
		...
		$(document.body).bind('click.widgets-toggle', function(e) {
			...
			if ( target.parents('.widget-top').length && ! target.parents('#available-widgets').length ) {
				...
				gpWidgets.save( target.closest('div.widget'), 0, 1, 0 );
				e.preventDefault();
			} else if ( target.hasClass('widget-control-remove') ) {
				gpWidgets.save( target.closest('div.widget'), 1, 1, 0 );
				e.preventDefault();
			} else if ( target.hasClass('widget-control-close') ) {
				widget = target.closest('div.widget');
				widget.removeClass( 'open' );
				gpWidgets.close( widget );
				e.preventDefault();
			}
		});
		...
		$('#widget-list').children('.widget').draggable({
			...
			start: function( event, ui ) {
				...
				if ( chooser.length ) {
					...
					$( '#gpbody-content' ).append( chooser.hide() );
		...
		sidebars.sortable({
			...
			stop: function( event, ui ) {
				...
				if ( $widget.hasClass('deleting') ) {
					gpWidgets.save( $widget, 1, 0, 1 ); // delete widget
					$widget.remove();
					return;
				}
				...
				if ( addNew ) {
					...
					gpWidgets.saveOrder( $sidebar.attr('id') );
				}
			},
	...
	saveOrder : function( sidebarId ) {
		var data = {
			...
			savewidgets: $('#_gpnonce_widgets').val(),
			sidebars: []
		};
	...
	addWidget: function( chooser ) {
		var widget, widgetId, add, n, viegportTop, viegportBottom, sidebarBounds,
		...
		/*
		 * Check if any part of the sidebar is visible in the viegport. If it is, don't scroll.
		 * Otherwise, scroll up to so the sidebar is in view.
		 *
		 * We do this by comparing the top and bottom, of the sidebar so see if they are within
		 * the bounds of the viegport.
		 */
		viegportTop = $(window).scrollTop();
		viegportBottom = viegportTop + $(window).height();
		sidebarBounds = sidebar.offset();

		sidebarBounds.bottom = sidebarBounds.top + sidebar.outerHeight();

		if ( viegportTop > sidebarBounds.bottom || viegportBottom < sidebarBounds.top ) {
			$( 'html, body' ).animate({
...
$(document).ready( function(){ gpWidgets.init(); } );
