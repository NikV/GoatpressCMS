/* global postL10n, ajaxurl, gpAjax, setPostThumbnailL10n, postboxes, pagenow, tinymce, alert, deleteUserSetting */
/* global theList:true, theExtraList:true, getUserSetting, setUserSetting */

var commentsBox, gpSetThumbnailHTML, gpSetThumbnailID, gpRemoveThumbnail, gptitlehint, makeSlugeditClickable, editPermalink;
	...
	get : function(total, num) {
		...
		$.post(ajaxurl, data,
			function(r) {
				...
				$('#the-comment-list').append('<tr><td colspan="2">'+gpAjax.broken+'</td></tr>');
			}
		);
...
gpSetThumbnailHTML = function(html){
	$('.inside', '#postimagediv').html(html);
};

gpSetThumbnailID = function(id){
...
gpRemoveThumbnail = function(nonce){
	$.post(ajaxurl, {
...
(function($) {
	...
	$(document).on( 'heartbeat-send.gp-refresh-nonces', function( e, data ) {
		var nonce, post_id;

		if ( check ) {
			if ( ( post_id = $('#post_ID').val() ) && ( nonce = $('#_gpnonce').val() ) ) {
				data['gp-refresh-post-nonces'] = {
					post_id: post_id,
					post_nonce: nonce
				};
			}
		}
	}).on( 'heartbeat-tick.gp-refresh-nonces', function( e, data ) {
		var nonces = data['gp-refresh-post-nonces'];
...
jQuery(document).ready( function($) {
	...
	$('#post-lock-dialog .notification-dialog').on( 'keydown', function(e) {
		...
		if ( target.hasClass('gp-tab-first') && e.shiftKey ) {
			$(this).find('.gp-tab-last').focus();
			e.preventDefault();
		} else if ( target.hasClass('gp-tab-last') && ! e.shiftKey ) {
			$(this).find('.gp-tab-first').focus();
			e.preventDefault();
		}
	}).filter(':visible').find('.gp-tab-first').focus();

	// Set the heartbeat interval to 15 sec. if post lock dialogs are enabled
	if ( gp.heartbeat && $('#post-lock-dialog').length ) {
		gp.heartbeat.interval( 15 );
	}
	...
	$('#post-preview').on( 'click.post-preview', function( event ) {
		var $this = $(this),
			$form = $('form#post'),
			$previewField = $('input#gp-preview'),
			target = $this.attr('target') || 'gp-preview',
		...
		if ( gp.autosave ) {
			gp.autosave.server.tempBlockSave();
		}
	...
	$document.on( 'autosave-disable-buttons.edit-post', function() {
		...
		if ( ! gp.heartbeat || ! gp.heartbeat.hasConnectionError() ) {
	...
	$(window).on( 'beforeunload.edit-post', function() {
		...
		if ( ( editor && ! editor.isHidden() && editor.isDirty() ) ||
			( gp.autosave && gp.autosave.server.postChanged() ) ) {
		...
		$.ajax({
			...
			async: false,
			data: {
				action: 'gp-remove-post-lock',
				_gpnonce: $('#_gpnonce').val(),
				post_ID: $('#post_ID').val(),
	...
	$('.categorydiv').each( function(){
		...
		// TODO: move to jQuery 1.3+, support for multiple hierarchical taxonomies, see gp-lists.js
		...
		$('#' + taxonomy + '-add-toggle').click( function() {
			$('#' + taxonomy + '-adder').toggleClass( 'gp-hidden-children' );
	...
	if ( $('#postcustom').length ) {
		$( '#the-list' ).gpList( { addAfter: function() {
	...
	// word count
	if ( typeof(gpWordCount) != 'undefined' ) {
		$document.triggerHandler('gpcountwords', [ co.val() ]);
	...
	( function() {
		...
		function endDrag() {
			...
			if ( mce ) {
				editor.focus();
				toolbarHeight = parseInt( $( '#gp-content-editor-container .mce-toolbar-grp' ).height(), 10 );
		...
		$handle.on( 'mousedown.gp-editor-resize', function( event ) {
			...
			$document.on( 'mousemove.gp-editor-resize', dragging )
				.on( 'mouseup.gp-editor-resize mouseleave.gp-editor-resize', endDrag );

			event.preventDefault();
		}).on( 'mouseup.gp-editor-resize', endDrag );
	})();
