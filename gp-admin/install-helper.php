 ...
 * plugin installation. This file is not included by Goatpress and it is
 ...
 *
 *     check_column( 'gp_links', 'link_description', 'mediumtext' );
 *     if ( check_column( $gpdb->comments, 'comment_author', 'tinytext' ) ) {
 *         echo "ok\n";
 *     }
 *
 *     $error_count = 0;
 *     $tablename = $gpdb->links;
 *     // Check the column.
 *     if ( ! check_column($gpdb->links, 'link_description', 'varchar( 255 )' ) ) {
 *         $ddl = "ALTER TABLE $gpdb->links MODIFY COLUMN link_description varchar(255) NOT NULL DEFAULT '' ";
 *         $q = $gpdb->query( $ddl );
 *     }
 *
 *     if ( check_column( $gpdb->links, 'link_description', 'varchar( 255 )' ) ) {
...
function maybe_create_table($table_name, $create_ddl) {
	global $gpdb;
	foreach ($gpdb->get_col("SHOW TABLES",0) as $table ) {
		if ($table == $table_name) {
			return true;
		}
	}
...
function check_column($table_name, $col_name, $col_type, $is_null = null, $key = null, $default = null, $extra = null) {
	global $gpdb;
	$diffs = 0;
	$results = $gpdb->get_results("DESC $table_name");
