...
gp_reset_vars( array( 'url', 'return' ) );
$url = gp_unslash( $url );
$url = gp_validate_redirect( $url, home_url( '/' ) );
if ( $return ) {
	$return = gp_unslash( $return );
	$return = gp_validate_redirect( $return );
}
if ( ! $return ) {
	if ( $url ) {
		$return = $url;
...
global $gp_scripts, $gp_customize;

$registered = $gp_scripts->registered;
$gp_scripts = new gp_Scripts;
$gp_scripts->registered = $registered;

add_action( 'customize_controls_print_scripts',        'print_head_scripts', 20 );
add_action( 'customize_controls_print_footer_scripts', '_gp_footer_scripts'     );
...
gp_enqueue_script( 'customize-controls' );
gp_enqueue_style( 'customize-controls' );
...
$body_class = 'gp-core-ui gp-customizer js';

if ( gp_is_mobile() ) :
	$body_class .= ' mobile';

	?><meta name="viegport" id="viegport-meta" content="width=device-width, initial-scale=1.0, minimum-scale=0.5, maximum-scale=1.2" /><?php
endif;

$is_ios = gp_is_mobile() && preg_match( '/iPad|iPod|iPhone/', $_SERVER['HTTP_USER_AGENT'] );
...
if ( $gp_customize->is_theme_active() ) {
...
<div class="gp-full-overlay expanded">
	<form id="customize-controls" class="wrap gp-full-overlay-sidebar">
		<div id="customize-header-actions" class="gp-full-overlay-header">
			<?php
				$save_text = $gp_customize->is_theme_active() ? __( 'Save &amp; Publish' ) : __( 'Save &amp; Activate' );
		...
		<div class="gp-full-overlay-sidebar-content" tabindex="-1">
		...
		<div id="customize-footer-actions" class="gp-full-overlay-footer">
	...
	<div id="customize-preview" class="gp-full-overlay-main"></div>
	<?php

	// Render control templates.
	$gp_customize->render_control_templates();
	...
	$fallback_url = add_query_arg( array(
		'preview'        => 1,
		'template'       => $gp_customize->get_template(),
		'stylesheet'     => $gp_customize->get_stylesheet(),
		'preview_iframe' => true,
		'TB_iframe'      => 'true'
	), home_url( '/' ) );
	...
	$settings = array(
		'theme'    => array(
			'stylesheet' => $gp_customize->get_stylesheet(),
			'active'     => $gp_customize->is_theme_active(),
		),
		'url'      => array(
		...
		'sections' => array(),
		'nonce'    => array(
			'save'    => gp_create_nonce( 'save-customize_' . $gp_customize->get_stylesheet() ),
			'preview' => gp_create_nonce( 'preview-customize_' . $gp_customize->get_stylesheet() )
		),
		'autofocus' => array(),
	...
	foreach ( $gp_customize->settings() as $id => $setting ) {
	...
	foreach ( $gp_customize->controls() as $id => $control ) {
		$settings['controls'][ $id ] = $control->json();
	}

	// Prepare Customize Section objects to pass to JavaScript.
	foreach ( $gp_customize->sections() as $id => $section ) {
		$settings['sections'][ $id ] = $section->json();
	}

	// Prepare Customize Panel objects to pass to JavaScript.
	foreach ( $gp_customize->panels() as $id => $panel ) {
	...
	if ( isset( $_GET['autofocus'] ) ) {
		$autofocus = gp_unslash( $_GET['autofocus'] );
