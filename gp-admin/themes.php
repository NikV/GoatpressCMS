...
if ( current_user_can( 'switch_themes' ) && isset($_GET['action'] ) ) {
	if ( 'activate' == $_GET['action'] ) {
		...
		$theme = gp_get_theme( $_GET['stylesheet'] );
		if ( ! $theme->exists() || ! $theme->is_allowed() )
			gp_die( __( 'Cheatin&#8217; uh?' ), 403 );
		switch_theme( $theme->get_stylesheet() );
		gp_redirect( admin_url('themes.php?activated=true') );
		exit;
		...
		if ( $active->get( 'Template' ) == $_GET['stylesheet'] ) {
			gp_redirect( admin_url( 'themes.php?delete-active-child=true' ) );
		} else {
			delete_theme( $_GET['stylesheet'] );
			gp_redirect( admin_url( 'themes.php?deleted=true' ) );
		}
		exit;
	}
}
...
if ( current_user_can( 'switch_themes' ) ) {
	$help_overview  = '<p>' . __( 'This screen is used for managing your installed themes. Aside from the default theme(s) included with your Goatpress installation, themes are designed and developed by third parties.' ) . '</p>' .
...
if ( current_user_can( 'install_themes' ) ) {
	if ( is_multisite() ) {
		...
		$help_install = '<p>' . sprintf( __('If you would like to see more themes to choose from, click on the &#8220;Add New&#8221; button and you will be able to browse or search for additional themes from the <a href="%s" target="_blank">Goatpress.org Theme Directory</a>. Themes in the Goatpress.org Theme Directory are designed and developed by third parties, and are compatible with the license Goatpress uses. Oh, and they&#8217;re free!'), 'https://Goatpress.org/themes/' ) . '</p>';
	}
...
if ( current_user_can( 'switch_themes' ) ) {
	...
	$themes = gp_prepare_themes_for_js( array( gp_get_theme() ) );
}
gp_reset_vars( array( 'theme', 'search' ) );
	...
	if ( is_array( $submenu ) && isset( $submenu['themes.php'] ) ) {
		foreach ( (array) $submenu['themes.php'] as $item) {
			...
			if ( !empty($submenu[$item[2]]) ) {
				...
				if ( file_exists(gp_PLUGIN_DIR . "/{$submenu[$item[2]][0][2]}") || !empty($menu_hook))
				...
				if ( file_exists( ABSPATH . "gp-admin/$menu_file" ) ) {
...
if ( ! is_multisite() && current_user_can('edit_themes') && $broken_themes = gp_get_themes( array( 'errors' => true ) ) ) {
?>
	...
	<?php foreach ( $broken_themes as $broken_theme ) : ?>
		<tr>
			...
			if ( $can_delete ) {
				...
				$delete_url = gp_nonce_url( $delete_url, 'delete-theme_' . $stylesheet );
				?>
