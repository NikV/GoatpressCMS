...
%%s' ), get_bloginfo( 'name' ), home_url(), gp_specialchars_decode( translate_user_role( $role['name'] ) ) );
	}
	add_filter( 'gpmu_signup_user_notification_email', 'admin_created_user_email' );
}

if ( isset($_REQUEST['action']) && 'adduser' == $_REQUEST['action'] ) {
	check_admin_referer( 'add-user', '_gpnonce_add-user' );

	$user_details = null;
	$user_email = gp_unslash( $_REQUEST['email'] );
	if ( false !== strpos( $user_email, '@' ) ) {
		...
		if ( is_super_admin() ) {
			...
			gp_redirect( add_query_arg( array('update' => 'enter_email'), 'user-new.php' ) );
			die();
		}
	}

	if ( !$user_details ) {
		gp_redirect( add_query_arg( array('update' => 'does_not_exist'), 'user-new.php' ) );
		die();
	}
...
%4$s' );
			gp_mail( $new_user_email, sprintf( __( '[%s] Joining confirmation' ), gp_specialchars_decode( get_option( 'blogname' ) ) ), sprintf( $message, get_option( 'blogname' ), home_url(), gp_specialchars_decode( translate_user_role( $role['name'] ) ), home_url( "/newbloguser/$newuser_key/" ) ) );
	...
	check_admin_referer( 'create-user', '_gpnonce_create-user' );
	...
	if ( ! is_multisite() ) {
		...
		$new_user_email = gp_unslash( $_REQUEST['email'] );
		$user_details = gpmu_validate_user_signup( $_REQUEST['user_login'], $new_user_email );
		if ( is_gp_error( $user_details[ 'errors' ] ) && !empty( $user_details[ 'errors' ]->errors ) ) {
			 ...
			 */
			$new_user_login = apply_filters( 'pre_user_login', sanitize_user( gp_unslash( $_REQUEST['user_login'] ), true ) );
			if ( isset( $_POST[ 'noconfirmation' ] ) && is_super_admin() ) {
				add_filter( 'gpmu_signup_user_notification', '__return_false' ); // Disable confirmation email
				add_filter( 'gpmu_welcome_user_notification', '__return_false' ); // Disable welcome email
			}
			gpmu_signup_user( $new_user_login, $new_user_email, array( 'add_to_blog' => $gpdb->blogid, 'new_role' => $_REQUEST['role'] ) );
			if ( isset( $_POST[ 'noconfirmation' ] ) && is_super_admin() ) {
				$key = $gpdb->get_var( $gpdb->prepare( "SELECT activation_key FROM {$gpdb->signups} WHERE user_login = %s AND user_email = %s", $new_user_login, $new_user_email ) );
				gpmu_activate_signup( $key );
				$redirect = add_query_arg( array('update' => 'addnoconfirmation'), 'user-new.php' );
			} else {
...
get_current_screen()->set_help_sidebar(
    ...
    '<p>' . __('<a href="http://codex.Goatpress.org/Users_Add_New_Screen" target="_blank">Documentation on Adding New Users</a>') . '</p>' .
    '<p>' . __('<a href="https://Goatpress.org/support/" target="_blank">Support Forums</a>') . '</p>'
);

gp_enqueue_script('gp-ajax-response');
...
if ( is_multisite() && current_user_can( 'promote_users' ) && ! gp_is_large_network( 'users' )
	...
	gp_enqueue_script( 'user-suggest' );
}
...
<?php if ( isset($errors) && is_gp_error( $errors ) ) : ?>
	<div class="error">
		<ul>
		<?php
...
<?php if ( isset($add_user_errors) && is_gp_error( $add_user_errors ) ) : ?>
	<div class="error">
		<?php
...
<?php gp_nonce_field( 'add-user', '_gpnonce_add-user' ) ?>

<table class="form-table">
	<tr class="form-field form-required">
		<th scope="row"><label for="adduser-email"><?php echo $label; ?></label></th>
		<td><input name="email" type="<?php echo $type; ?>" id="adduser-email" class="gp-suggest-user" value="" /></td>
	</tr>
	<tr class="form-field">
		<th scope="row"><label for="adduser-role"><?php _e('Role'); ?></label></th>
		<td><select name="role" id="adduser-role">
			<?php gp_dropdown_roles( get_option('default_role') ); ?>
			</select>
		</td>
	</tr>
...
<form method="post" name="createuser" id="createuser" class="validate" novalidate="novalidate"<?php
	/** This action is documented in gp-admin/user-new.php */
	do_action( 'user_new_form_tag' );
?>>
<input name="action" type="hidden" value="createuser" />
<?php gp_nonce_field( 'create-user', '_gpnonce_create-user' ); ?>
<?php
// Load up the passed data, else set to a default.
$creating = isset( $_POST['createuser'] );

$new_user_login = $creating && isset( $_POST['user_login'] ) ? gp_unslash( $_POST['user_login'] ) : '';
$new_user_firstname = $creating && isset( $_POST['first_name'] ) ? gp_unslash( $_POST['first_name'] ) : '';
$new_user_lastname = $creating && isset( $_POST['last_name'] ) ? gp_unslash( $_POST['last_name'] ) : '';
$new_user_email = $creating && isset( $_POST['email'] ) ? gp_unslash( $_POST['email'] ) : '';
$new_user_uri = $creating && isset( $_POST['url'] ) ? gp_unslash( $_POST['url'] ) : '';
$new_user_role = $creating && isset( $_POST['role'] ) ? gp_unslash( $_POST['role'] ) : '';
$new_user_send_password = $creating && isset( $_POST['send_password'] ) ? gp_unslash( $_POST['send_password'] ) : '';
$new_user_ignore_pass = $creating && isset( $_POST['noconfirmation'] ) ? gp_unslash( $_POST['noconfirmation'] ) : '';
...
<?php } // !is_multisite ?>
	<tr class="form-field">
		...
		<td><select name="role" id="role">
			...
			gp_dropdown_roles($new_user_role);
			?>
			</select>
		</td>
	</tr>
