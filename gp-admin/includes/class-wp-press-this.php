...
class gp_Press_This {
	...
	public function site_settings() {
		return array(
			...
			// Needs to be set here and in get_shortcut_link() in gp-includes/link-template.php.
			'version' => '8',
	...
	public function side_load_images( $post_id, $content = '' ) {
		...
		if ( preg_match_all( '/<img [^>]+>/', $content, $matches ) && current_user_can( 'upload_files' ) ) {
			foreach ( (array) $matches[0] as $image ) {
				...
				// Don't try to sideload a file without a file extension, leads to gp upload error.
	...
	public function save_post() {
		if ( empty( $_POST['post_ID'] ) || ! $post_id = (int) $_POST['post_ID'] ) {
			gp_send_json_error( array( 'errorMessage' => __( 'Missing post ID.' ) ) );
		}

		if ( empty( $_POST['_gpnonce'] ) || ! gp_verify_nonce( $_POST['_gpnonce'], 'update-post_' . $post_id ) ||
			! current_user_can( 'edit_post', $post_id ) ) {

			gp_send_json_error( array( 'errorMessage' => __( 'Invalid post.' ) ) );
		}
		...
		$updated = gp_update_post( $post, true );

		if ( is_gp_error( $updated ) ) {
			gp_send_json_error( array( 'errorMessage' => $updated->get_error_message() ) );
		} else {
			...
			if ( 'publish' === get_post_status( $post_id ) ) {
				...
				/** This filter is documented in gp-admin/includes/class-gp-press-this.php */
				$redirect = apply_filters( 'press_this_save_redirect', get_edit_post_link( $post_id, 'raw' ), $post_id, $post['post_status'] );
			}

			gp_send_json_success( array( 'redirect' => $redirect ) );
		}
	}
	...
	public function add_category() {
		if ( false === gp_verify_nonce( $_POST['new_cat_nonce'], 'add-category' ) ) {
			gp_send_json_error();
		}
		...
		if ( empty( $added ) ) {
			gp_send_json_error( array( 'errorMessage' => __( 'This category cannot be added. Please change the name and try again.' ) ) );
		}

		foreach ( $added as $new_cat_id ) {
			$new_cat = get_category( $new_cat_id );

			if ( is_gp_error( $new_cat ) ) {
				gp_send_json_error( array( 'errorMessage' => __( 'Error while adding the category. Please try again later.' ) ) );
			}
	...
	public function fetch_source_html( $url ) {
		...
		if ( ! is_gp_error( $source_tmp_file ) && file_exists( $source_tmp_file ) ) {
			// Get the content of the source page from the tmp file..

			$source_content = gp_kses(
		...
		} else if ( is_gp_error( $source_tmp_file ) ) {
			$source_content = new gp_Error( 'upload-error',  sprintf( __( 'Error: %s' ), sprintf( __( 'Could not download the source URL (native error: %s).' ), $source_tmp_file->get_error_message() ) ) );
		} else if ( ! file_exists( $source_tmp_file ) ) {
			$source_content = new gp_Error( 'no-local-file',  sprintf( __( 'Error: %s' ), __( 'Could not save or locate the temporary download file for the source URL.' ) ) );
		}
	...
	private function _limit_img( $src ) {
		...
		if ( preg_match( '/\/ad[sx]{1}?\//', $src ) ) {
			...
			return '';
		} else if ( preg_match( '/\/gp-includes\//', $src ) ) {
			// Classic gp interface images
			return '';
	...
	public function merge_or_fetch_data() {
		...
		foreach ( array( 'u', 's', 't', 'v' ) as $key ) {
			if ( ! empty( $_POST[ $key ] ) ) {
				$value = gp_unslash( $_POST[ $key ] );
			} else if ( ! empty( $_GET[ $key ] ) ) {
				$value = gp_unslash( $_GET[ $key ] );
			} else {
				continue;
			}
			...
			if ( empty( $_POST ) && ! empty( $data['u'] ) ) {
				...
				foreach ( array( '_images', '_embeds' ) as $type ) {
					...
					foreach ( $items as $key => $value ) {
						if ( $type === '_images' ) {
							$value = $this->_limit_img( gp_unslash( $value ) );
						} else {
							$value = $this->_limit_embed( gp_unslash( $value ) );
						}
				...
				foreach ( array( '_meta', '_links' ) as $type ) {
					...
					foreach ( $items as $key => $value ) {
						...
						if ( $type === '_meta' ) {
							$value = $this->_limit_string( gp_unslash( $value ) );
							...
							if ( in_array( $key, array( 'canonical', 'shortlink', 'icon' ), true ) ) {
								$data[ $type ][ $key ] = $this->_limit_url( gp_unslash( $value ) );
							}
						}
					}
				}
	...
	public function categories_html( $post ) {
		...
		<div aria-label="<?php esc_attr_e( 'Categories' ); ?>">
			<ul class="categories-select">
				<?php gp_terms_checklist( $post->ID, array( 'taxonomy' => 'category', 'list_only' => true ) ); ?>
			</ul>
		</div>
		<?php
	}
	...
	public function tags_html( $post ) {
		...
		if ( ! $esc_tags || is_gp_error( $esc_tags ) ) {
			$esc_tags = '';
		}
...
<head>
	...
	<script>
		window.gpPressThisData   = <?php echo gp_json_encode( $site_data ); ?>;
		window.gpPressThisConfig = <?php echo gp_json_encode( $site_settings ); ?>;
	</script>
	...
	<?php
		...
		gp_enqueue_media( array( 'post' => $post_ID ) );
		gp_enqueue_style( 'press-this' );
		gp_enqueue_script( 'press-this' );
		gp_enqueue_script( 'json2' );
...
<?php
	...
	$admin_body_class .= ' branch-' . str_replace( array( '.', ',' ), '-', floatval( $gp_version ) );
	$admin_body_class .= ' version-' . str_replace( '.', '-', preg_replace( '/^([.0-9]+).*/', '$1', $gp_version ) );
...
?>
<body class="gp-admin gp-core-ui <?php echo $admin_body_classes . ' ' . $admin_body_class; ?>">
	<div id="adminbar" class="adminbar">
		<h1 id="current-site" class="current-site">
			<a class="current-site-link" href="<?php echo esc_url( home_url( '/' ) ); ?>" target="_blank" rel="home">
				<span class="dashicons dashicons-Goatpress"></span>
	...
	<form id="pressthis-form" method="post" action="post.php" autocomplete="off">
		...
		gp_nonce_field( 'update-post_' . $post_ID, '_gpnonce', false );
		gp_nonce_field( 'add-category', '_ajax_nonce-add-category', false );
	...
	<div class="wrapper">
		<div class="editor-wrapper">
			...
			<div id="app-container" class="editor">
				...
				<?php
				gp_editor( $post_content, 'pressthis', array(
					...
					'tinymce'          => array(
						'resize'                => false,
						'Goatpress_adv_hidden'  => false,
						'add_unload_trigger'    => false,
						'statusbar'             => false,
						'autoresize_min_height' => 600,
						'gp_autoresize_on'      => true,
						'plugins'               => 'lists,media,paste,tabfocus,fullscreen,Goatpress,gpautoresize,gpeditimage,gpgallery,gplink,gpview',
...
$GLOBALS['gp_press_this'] = new gp_Press_This;
