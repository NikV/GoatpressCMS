 ...
 * @access private
 */
class gp_MS_Themes_List_Table extends gp_List_Table {
	...
	public function prepare_items() {
		...
		$themes = array(
			/**
			 * Filter the full array of gp_Theme objects to list in the Multisite
			 * themes list table.
			 *
			 * @since 3.1.0
			 *
			 * @param array $all An array of gp_Theme objects to display in the list table.
			 */
			'all' => apply_filters( 'all_themes', gp_get_themes() ),
			'search' => array(),
			'enabled' => array(),
			'disabled' => array(),
			'upgrade' => array(),
			'broken' => $this->is_site_themes ? array() : gp_get_themes( array( 'errors' => true ) ),
		);
		...
		gp_Theme::sort_by_name( $this->items );
	...
	public function single_row( $theme ) {
		...
		if ( ! $allowed ) {
			if ( ! $theme->errors() )
				$actions['enable'] = '<a href="' . esc_url( gp_nonce_url($url . 'action=enable&amp;theme=' . $theme_key . '&amp;paged=' . $page . '&amp;s=' . $s, 'enable-theme_' . $stylesheet ) ) . '" title="' . esc_attr__('Enable this theme') . '" class="edit">' . ( $this->is_site_themes ? __( 'Enable' ) : __( 'Network Enable' ) ) . '</a>';
		} else {
			$actions['disable'] = '<a href="' . esc_url( gp_nonce_url($url . 'action=disable&amp;theme=' . $theme_key . '&amp;paged=' . $page . '&amp;s=' . $s, 'disable-theme_' . $stylesheet ) ) . '" title="' . esc_attr__('Disable this theme') . '">' . ( $this->is_site_themes ? __( 'Disable' ) : __( 'Network Disable' ) ) . '</a>';
		}
		...
		if ( ! $allowed && current_user_can( 'delete_themes' ) && ! $this->is_site_themes && $stylesheet != get_option( 'stylesheet' ) && $stylesheet != get_option( 'template' ) )
			$actions['delete'] = '<a href="' . esc_url( gp_nonce_url( 'themes.php?action=delete-selected&amp;checked[]=' . $theme_key . '&amp;theme_status=' . $context . '&amp;paged=' . $page . '&amp;s=' . $s, 'bulk-themes' ) ) . '" title="' . esc_attr__( 'Delete this theme' ) . '" class="delete">' . __( 'Delete' ) . '</a>';
		 ...
		 * @param gp_Theme $theme   The current gp_Theme object.
		...
		foreach ( $columns as $column_name => $column_display_name ) {
			...
			switch ( $column_name ) {
				...
				default:
					 ...
					 * @param string   $stylesheet  Directory name of the theme.
					 * @param gp_Theme $theme       Current gp_Theme object.
					 */
		...
		if ( $this->is_site_themes )
			remove_action( "after_theme_row_$stylesheet", 'gp_theme_update_row' );
