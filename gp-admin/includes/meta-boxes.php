...
?>
<a class="preview button" href="<?php echo $preview_link; ?>" target="gp-preview-<?php echo (int) $post->ID; ?>" id="post-preview"><?php echo $preview_button; ?></a>
<input type="hidden" name="gp-preview" id="gp-preview" value="" />
</div>
	...
	<div id="taxonomy-<?php echo $tax_name; ?>" class="categorydiv">
		...
		<div id="<?php echo $tax_name; ?>-pop" class="tabs-panel" style="display: none;">
			<ul id="<?php echo $tax_name; ?>checklist-pop" class="categorychecklist form-no-clear" >
				<?php $popular_ids = gp_popular_terms_checklist( $tax_name ); ?>
			</ul>
		</div>
		...
		<div id="<?php echo $tax_name; ?>-all" class="tabs-panel">
            ...
            ?>
			<ul id="<?php echo $tax_name; ?>checklist" data-gp-lists="list:<?php echo $tax_name; ?>" class="categorychecklist form-no-clear">
				<?php gp_terms_checklist( $post->ID, array( 'taxonomy' => $tax_name, 'popular_cats' => $popular_ids ) ); ?>
			</ul>
		</div>
	<?php if ( current_user_can( $taxonomy->cap->edit_terms ) ) : ?>
			<div id="<?php echo $tax_name; ?>-adder" class="gp-hidden-children">
				<h4>
				...
				<p id="<?php echo $tax_name; ?>-add" class="category-add gp-hidden-child">
					<label class="screen-reader-text" for="new<?php echo $tax_name; ?>"><?php echo $taxonomy->labels->add_new_item; ?></label>
					<input type="text" name="new<?php echo $tax_name; ?>" id="new<?php echo $tax_name; ?>" class="form-required form-input-tip" value="<?php echo esc_attr( $taxonomy->labels->new_item_name ); ?>" aria-required="true"/>
					<label class="screen-reader-text" for="new<?php echo $tax_name; ?>_parent">
						<?php echo $taxonomy->labels->parent_item_colon; ?>
					</label>
					<?php gp_dropdown_categories( array( 'taxonomy' => $tax_name, 'hide_empty' => 0, 'name' => 'new' . $tax_name . '_parent', 'orderby' => 'name', 'hierarchical' => 1, 'show_option_none' => '&mdash; ' . $taxonomy->labels->parent_item . ' &mdash;' ) ); ?>
					<input type="button" id="<?php echo $tax_name; ?>-add-submit" data-gp-lists="add:<?php echo $tax_name; ?>checklist:<?php echo $tax_name; ?>-add" class="button category-add-submit" value="<?php echo esc_attr( $taxonomy->labels->add_new_item ); ?>" />
					<?php gp_nonce_field( 'add-' . $tax_name, '_ajax_nonce-add-' . $tax_name, false ); ?>
...
<p><?php _e('Excerpts are optional hand-crafted summaries of your content that can be used in your theme. <a href="http://codex.Goatpress.org/Excerpt" target="_blank">Learn more about manual excerpts.</a>'); ?></p>
<?php
}
...
<p><?php _e('Trackbacks are a way to notify legacy blog systems that you&#8217;ve linked to them. If you link other Goatpress sites, they&#8217;ll be notified automatically using <a href="http://codex.Goatpress.org/Introduction_to_Blogging#Managing_Comments" target="_blank">pingbacks</a>, no other action necessary.'); ?></p>
<?php
if ( ! empty($pings) )
	echo $pings;
}
...
<p><?php _e('Custom fields can be used to add extra metadata to a post that you can <a href="http://codex.Goatpress.org/Using_Custom_Fields" target="_blank">use in your theme</a>.'); ?></p>
<?php
}
...
<p class="meta-options">
	...
	<label for="ping_status" class="selectit"><input name="ping_status" type="checkbox" id="ping_status" value="open" <?php checked($post->ping_status, 'open'); ?> /> <?php printf( __( 'Allow <a href="%s" target="_blank">trackbacks and pingbacks</a> on this page.' ), __( 'http://codex.Goatpress.org/Introduction_to_Blogging#Managing_Comments' ) ); ?></label>
	<?php
	/**
	 ...
	 * @since 3.1.0
	 *
	 * @param gp_Post $post gp_Post object of the current post.
	 */
...
function post_comment_meta_box( $post ) {
	gp_nonce_field( 'get-comments', 'add_comment_nonce', false );
	?>
...
function page_attributes_meta_box($post) {
	...
	if ( $post_type_object->hierarchical ) {
		 ...
		 * @since 3.3.0
		 *
		 * @see gp_dropdown_pages()
		 *
		 * @param array   $dropdown_args Array of arguments used to generate the pages drop-down.
		 * @param gp_Post $post          The current gp_Post object.
		 */
		$dropdown_args = apply_filters( 'page_attributes_dropdown_pages_args', $dropdown_args, $post );
		$pages = gp_dropdown_pages( $dropdown_args );
		if ( ! empty($pages) ) {
?>
...
<div id="major-publishing-actions">
<?php
/** This action is documented in gp-admin/includes/meta-boxes.php */
do_action( 'post_submitbox_start' );
?>
<div id="delete-action">
<?php
if ( !empty($_GET['action']) && 'edit' == $_GET['action'] && current_user_can('manage_links') ) { ?>
	<a class="submitdelete deletion" href="<?php echo gp_nonce_url("link.php?action=delete&amp;link_id=$link->link_id", 'delete-bookmark_' . $link->link_id); ?>" onclick="if ( confirm('<?php echo esc_js(sprintf(__("You are about to delete this link '%s'\n 'Cancel' to stop, 'OK' to delete."), $link->link_name )); ?>') ) {return true;}return false;"><?php _e('Delete'); ?></a>
<?php } ?>
</div>
...
<div id="taxonomy-linkcategory" class="categorydiv">
	...
	<div id="categories-all" class="tabs-panel">
		<ul id="categorychecklist" data-gp-lists="list:category" class="categorychecklist form-no-clear">
			<?php
			if ( isset($link->link_id) )
				gp_link_category_checklist($link->link_id);
			else
				gp_link_category_checklist();
			?>
		</ul>
	</div>
	...
	<div id="categories-pop" class="tabs-panel" style="display: none;">
		<ul id="categorychecklist-pop" class="categorychecklist form-no-clear">
			<?php gp_popular_terms_checklist('link_category'); ?>
		</ul>
	</div>

	<div id="category-adder" class="gp-hidden-children">
		<h4><a id="category-add-toggle" href="#category-add"><?php _e( '+ Add New Category' ); ?></a></h4>
		<p id="link-category-add" class="gp-hidden-child">
			<label class="screen-reader-text" for="newcat"><?php _e( '+ Add New Category' ); ?></label>
			<input type="text" name="newcat" id="newcat" class="form-required form-input-tip" value="<?php esc_attr_e( 'New category name' ); ?>" aria-required="true" />
			<input type="button" id="link-category-add-submit" data-gp-lists="add:categorychecklist:link-category-add" class="button" value="<?php esc_attr_e( 'Add' ); ?>" />
			<?php gp_nonce_field( 'add-link-category', '_ajax_nonce', false ); ?>
...
function post_thumbnail_meta_box( $post ) {
	...
	echo _gp_post_thumbnail_html( $thumbnail_id, $post->ID );
}
