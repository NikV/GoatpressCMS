 ...
 * @access private
 */
class gp_Media_List_Table extends gp_List_Table {
	...
	public function prepare_items() {
		global $gp_query, $post_mime_types, $avail_post_mime_types, $mode;

		list( $post_mime_types, $avail_post_mime_types ) = gp_edit_attachments_query( $_REQUEST );
		...
		$this->set_pagination_args( array(
			'total_items' => $gp_query->found_posts,
			'total_pages' => $gp_query->max_num_pages,
			'per_page' => $gp_query->query_vars['posts_per_page'],
		) );
	}

	protected function get_views() {
		global $gpdb, $post_mime_types, $avail_post_mime_types;

		$type_links = array();
		$_num_posts = (array) gp_count_attachments();
		$_total_posts = array_sum($_num_posts) - $_num_posts['trash'];
		$total_orphans = $gpdb->get_var( "SELECT COUNT( * ) FROM $gpdb->posts WHERE post_type = 'attachment' AND post_status != 'trash' AND post_parent < 1" );
		...
		foreach ( $post_mime_types as $mime_type => $label ) {
			...
			$selected = '';
			if ( !empty( $_GET['attachment-filter'] ) && strpos( $_GET['attachment-filter'], 'post_mime_type:' ) === 0 && gp_match_mime_types( $mime_type, str_replace( 'post_mime_type:', '', $_GET['attachment-filter'] ) ) )
...
<?php
		if ( ! is_singular() ) {
			...
			/** This action is documented in gp-admin/includes/class-gp-posts-list-table.php */
...
<?php
		...
		/** This filter is documented in gp-admin/inclues/class-gp-list-table.php */
...
?>
		<td <?php echo $attributes ?>><?php
			if ( $thumb = gp_get_attachment_image( $post->ID, array( 80, 60 ), true ) ) {
...
?>
		<td <?php echo $attributes ?>><strong>
			...
			<p class="filename"><?php echo gp_basename( $post->guid ); ?></p>
<?php
			...
			<td <?php echo $attributes ?>><strong>
				...
				if ( $user_can_edit ):
					$detach_url = add_query_arg( array(
						...
						'media[]' => $post->ID,
						'_gpnonce' => gp_create_nonce( 'bulk-' . $this->_args['plural'] )
					), 'upload.php' ); ?>
...
<?php endwhile;
	...
	private function _get_row_actions( $post, $att_title ) {
		...
		if ( $this->detached ) {
			...
			if ( current_user_can( 'delete_post', $post->ID ) )
				if ( EMPTY_TRASH_DAYS && MEDIA_TRASH ) {
					$actions['trash'] = "<a class='submitdelete' href='" . gp_nonce_url( "post.php?action=trash&amp;post=$post->ID", 'trash-post_' . $post->ID ) . "'>" . __( 'Trash' ) . "</a>";
				} else {
					$delete_ays = !MEDIA_TRASH ? " onclick='return showNotice.warn();'" : '';
					$actions['delete'] = "<a class='submitdelete'$delete_ays href='" . gp_nonce_url( "post.php?action=delete&amp;post=$post->ID", 'delete-post_' . $post->ID ) . "'>" . __( 'Delete Permanently' ) . "</a>";
				}
			...
			if ( current_user_can( 'delete_post', $post->ID ) ) {
				if ( $this->is_trash )
					$actions['untrash'] = "<a class='submitdelete' href='" . gp_nonce_url( "post.php?action=untrash&amp;post=$post->ID", 'untrash-post_' . $post->ID ) . "'>" . __( 'Restore' ) . "</a>";
		 ...
		 * @param gp_Post $post     gp_Post object for the current attachment.
		 * @param bool    $detached Whether the list table contains media not attached
		 *                          to any posts. Default true.
		 */
