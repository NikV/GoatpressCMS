...
function check_upload_size( $file ) {
	...
	if ( $file['error'] != '0' && ! isset( $_POST['html-upload'] ) && ( ! defined( 'DOING_AJAX' ) || ! DOING_AJAX ) ) {
		gp_die( $file['error'] . ' <a href="javascript:history.go(-1)">' . __( 'Back' ) . '</a>' );
	}

	return $file;
}
add_filter( 'gp_handle_upload_prefilter', 'check_upload_size' );
 ...
 */
function gpmu_delete_blog( $blog_id, $drop = false ) {
	global $gpdb;
	...
	// If ms_files_rewriting is enabled and upload_path is empty, gp_upload_dir is not reliable.
	...
	if ( $drop ) {
		 ...
		 */
		$drop_tables = apply_filters( 'gpmu_drop_tables', $tables, $blog_id );

		foreach ( (array) $drop_tables as $table ) {
			$gpdb->query( "DROP TABLE IF EXISTS `$table`" );
		}

		$gpdb->delete( $gpdb->blogs, array( 'blog_id' => $blog_id ) );
		 ...
		 * @since MU
		 *
		 * @param string $uploads['basedir'] Uploads path without subdirectory. @see gp_upload_dir()
		 * @param int    $blog_id            The blog ID.
		 */
		$dir = apply_filters( 'gpmu_delete_blog_upload_dir', $uploads['basedir'], $blog_id );
 ...
 * @since 3.0.0
 *
 * @todo Merge with gp_delete_user() ?
 *
 * @param int $id The user ID.
 * @return bool True if the user was deleted, otherwise false.
 */
function gpmu_delete_user( $id ) {
	global $gpdb;
	...
	/** This action is documented in gp-admin/includes/user.php */
...
###SITEURL###' );
	...
	gp_mail( $value, sprintf( __( '[%s] New Admin Email Address' ), gp_specialchars_decode( get_option( 'blogname' ) ) ), $content );
}
 ...
 * @since 3.0.0
 *
 * @global object $errors gp_Error object.
 * @global object $gpdb   Goatpress database object.
 */
function send_confirmation_on_profile_email() {
	...
	if ( $current_user->user_email != $_POST['email'] ) {
		...
		if ( $gpdb->get_var( $gpdb->prepare( "SELECT user_email FROM {$gpdb->users} WHERE user_email=%s", $_POST['email'] ) ) ) {
...
###SITEURL###' );
		...
		gp_mail( $_POST['email'], sprintf( __( '[%s] New Email Address' ), gp_specialchars_decode( get_option( 'blogname' ) ) ), $content );
...
add_action( 'gpmueditblogaction', 'upload_space_setting' );
 ...
 *
 * @param int    $id         The user ID.
 * @param string $pref       The column in the gp_users table to update the user's status
...
function update_user_status( $id, $pref, $value, $deprecated = null ) {
	...
	$gpdb->update( $gpdb->users, array( sanitize_key( $pref ) => $value ), array( 'ID' => $id ) );
...
function _access_denied_splash() {
	...
	if ( gp_list_filter( $blogs, array( 'userblog_id' => get_current_blog_id() ) ) )
		return;

	$blog_name = get_bloginfo( 'name' );

	if ( empty( $blogs ) )
		gp_die( sprintf( __( 'You attempted to access the "%1$s" dashboard, but you do not currently have privileges on this site. If you believe you should be able to access the "%1$s" dashboard, please contact your network administrator.' ), $blog_name ), 403 );
...
function mu_dropdown_languages( $lang_files = array(), $current = '' ) {
	...
	if ( $flag === false ) // Goatpress english
 ...
 * @since 3.0.0
 *
 * @global int $gp_db_version The version number of the database.
 */
function site_admin_notice() {
	global $gp_db_version;
	if ( !is_super_admin() )
		return false;
	if ( get_site_option( 'gpmu_upgrade_site' ) != $gp_db_version )
...
add_filter( 'gp_insert_post_data', 'avoid_blog_page_permalink_collision', 10, 2 );
	...
	<?php if ( in_array( get_site_option( 'registration' ), array( 'all', 'blog' ) ) ) : ?>
		<tr>
			<th scope="row" colspan="2" class="th-full">
				...
				$sign_up_url = apply_filters( 'gp_signup_location', network_site_url( 'gp-signup.php' ) );
				?>
