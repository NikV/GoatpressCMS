 ...
 * @access private
 */
class gp_Posts_List_Table extends gp_List_Table {
	...
	public function __construct( $args = array() ) {
		...
		if ( !current_user_can( $post_type_object->cap->edit_others_posts ) ) {
			...
			$this->user_posts_count = $gpdb->get_var( $gpdb->prepare( "
				SELECT COUNT( 1 ) FROM $gpdb->posts
		...
		if ( 'post' == $post_type && $sticky_posts = get_option( 'sticky_posts' ) ) {
			...
			$this->sticky_posts_count = $gpdb->get_var( $gpdb->prepare( "SELECT COUNT( 1 ) FROM $gpdb->posts WHERE post_type = %s AND post_status NOT IN ('trash', 'auto-draft') AND ID IN ($sticky_posts)", $post_type ) );
		}
	}
	...
	public function prepare_items() {
		global $avail_post_stati, $gp_query, $per_page, $mode;

		$avail_post_stati = gp_edit_posts_query();

		$this->set_hierarchical_display( is_post_type_hierarchical( $this->screen->post_type ) && 'menu_order title' == $gp_query->query['orderby'] );

		$total_items = $this->hierarchical_display ? $gp_query->post_count : $gp_query->found_posts;

		$post_type = $this->screen->post_type;
		$per_page = $this->get_items_per_page( 'edit_' . $post_type . '_per_page' );

		/** This filter is documented in gp-admin/includes/post.php */
	...
	protected function get_views() {
		...
		$status_links = array();
		$num_posts = gp_count_posts( $post_type, 'readable' );
		$class = '';
		$allposts = '';
...
<?php
		if ( 'top' == $which && !is_singular() ) {
			...
			if ( is_object_in_taxonomy( $this->screen->post_type, 'category' ) ) {
				...
				gp_dropdown_categories( $dropdown_options );
			}
...
<?php
	...
	public function get_columns() {
		...
		$taxonomies = gp_filter_object_list( $taxonomies, array( 'show_admin_column' => true ), 'and', 'name' );
	...
	/**
	 * @global gp_Query $gp_query
	 * @global int $per_page
	 * @param array $posts
	 * @param int $level
	 */
	public function display_rows( $posts = array(), $level = 0 ) {
		global $gp_query, $per_page;

		if ( empty( $posts ) )
			$posts = $gp_query->posts;
		...
		if ( empty( $_REQUEST['s'] ) ) {
			...
			foreach ( $pages as $page ) {
				...
				if ( $page->post_parent == $page->ID ) {
					$page->post_parent = 0;
					$gpdb->update( $gpdb->posts, array( 'post_parent' => 0 ), array( 'ID' => $page->ID ) );
	...
	public function single_row( $post, $level = 0 ) {
		...
		$lock_holder = gp_check_post_lock( $post->ID );
		if ( $lock_holder ) {
			$classes .= ' gp-locked';
			...
			case 'title':
				...
				if ( current_user_can( 'delete_post', $post->ID ) ) {
					if ( 'trash' == $post->post_status )
						$actions['untrash'] = "<a title='" . esc_attr__( 'Restore this item from the Trash' ) . "' href='" . gp_nonce_url( admin_url( sprintf( $post_type_object->_edit_link . '&amp;action=untrash', $post->ID ) ), 'untrash-post_' . $post->ID ) . "'>" . __( 'Restore' ) . "</a>";
	...
	<?php endif; // $bulk
		...
		if ( post_type_supports( $screen->post_type, 'author' ) ) :
			...
			if ( is_super_admin() || current_user_can( $post_type_object->cap->edit_others_posts ) ) :
				...
				if ( $authors = gp_dropdown_users( $users_opt ) ) :
	...
	<?php foreach ( $hierarchical_taxonomies as $taxonomy ) : ?>
			...
			<ul class="cat-checklist <?php echo esc_attr( $taxonomy->name )?>-checklist">
				<?php gp_terms_checklist( null, array( 'taxonomy' => $taxonomy->name ) ) ?>
			</ul>
	...
	<?php
		...
		gp_dropdown_pages( $dropdown_args );
	?>
			</label>
	...
	<?php
		...
		foreach ( $columns as $column_name => $column_display_name ) {
			...
			if ( $bulk ) {
				 ...
				 * @param gp_Post $post_type   The post type slug.
				 */
	...
	?>
		<p class="submit inline-edit-save">
			...
			<?php if ( ! $bulk ) {
				gp_nonce_field( 'inlineeditnonce', '_inline_edit', false );
				?>
