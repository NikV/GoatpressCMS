<?php
/**
 * Goatpress Theme Install Administration API
 *
 * @package Goatpress
...
function install_themes_feature_list() {
	...
	if ( !$cache = get_transient( 'gporg_theme_feature_list' ) )
		set_transient( 'gporg_theme_feature_list', array(), 3 * HOUR_IN_SECONDS );
	...
	set_transient( 'gporg_theme_feature_list', $feature_list, 3 * HOUR_IN_SECONDS );
...
<form method="post" enctype="multipart/form-data" class="gp-upload-form" action="<?php echo self_admin_url('update.php?action=upload-theme'); ?>">
	<?php gp_nonce_field( 'theme-upload'); ?>
...
function display_theme( $theme ) {
	...
	if ( ! isset( $gp_list_table ) ) {
		$gp_list_table = _get_list_table('gp_Theme_Install_List_Table');
	}
	$gp_list_table->prepare_items();
	$gp_list_table->single_row( $theme );
}
...
function install_theme_information() {
	...
	$theme = themes_api( 'theme_information', array( 'slug' => gp_unslash( $_REQUEST['theme'] ) ) );

	if ( is_gp_error( $theme ) )
		gp_die( $theme );
	...
	$gp_list_table->theme_installer_single( $theme );
	iframe_footer();
	exit;
}
