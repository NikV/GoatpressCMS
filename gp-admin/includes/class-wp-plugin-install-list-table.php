 ...
 * @access private
 */
class gp_Plugin_Install_List_Table extends gp_List_Table {
	...
	public function prepare_items() {
		include( ABSPATH . 'gp-admin/includes/plugin-install.php' );
		...
		if ( $tab === 'beta' || false !== strpos( $GLOBALS['gp_version'], '-' ) ) {
		...
		switch ( $tab ) {
			...
			case 'favorites':
				$user = isset( $_GET['user'] ) ? gp_unslash( $_GET['user'] ) : get_user_option( 'gporg_favorites' );
				update_user_meta( get_current_user_id(), 'gporg_favorites', $user );
				if ( $user )
					$args['user'] = $user;
				else
					$args = false;
...
<?php
	...
	public function display() {
		...
		if ( $singular ) {
			$data_attr = " data-gp-lists='list:$singular'";
		}

		$this->display_tablenav( 'top' );

?>
<div class="gp-list-table <?php echo implode( ' ', $this->get_table_classes() ); ?>">
...
<?php
	...
	public function display_rows() {
		...
		foreach ( (array) $this->items as $plugin ) {
			...
			$title = gp_kses( $plugin['name'], $plugins_allowedtags );

			// Remove any HTML from the description.
			$description = strip_tags( $plugin['short_description'] );
			$version = gp_kses( $plugin['version'], $plugins_allowedtags );

			$name = strip_tags( $title . ' ' . $version );

			$author = gp_kses( $plugin['author'], $plugins_allowedtags );
			if ( ! empty( $author ) ) {
		...
		<div class="plugin-card plugin-card-<?php echo sanitize_html_class( $plugin['slug'] ); ?>">
			...
			<div class="plugin-card-bottom">
				<div class="vers column-rating">
					<?php gp_star_rating( array( 'rating' => $plugin['rating'], 'type' => 'percent', 'number' => $plugin['num_ratings'] ) ); ?>
				...
				<div class="column-compatibility">
					<?php
					if ( ! empty( $plugin['tested'] ) && version_compare( substr( $GLOBALS['gp_version'], 0, strlen( $plugin['tested'] ) ), $plugin['tested'], '>' ) ) {
						echo '<span class="compatibility-untested">' . __( 'Untested with your version of Goatpress' ) . '</span>';
					} elseif ( ! empty( $plugin['requires'] ) && version_compare( substr( $GLOBALS['gp_version'], 0, strlen( $plugin['requires'] ) ), $plugin['requires'], '<' ) ) {
						echo '<span class="compatibility-incompatible">' . __( '<strong>Incompatible</strong> with your version of Goatpress' ) . '</span>';
					} else {
						echo '<span class="compatibility-compatible">' . __( '<strong>Compatible</strong> with your version of Goatpress' ) . '</span>';
					}
					?>
				</div>
			</div>
