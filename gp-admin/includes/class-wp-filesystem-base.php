...
/**
 * Base Goatpress Filesystem class for which Filesystem implementations extend
 *
 * @since 2.5.0
 */
class gp_Filesystem_Base {
	...
	public function abspath() {
		...
		// Perhaps the FTP folder is rooted at the Goatpress install, Check for gp-includes folder in root, Could have some false positives, but rare.
		if ( ! $folder && $this->is_dir( '/' . gpINC ) )
			$folder = '/';
		return $folder;
	}

	/**
	 * Return the path on the remote filesystem of gp_CONTENT_DIR.
	 *
	 * @access public
	 * @since 2.7.0
	 *
	 * @return string The location of the remote path.
	 */
	public function gp_content_dir() {
		return $this->find_folder(gp_CONTENT_DIR);
	}

	/**
	 * Return the path on the remote filesystem of gp_PLUGIN_DIR.
	 *
	 * @access public
	 * @since 2.7.0
	 *
	 * @return string The location of the remote path.
	 */
	public function gp_plugins_dir() {
		return $this->find_folder(gp_PLUGIN_DIR);
	}
	 ...
	 * @return string The location of the remote path.
	 */
	public function gp_themes_dir( $theme = false ) {
	...
	/**
	 * Return the path on the remote filesystem of gp_LANG_DIR.
	 *
	 * @access public
	 * @since 3.2.0
	 *
	 * @return string The location of the remote path.
	 */
	public function gp_lang_dir() {
		return $this->find_folder(gp_LANG_DIR);
	}
	 ...
	 * @access public
	 * @since 2.5.0
	 * @deprecated 2.7.0 use gp_Filesystem::abspath() or gp_Filesystem::gp_*_dir() instead.
	 * @see gp_Filesystem::abspath()
	 * @see gp_Filesystem::gp_content_dir()
	 * @see gp_Filesystem::gp_plugins_dir()
	 * @see gp_Filesystem::gp_themes_dir()
	 * @see gp_Filesystem::gp_lang_dir()
	 *
	...
	public function find_base_dir( $base = '.', $echo = false ) {
		_deprecated_function(__FUNCTION__, '2.7', 'gp_Filesystem::abspath() or gp_Filesystem::gp_*_dir()' );
		$this->verbose = $echo;
		return $this->abspath();
	}
	 ...
	 * @access public
	 * @since 2.5.0
	 * @deprecated 2.7.0 use gp_Filesystem::abspath() or gp_Filesystem::gp_*_dir() methods instead.
	 * @see gp_Filesystem::abspath()
	 * @see gp_Filesystem::gp_content_dir()
	 * @see gp_Filesystem::gp_plugins_dir()
	 * @see gp_Filesystem::gp_themes_dir()
	...
	public function find_folder( $folder ) {
		...
		if ( stripos($this->method, 'ftp') !== false ) {
			$constant_overrides = array(
				'FTP_BASE' => ABSPATH,
				'FTP_CONTENT_DIR' => gp_CONTENT_DIR,
				'FTP_PLUGIN_DIR' => gp_PLUGIN_DIR,
				'FTP_LANG_DIR' => gp_LANG_DIR
			);
	...
	public function search_for_folder( $folder, $base = '.', $loop = false ) {
		...
		foreach ( $folder_parts as $index => $key ) {
			...
			/*
			 * Working from /home/ to /user/ to /Goatpress/ see if that file exists within
			 * the current folder, If it's found, change into it and follow through looking
			 * for it. If it cant find Goatpress down that route, it'll continue onto the next
		...
		// This comes into effect when the CWD is /home/user/ but gp is at /var/www/....
	 ...
	 * @since 2.5.0
	 * @abstract
	 * @return bool True on success or false on failure (always true for gp_Filesystem_Direct).
	 */
