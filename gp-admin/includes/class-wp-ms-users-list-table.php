 ...
 * @access private
 */
class gp_MS_Users_List_Table extends gp_List_Table {
	...
	public function prepare_items() {
		global $usersearch, $role, $gpdb, $mode;
		...
		if ( gp_is_large_network( 'users' ) )
			$args['search'] = ltrim( $args['search'], '*' );

		if ( $role == 'super' ) {
			$logins = implode( "', '", get_super_admins() );
			$args['include'] = $gpdb->get_col( "SELECT ID FROM $gpdb->users WHERE user_login IN ('$logins')" );
		}
		 ...
		 */
		if ( !$usersearch && gp_is_large_network( 'users' ) ) {
	...
	public function get_columns() {
		 ...
		 */
		$users_columns = apply_filters( 'gpmu_users_columns', $users_columns );
			...
			foreach ( $columns as $column_name => $column_display_name ) :
				...
				switch ( $column_name ) {
					...
					case 'username':
						...
						$edit_link = esc_url( add_query_arg( 'gp_http_referer', urlencode( gp_unslash( $_SERVER['REQUEST_URI'] ) ), get_edit_user_link( $user->ID ) ) );
						...
						echo "<td $attributes>"; ?>
							...
							<?php
								...
								if ( current_user_can( 'delete_user', $user->ID ) && ! in_array( $user->user_login, $super_admins ) ) {
									$actions['delete'] = '<a href="' . $delete = esc_url( network_admin_url( add_query_arg( '_gp_http_referer', urlencode( gp_unslash( $_SERVER['REQUEST_URI'] ) ), gp_nonce_url( 'users.php', 'deleteuser' ) . '&amp;action=deleteuser&amp;id=' . $user->ID ) ) ) . '" class="delete">' . __( 'Delete' ) . '</a>';
								}
					...
					default:
						echo "<td $attributes>";
						/** This filter is documented in gp-admin/includes/class-gp-users-list-table.php */
