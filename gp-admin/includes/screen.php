 ...
 * @since 2.7.0
 *
 * @param string|gp_Screen $screen The screen you want the headers for
 ...
 * @since 2.7.0
 *
 * @param string|gp_Screen $screen The screen you want the hidden columns for
 * @return array
 */
...
function meta_box_prefs( $screen ) {
	...
	if ( empty($gp_meta_boxes[$screen->id]) )
		return;

	$hidden = get_hidden_meta_boxes($screen);

	foreach ( array_keys($gp_meta_boxes[$screen->id]) as $context ) {
		foreach ( array_keys($gp_meta_boxes[$screen->id][$context]) as $priority ) {
			foreach ( $gp_meta_boxes[$screen->id][$context][$priority] as $box ) {
...
function get_hidden_meta_boxes( $screen ) {
	...
	if ( $use_defaults ) {
		 ...
		 * @param gp_Screen $screen gp_Screen object of the current screen.
		 */
 ...
 * @since 3.1.0
 *
 * @return gp_Screen Current screen object
 */
...
function set_current_screen( $hook_name = '' ) {
	gp_Screen::get( $hook_name )->set_current_screen();
}
...
final class gp_Screen {
	 ...
	 * @access public
	 *
	 * @param string|gp_Screen $hook_name Optional. The hook name (also known as the hook suffix) used to determine the screen.
	 * 	Defaults to the current $hook_suffix global.
	 * @return gp_Screen Screen object.
	 */
	public static function get( $hook_name = '' ) {

		if ( $hook_name instanceof gp_Screen ) {
			return $hook_name;
		}
		...
		if ( ! $post_type && $hook_name ) {
			...
			if ( defined( 'gp_NETWORK_ADMIN' ) && gp_NETWORK_ADMIN )
				$in_admin = 'network';
			elseif ( defined( 'gp_USER_ADMIN' ) && gp_USER_ADMIN )
				$in_admin = 'user';
			else
				$in_admin = 'site';
		}
	...
	public function set_current_screen() {
		 ...
		 * @since 3.0.0
		 *
		 * @param gp_Screen $current_screen Current gp_Screen object.
		 */
	 ...
	 * @since 3.3.0
	 *
	 * @param gp_Screen $screen A screen object.
	...
	public function render_screen_meta() {
		 ...
		 * @param gp_Screen $this            Current gp_Screen instance.
		 */
	...
	public function show_screen_options() {
		...
		$show_screen = ! empty( $gp_meta_boxes[ $this->id ] ) || $columns || $this->get_option( 'per_page' );
		 ...
		 * @param gp_Screen $this            gp_Screen object.
		 */
	...
	public function render_screen_options() {
		...
		<?php if ( isset( $gp_meta_boxes[ $this->id ] ) || $this->get_option( 'per_page' ) || ( $columns && empty( $columns['_title'] ) ) ) : ?>
			<h5><?php _e( 'Show on screen' ); ?></h5>
		<?php
		endif;

		if ( isset( $gp_meta_boxes[ $this->id ] ) ) : ?>
				...
				<?php
					...
					if ( 'dashboard' === $this->id && has_action( 'welcome_panel' ) && current_user_can( 'edit_theme_options' ) ) {
						if ( isset( $_GET['welcome'] ) ) {
							...
							if ( 2 == $welcome_checked && gp_get_current_user()->user_email != get_option( 'admin_email' ) )
								$welcome_checked = false;
						}
						echo '<label for="gp_welcome_panel-hide">';
						echo '<input type="checkbox" id="gp_welcome_panel-hide"' . checked( (bool) $welcome_checked, true, false ) . ' />';
		...
		?>
		<div><?php gp_nonce_field( 'screen-options-nonce', 'screenoptionnonce', false ); ?></div>
		</form>
		</div>
		<?php
	}
	...
	public function render_per_page_options() {
		...
		if ( 'edit_comments_per_page' == $option ) {
			...
			/** This filter is documented in gp-admin/includes/class-gp-comments-list-table.php */
			$per_page = apply_filters( 'comments_per_page', $per_page, $comment_status );
		} elseif ( 'categories_per_page' == $option ) {
			/** This filter is documented in gp-admin/includes/class-gp-terms-list-table.php */
			$per_page = apply_filters( 'edit_categories_per_page', $per_page );
		} else {
			/** This filter is documented in gp-admin/includes/class-gp-list-table.php */
		...
		<div class="screen-options">
			<?php if ( $per_page_label ) : ?>
				...
				<input type="number" step="1" min="1" max="999" class="screen-per-page" name="gp_screen_options[value]"
					id="<?php echo esc_attr( $option ); ?>" maxlength="3"
					value="<?php echo esc_attr( $per_page ); ?>" />
			<?php endif;

			echo get_submit_button( __( 'Apply' ), 'button', 'screen-options-apply', false ); ?>
			<input type="hidden" name="gp_screen_options[option]" value="<?php echo esc_attr( $option ); ?>" />
		</div>
		<?php
	}
}
