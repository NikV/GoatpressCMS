 ...
 * @access private
 */
class gp_Terms_List_Table extends gp_List_Table {
	...
	public function prepare_items() {
		...
		$search = !empty( $_REQUEST['s'] ) ? trim( gp_unslash( $_REQUEST['s'] ) ) : '';
		...
		if ( !empty( $_REQUEST['orderby'] ) )
			$args['orderby'] = trim( gp_unslash( $_REQUEST['orderby'] ) );

		if ( !empty( $_REQUEST['order'] ) )
			$args['order'] = trim( gp_unslash( $_REQUEST['order'] ) );

		$this->callback_args = $args;

		$this->set_pagination_args( array(
			'total_items' => gp_count_terms( $this->screen->taxonomy, compact( 'search' ) ),
	...
	public function display_rows_or_placeholder() {
		...
		$args = gp_parse_args( $this->callback_args, array(
			'page' => 1,
			'number' => 20,
			'search' => '',
			'hide_empty' => 0
	...
	public function column_name( $tag ) {
		 ...
		 * @since 2.5.0
		 *
		 * @see gp_Terms_List_Table::column_name()
		 *
		...
		if ( current_user_can( $tax->cap->delete_terms ) && $tag->term_id != $default_term )
			$actions['delete'] = "<a class='delete-tag' href='" . gp_nonce_url( "edit-tags.php?action=delete&amp;taxonomy=$taxonomy&amp;tag_ID=$tag->term_id", 'delete-tag_' . $tag->term_id ) . "'>" . __( 'Delete' ) . "</a>";
		if ( $tax->public )
...
?>
	...
	?>
		...
		<p class="inline-edit-save submit">
			...
			<?php gp_nonce_field( 'taxinlineeditnonce', '_inline_edit', false ); ?>
