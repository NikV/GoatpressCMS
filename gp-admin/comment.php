...
case 'editcomment' :
	...
	include( ABSPATH . 'gp-admin/edit-form-comment.php' );
...
case 'spam'    :
	...
	if ( !$comment = get_comment_to_edit( $comment_id ) ) {
		gp_redirect( admin_url('edit-comments.php?error=1') );
		die();
	}

	if ( !current_user_can( 'edit_comment', $comment->comment_ID ) ) {
		gp_redirect( admin_url('edit-comments.php?error=2') );
		die();
	}

	// No need to re-approve/re-trash/re-spam a comment.
	if ( $action == str_replace( '1', 'approve', $comment->comment_approved ) ) {
		gp_redirect( admin_url( 'edit-comments.php?same=' . $comment_id ) );
		die();
 	}
...
case 'unapprovecomment' :
	...
	if ( '' != gp_get_referer() && ! $noredir && false === strpos(gp_get_referer(), 'comment.php') )
		$redir = gp_get_referer();
	elseif ( '' != gp_get_original_referer() && ! $noredir )
		$redir = gp_get_original_referer();
