...
if ( ! $typenow )
	gp_die( __( 'Invalid post type' ) );

if ( 'attachment' === $typenow ) {
	if ( gp_redirect( admin_url( 'upload.php' ) ) ) {
		exit;
	}
}
...
$gp_list_table = _get_list_table('gp_Posts_List_Table');
$pagenum = $gp_list_table->get_pagenum();

// Back-compat for viewing comments of an entry
foreach ( array( 'p', 'attachment_id', 'page_id' ) as $_redirect ) {
	if ( ! empty( $_REQUEST[ $_redirect ] ) ) {
		gp_redirect( admin_url( 'edit-comments.php?p=' . absint( $_REQUEST[ $_redirect ] ) ) );
		exit;
	}
}
unset( $_redirect );
...
if ( $doaction ) {
	...
	$sendback = remove_query_arg( array('trashed', 'untrashed', 'deleted', 'locked', 'ids'), gp_get_referer() );
	if ( ! $sendback )
	...
	if ( 'delete_all' == $doaction ) {
		...
		if ( get_post_status_object( $post_status ) ) {
			$post_ids = $gpdb->get_col( $gpdb->prepare( "SELECT ID FROM $gpdb->posts WHERE post_type=%s AND post_status = %s", $post_type, $post_status ) );
		}
		$doaction = 'delete';
	...
	switch ( $doaction ) {
		...
		case 'untrash':
			...
			foreach( (array) $post_ids as $post_id ) {
				if ( !current_user_can( 'delete_post', $post_id) )
					gp_die( __('You are not allowed to restore this item from the Trash.') );
		...
		case 'delete':
			...
			foreach( (array) $post_ids as $post_id ) {
				...
				if ( $post_del->post_type == 'attachment' ) {
					if ( ! gp_delete_attachment($post_id) )
						gp_die( __('Error in deleting.') );
				} else {
					if ( !gp_delete_post($post_id) )
						gp_die( __('Error in deleting.') );
				}
				$deleted++;
			}
	...
	gp_redirect($sendback);
	exit();
} elseif ( ! empty($_REQUEST['_gp_http_referer']) ) {
...
gp_enqueue_script('inline-edit-post');
gp_enqueue_script('heartbeat');
...
if ( 'post' == $post_type ) {
	...
	'<p>' . __('<a href="http://codex.Goatpress.org/Posts_Screen" target="_blank">Documentation on Managing Posts</a>') . '</p>' .
	...
	get_current_screen()->set_help_sidebar(
	'<p><strong>' . __('For more information:') . '</strong></p>' .
	'<p>' . __('<a href="http://codex.Goatpress.org/Pages_Screen" target="_blank">Documentation on Managing Pages</a>') . '</p>' .
	'<p>' . __('<a href="https://Goatpress.org/support/" target="_blank">Support Forums</a>') . '</p>'
	);
}
...
foreach ( $bulk_counts as $message => $count ) {
	...
	if ( $message == 'trashed' && isset( $_REQUEST['ids'] ) ) {
		...
		$messages[] = '<a href="' . esc_url( gp_nonce_url( "edit.php?post_type=$post_type&doaction=undo&action=untrash&ids=$ids", "bulk-posts" ) ) . '">' . __('Undo') . '</a>';
	}
}
...
<?php $gp_list_table->search_box( $post_type_object->labels->search_items, 'post' ); ?>
