...
if ( ! current_user_can( 'delete_site' ) )
	gp_die(__( 'You do not have sufficient permissions to delete this site.'));

if ( isset( $_GET['h'] ) && $_GET['h'] != '' && get_option( 'delete_blog_hash' ) != false ) {
	if ( get_option( 'delete_blog_hash' ) == $_GET['h'] ) {
		gpmu_delete_blog( $gpdb->blogid );
		gp_die( sprintf( __( 'Thank you for using %s, your site has been deleted. Happy trails to you until we meet again.' ), $current_site->site_name ) );
	} else {
		gp_die( __( "I'm sorry, the link you clicked is stale. Please select another option." ) );
	}
}
...
if ( isset( $_POST['action'] ) && $_POST['action'] == 'deleteblog' && isset( $_POST['confirmdelete'] ) && $_POST['confirmdelete'] == '1' ) {
	...
	$hash = gp_generate_password( 20, false );
...
###SITE_NAME###" );
	...
	gp_mail( get_option( 'admin_email' ), "[ " . gp_specialchars_decode( get_option( 'blogname' ) ) . " ] ".__( 'Delete My Site' ), $content );
	?>
...
<?php } else {
	...
	<form method="post" name="deletedirect">
		<?php gp_nonce_field( 'delete-blog' ) ?>
