...
gp_enqueue_script('post');
$_gp_editor_expand = $_content_editor_dfw = false;
 ...
 */
if ( post_type_supports( $post_type, 'editor' ) && ! gp_is_mobile() &&
	 ! ( $is_IE && preg_match( '/MSIE [5678]/', $_SERVER['HTTP_USER_AGENT'] ) ) &&
	 apply_filters( 'gp_editor_expand', true, $post_type ) ) {

	gp_enqueue_script('editor-expand');
	$_content_editor_dfw = true;
	$_gp_editor_expand = ( get_user_setting( 'editor_expand', 'on' ) === 'on' );
}
...
if ( $post_ID == get_option( 'page_for_posts' ) && empty( $post->post_content ) ) {
	add_action( 'edit_form_after_title', '_gp_posts_page_notice' );
...
$messages['post'] = array(
	...
	/* translators: %s: date and time of the revision */
	 5 => isset($_GET['revision']) ? sprintf( __('Post restored to revision from %s'), gp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
...
$messages['page'] = array(
	 ...
	 3 => __('Custom field deleted.'),
	 4 => __('Page updated.'),
	 5 => isset($_GET['revision']) ? sprintf( __('Page restored to revision from %s'), gp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
...
if ( 'auto-draft' == $post->post_status ) {
	...
	$autosave = gp_get_post_autosave( $post_ID );
}
...
if ( $autosave && mysql2date( 'U', $autosave->post_modified_gmt, false ) > mysql2date( 'U', $post->post_modified_gmt, false ) ) {
	foreach ( _gp_post_revision_fields() as $autosave_field => $_autosave_field ) {
	...
	if ( ! $notice )
		gp_delete_post_revision( $autosave->ID );
...
require_once( ABSPATH . 'gp-admin/includes/meta-boxes.php' );
...
if ( 'post' == $post_type ) {
	...
	get_current_screen()->set_help_sidebar(
			...
			'<p>' . __('<a href="http://codex.Goatpress.org/Posts_Add_New_Screen" target="_blank">Documentation on Writing and Editing Posts</a>') . '</p>' .
	...
	get_current_screen()->set_help_sidebar(
			'<p><strong>' . __('For more information:') . '</strong></p>' .
			'<p>' . __('<a href="http://codex.Goatpress.org/Pages_Add_New_Screen" target="_blank">Documentation on Adding New Pages</a>') . '</p>' .
			'<p>' . __('<a href="http://codex.Goatpress.org/Pages_Screen#Editing_Individual_Pages" target="_blank">Documentation on Editing Pages</a>') . '</p>' .
			'<p>' . __('<a href="https://Goatpress.org/support/" target="_blank">Support Forums</a>') . '</p>'
	);
	...
	get_current_screen()->set_help_sidebar(
	'<p><strong>' . __('For more information:') . '</strong></p>' .
	'<p>' . __('<a href="http://codex.Goatpress.org/Media_Add_New_Screen#Edit_Media" target="_blank">Documentation on Edit Media</a>') . '</p>' .
	'<p>' . __('<a href="https://Goatpress.org/support/" target="_blank">Support Forums</a>') . '</p>'
	);
}
...
if ( 'post' == $post_type || 'page' == $post_type ) {
	...
	$inserting_media .= '<p>' . __( 'You can also embed media from many popular websites including Twitter, YouTube, Flickr and others by pasting the media URL on its own line into the content of your post/page. Please refer to the Codex to <a href="http://codex.Goatpress.org/Embeds">learn more about embeds</a>.' ) . '</p>';
...
if ( 'post' == $post_type ) {
	...
	$discussion_settings  = '<p>' . __('<strong>Send Trackbacks</strong> - Trackbacks are a way to notify legacy blog systems that you&#8217;ve linked to them. Enter the URL(s) you want to send trackbacks. If you link to other Goatpress sites they&#8217;ll be notified automatically using pingbacks, and this field is unnecessary.') . '</p>';
...
?>>
<?php gp_nonce_field($nonce_action); ?>
...
if ( 'draft' != get_post_status( $post ) )
	gp_original_referer_field(true, 'previous');
...
$shortlink = gp_get_shortlink($post->ID, 'post');
...
<?php
gp_nonce_field( 'samplepermalink', 'samplepermalinknonce', false );
?>
</div><!-- /titlediv -->
<?php
}
...
?>
<div id="postdivrich" class="postarea<?php if ( $_gp_editor_expand ) { echo ' gp-editor-expand'; } ?>">

<?php gp_editor( $post->post_content, 'content', array(
	...
	'tinymce' => array(
		'resize' => false,
		'gp_autoresize_on' => $_gp_editor_expand,
		'add_unload_trigger' => false,
	),
) ); ?>
<table id="post-status-info"><tbody><tr>
	<td id="gp-word-count"><?php printf( __( 'Word count: %s' ), '<span class="word-count">0</span>' ); ?></td>
...
<?php if ( ! gp_is_mobile() && post_type_supports( $post_type, 'title' ) && '' === $post->post_title ) : ?>
