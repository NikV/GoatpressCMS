...
class Custom_Background {
	...
	public function __construct($admin_header_callback = '', $admin_image_div_callback = '') {
		...
		add_action( 'gp_ajax_custom-background-add', array( $this, 'ajax_background_add' ) );

		// Unused since 3.5.0.
		add_action( 'gp_ajax_set-background-image', array( $this, 'gp_set_background_image' ) );
	}
	...
	public function admin_load() {
		...
		get_current_screen()->set_help_sidebar(
			...
			'<p>' . __( '<a href="http://codex.Goatpress.org/Appearance_Background_Screen" target="_blank">Documentation on Custom Background</a>' ) . '</p>' .
			'<p>' . __( '<a href="https://Goatpress.org/support/" target="_blank">Support Forums</a>' ) . '</p>'
		);

		gp_enqueue_media();
		gp_enqueue_script('custom-background');
	...
	public function take_action() {
		...
		if ( isset($_POST['reset-background']) ) {
			check_admin_referer('custom-background-reset', '_gpnonce-custom-background-reset');
		...
		if ( isset($_POST['remove-background']) ) {
			...
			check_admin_referer('custom-background-remove', '_gpnonce-custom-background-remove');
			set_theme_mod('background_image', '');
			set_theme_mod('background_image_thumb', '');
			$this->updated = true;
			gp_safe_redirect( $_POST['_gp_http_referer'] );
			return;
		}
...
<td>
<form method="post">
<?php gp_nonce_field('custom-background-remove', '_gpnonce-custom-background-remove'); ?>
...
<td>
<form method="post">
<?php gp_nonce_field('custom-background-reset', '_gpnonce-custom-background-reset'); ?>
...
<td><form enctype="multipart/form-data" id="upload-form" class="gp-upload-form" method="post">
	<p>
		<label for="upload"><?php _e( 'Choose an image from your computer:' ); ?></label><br />
		<input type="file" id="upload" name="import" />
		<input type="hidden" name="action" value="save" />
		<?php gp_nonce_field( 'custom-background-upload', '_gpnonce-custom-background-upload' ); ?>
...
<?php gp_nonce_field('custom-background'); ?>
...
<?php
	...
	public function handle_upload() {
		...
		check_admin_referer('custom-background-upload', '_gpnonce-custom-background-upload');
		...
		// Save the data
		$id = gp_insert_attachment($object, $file);

		// Add the meta-data
		gp_update_attachment_metadata( $id, gp_generate_attachment_metadata( $id, $file ) );
		update_post_meta( $id, '_gp_attachment_is_custom_background', get_option('stylesheet' ) );

		set_theme_mod('background_image', esc_url_raw($url));

		$thumbnail = gp_get_attachment_image_src( $id, 'thumbnail' );
		set_theme_mod('background_image_thumb', esc_url_raw( $thumbnail[0] ) );

		/** This action is documented in gp-admin/custom-header.php */
		do_action( 'gp_create_file_in_uploads', $file, $id ); // For replication
		$this->updated = true;
	}
	...
	public function ajax_background_add() {
		...
		update_post_meta( $attachment_id, '_gp_attachment_is_custom_background', get_stylesheet() );
	 ...
	 * @deprecated 3.5.0
	 */
	public function gp_set_background_image() {
		...
		update_post_meta( $attachment_id, '_gp_attachment_is_custom_background', get_option('stylesheet' ) );
		$url = gp_get_attachment_image_src( $attachment_id, $size );
		$thumbnail = gp_get_attachment_image_src( $attachment_id, 'thumbnail' );
