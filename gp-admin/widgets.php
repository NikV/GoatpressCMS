...
/** Goatpress Administration Widgets API */
...
function gp_widgets_access_body_class($classes) {
	return "$classes widgets_access ";
}

if ( 'on' == $widgets_access ) {
	add_filter( 'admin_body_class', 'gp_widgets_access_body_class' );
} else {
	gp_enqueue_script('admin-widgets');
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __('<a href="http://codex.Goatpress.org/Appearance_Widgets_Screen" target="_blank">Documentation on Widgets</a>') . '</p>' .
	'<p>' . __('<a href="https://Goatpress.org/support/" target="_blank">Support Forums</a>') . '</p>'
);

if ( ! current_theme_supports( 'widgets' ) ) {
	gp_die( __( 'The theme you are currently using isn&#8217;t widget-aware, meaning that it has no sidebars that you are able to change. For information on making your theme widget-aware, please <a href="http://codex.Goatpress.org/Widgetizing_Themes">follow these instructions</a>.' ) );
}
...
if ( empty( $sidebars_widgets ) )
	$sidebars_widgets = gp_get_widget_defaults();

foreach ( $sidebars_widgets as $sidebar_id => $widgets ) {
	if ( 'gp_inactive_widgets' == $sidebar_id )
		continue;

	if ( !isset( $gp_registered_sidebars[ $sidebar_id ] ) ) {
...
if ( isset($_POST['savewidget']) || isset($_POST['removewidget']) ) {
	...
	if ( isset($_POST['removewidget']) && $_POST['removewidget'] ) {
		...
		if ( !in_array($widget_id, $sidebar, true) ) {
			gp_redirect( admin_url('widgets.php?error=0') );
			exit;
		}
	...
	gp_redirect( admin_url('widgets.php?message=0') );
	exit;
}
...
if ( isset($_GET['editwidget']) && $_GET['editwidget'] ) {
	...
	if ( isset($_GET['addnew']) ) {
		...
		$keys = array_keys( $gp_registered_sidebars );
		$sidebar = reset( $keys );

		if ( isset($_GET['base']) && isset($_GET['num']) ) { // multi-widget
			// Copy minimal info from an existing instance of this widget to a new instance
			foreach ( $gp_registered_widget_controls as $control ) {
				if ( $_GET['base'] === $control['id_base'] ) {
					$control_callback = $control['callback'];
					$multi_number = (int) $_GET['num'];
					$control['params'][0]['number'] = -1;
					$widget_id = $control['id'] = $control['id_base'] . '-' . $multi_number;
					$gp_registered_widget_controls[$control['id']] = $control;
					break;
				}
			}
		}
	...
	if ( isset($gp_registered_widget_controls[$widget_id]) && !isset($control) ) {
		$control = $gp_registered_widget_controls[$widget_id];
		$control_callback = $control['callback'];
	} elseif ( !isset($gp_registered_widget_controls[$widget_id]) && isset($gp_registered_widgets[$widget_id]) ) {
		$name = esc_html( strip_tags($gp_registered_widgets[$widget_id]['name']) );
	}
	...
	if ( !isset($sidebar) )
		$sidebar = isset($_GET['sidebar']) ? $_GET['sidebar'] : 'gp_inactive_widgets';
...
<?php
	foreach ( $gp_registered_sidebars as $sbname => $sbvalue ) {
		echo "\t\t<tr><td><label><input type='radio' name='sidebar' value='" . esc_attr($sbname) . "'" . checked( $sbname, $sidebar, false ) . " /> $sbvalue[name]</label></td><td>";
		if ( 'gp_inactive_widgets' == $sbname || 'orphaned_widgets' == substr( $sbname, 0, 16 ) ) {
			echo '&nbsp;';
		} else {
...
<?php	gp_nonce_field("save-delete-widget-$widget_id"); ?>
	<br class="clear" />
	</div>
	</form>
	</div>
		...
		printf(
			...
			esc_url( add_query_arg(
				array(
					...
					'return' => urlencode( gp_unslash( $_SERVER['REQUEST_URI'] ) )
				),
...
foreach ( $gp_registered_sidebars as $sidebar => $registered_sidebar ) {
	if ( false !== strpos( $registered_sidebar['class'], 'inactive-sidebar' ) || 'orphaned_widgets' == substr( $sidebar, 0, 16 ) ) {
		...
		<div class="<?php echo esc_attr( $wrap_class ); ?>">
			<div class="widget-holder inactive">
				<?php gp_list_widget_controls( $registered_sidebar['id'], $registered_sidebar['name'] ); ?>
				<div class="clear"></div>
			</div>
		</div>
		<?php
...
foreach ( $theme_sidebars as $sidebar => $registered_sidebar ) {
	...
	?>
	<div class="<?php echo esc_attr( $wrap_class ); ?>">
		<?php gp_list_widget_controls( $sidebar, $registered_sidebar['name'] ); // Show the control forms for each of the widgets in this sidebar ?>
	</div>
	<?php
...
<form method="post">
<?php gp_nonce_field( 'save-sidebar-widgets', '_gpnonce_widgets', false ); ?>
</form>
<br class="clear" />
</div>
