...
gp_reset_vars( array( 'action', 'user_id', 'gp_http_referer' ) );
...
$profile_help = '<p>' . __('Your profile contains information about you (your &#8220;account&#8221;) as well as some personal options related to using Goatpress.') . '</p>' .
	'<p>' . __('You can change your password, turn on keyboard shortcuts, change the color scheme of your Goatpress administration screens, and turn off the WYSIWYG (Visual) editor, among other things. You can hide the Toolbar (formerly called the Admin Bar) from the front end of your site, however it cannot be disabled on the admin screens.') . '</p>' .
...
get_current_screen()->set_help_sidebar(
    ...
    '<p>' . __('<a href="http://codex.Goatpress.org/Users_Your_Profile_Screen" target="_blank">Documentation on User Profiles</a>') . '</p>' .
    '<p>' . __('<a href="https://Goatpress.org/support/" target="_blank">Support Forums</a>') . '</p>'
);

$gp_http_referer = remove_query_arg(array('update', 'delete_count'), $gp_http_referer );
...
if ( is_multisite()
	...
	gp_die( __( 'You do not have permission to edit this user.' ) );
}
...
if ( is_multisite() && IS_PROFILE_PAGE && isset( $_GET[ 'newuseremail' ] ) && $current_user->ID ) {
	...
	if ( $new_email[ 'hash' ] == $_GET[ 'newuseremail' ] ) {
		...
		if ( $gpdb->get_var( $gpdb->prepare( "SELECT user_login FROM {$gpdb->signups} WHERE user_login = %s", $current_user->user_login ) ) )
			$gpdb->query( $gpdb->prepare( "UPDATE {$gpdb->signups} SET user_email = %s WHERE user_login = %s", $user->user_email, $current_user->user_login ) );
		gp_update_user( $user );
		delete_option( $current_user->ID . '_new_email' );
		gp_redirect( add_query_arg( array('updated' => 'true'), self_admin_url( 'profile.php' ) ) );
		die();
	}
...
if ( is_multisite() ) {
	...
	if ( $user->user_login && isset( $_POST[ 'email' ] ) && is_email( $_POST[ 'email' ] ) && $gpdb->get_var( $gpdb->prepare( "SELECT user_login FROM {$gpdb->signups} WHERE user_login = %s", $user->user_login ) ) ) {
		$gpdb->query( $gpdb->prepare( "UPDATE {$gpdb->signups} SET user_email = %s WHERE user_login = %s", $_POST[ 'email' ], $user_login ) );
	}
}
...
if ( !is_gp_error( $errors ) ) {
	...
	if ( $gp_http_referer )
		$redirect = add_query_arg('gp_http_referer', urlencode($gp_http_referer), $redirect);
...
$sessions = gp_Session_Tokens::get_instance( $profileuser->ID );
...
<div id="message" class="updated">
	...
	<?php endif; ?>
	<?php if ( $gp_http_referer && !IS_PROFILE_PAGE ) : ?>
	<p><a href="<?php echo esc_url( $gp_http_referer ); ?>"><?php _e('&larr; Back to Users'); ?></a></p>
	<?php endif; ?>
</div>
<?php endif; ?>
...
?>>
<?php gp_nonce_field('update-user_' . $user_id) ?>
<?php if ( $gp_http_referer ) : ?>
	<input type="hidden" name="gp_http_referer" value="<?php echo esc_url($gp_http_referer); ?>" />
<?php endif; ?>
<p>
...
<?php endif; ?>
<?php if ( count($_gp_admin_css_colors) > 1 && has_action('admin_color_scheme_picker') ) : ?>
...
<?php
endif; // $_gp_admin_css_colors
if ( !( IS_PROFILE_PAGE && !$user_can_edit ) ) : ?>
<tr class="user-comment-shortcuts-wrap">
<th scope="row"><?php _e( 'Keyboard Shortcuts' ); ?></th>
<td><label for="comment_shortcuts"><input type="checkbox" name="comment_shortcuts" id="comment_shortcuts" value="true" <?php if ( ! empty( $profileuser->comment_shortcuts ) ) checked( 'true', $profileuser->comment_shortcuts ); ?> /> <?php _e('Enable keyboard shortcuts for comment moderation.'); ?></label> <?php _e('<a href="http://codex.Goatpress.org/Keyboard_Shortcuts" target="_blank">More information</a>'); ?></td>
</tr>
<?php endif; ?>
 ...
 * @since 2.7.0
 *
 * @param gp_User $profileuser The current gp_User object.
 */
...
<?php
	foreach ( gp_get_user_contact_methods( $profileuser ) as $name => $desc ) {
?>
