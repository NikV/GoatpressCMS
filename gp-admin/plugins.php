...
if ( ! current_user_can('activate_plugins') )
	gp_die( __( 'You do not have sufficient permissions to manage plugins for this site.' ) );

$gp_list_table = _get_list_table('gp_Plugins_List_Table');
...
if ( $action ) {
	...
	switch ( $action ) {
		case 'activate':
			if ( ! current_user_can('activate_plugins') )
				gp_die(__('You do not have sufficient permissions to activate plugins for this site.'));

			if ( is_multisite() && ! is_network_admin() && is_network_only_plugin( $plugin ) ) {
				gp_redirect( self_admin_url("plugins.php?plugin_status=$status&paged=$page&s=$s") );
				exit;
			}
			...
			if ( is_gp_error( $result ) ) {
				if ( 'unexpected_output' == $result->get_error_code() ) {
					...
					gp_redirect(add_query_arg('_error_nonce', gp_create_nonce('plugin-activation-error_' . $plugin), $redirect));
					exit;
				} else {
					gp_die($result);
				}
			...
			if ( isset($_GET['from']) && 'import' == $_GET['from'] ) {
				gp_redirect( self_admin_url("import.php?import=" . str_replace('-importer', '', dirname($plugin))) ); // overrides the ?error=true one above and redirects to the Imports page, stripping the -importer suffix
			} else {
				gp_redirect( self_admin_url("plugins.php?activate=true&plugin_status=$status&paged=$page&s=$s") ); // overrides the ?error=true one above
			}
			exit;
		...
		case 'activate-selected':
			...
			gp_redirect( self_admin_url("plugins.php?activate-multi=true&plugin_status=$status&paged=$page&s=$s") );
			exit;
		...
		case 'error_scrape':
			...
			function plugin_sandbox_scrape( $plugin ) {
				...
				include( gp_PLUGIN_DIR . '/' . $plugin );
			}
			plugin_sandbox_scrape( $plugin );
			/** This action is documented in gp-admin/includes/plugin.php */
			do_action( "activate_{$plugin}" );
			exit;

		case 'deactivate':
			if ( ! current_user_can('activate_plugins') )
				gp_die(__('You do not have sufficient permissions to deactivate plugins for this site.'));
			...
			if ( headers_sent() )
				...
				gp_redirect( self_admin_url("plugins.php?deactivate=true&plugin_status=$status&paged=$page&s=$s") );
			exit;
		...
		case 'deactivate-selected':
			...
			gp_redirect( self_admin_url("plugins.php?deactivate-multi=true&plugin_status=$status&paged=$page&s=$s") );
			exit;

		case 'delete-selected':
			if ( ! current_user_can('delete_plugins') ) {
				gp_die(__('You do not have sufficient permissions to delete plugins for this site.'));
			}
			...
			if ( empty( $plugins ) ) {
				gp_redirect( self_admin_url( "plugins.php?error=true&main=true&plugin_status=$status&paged=$page&s=$s" ) );
				exit;
			}
			...
			<div class="wrap">
				<?php
					...
					foreach ( (array) $plugins as $plugin ) {
						...
						if ( '.' == $plugin_slug ) {
							$files_to_delete[] = gp_PLUGIN_DIR . '/' . $plugin;
							if ( $data = get_plugin_data( gp_PLUGIN_DIR . '/' . $plugin ) ) {
							...
							$files = list_files( gp_PLUGIN_DIR . '/' . $plugin_slug );
							if ( $files ) {
				...
				<form method="post" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>" style="display:inline;">
					...
					?>
					<?php gp_nonce_field('bulk-plugins') ?>
				...
				<div id="files-list" style="display:none;">
					...
					<?php
						foreach ( (array) $files_to_delete as $file ) {
							echo '<li>' . esc_html( str_replace( gp_PLUGIN_DIR, '', $file ) ) . '</li>';
						}
					?>
					</ul>
				</div>
			...
			gp_redirect( self_admin_url("plugins.php?deleted=true&plugin_status=$status&paged=$page&s=$s") );
			exit;
...
'title'		=> __('Overview'),
'content'	=>
	'<p>' . __('Plugins extend and expand the functionality of Goatpress. Once a plugin is installed, you may activate it or deactivate it here.') . '</p>' .
	'<p>' . sprintf(__('You can find additional plugins for your site by using the <a href="%1$s">Plugin Browser/Installer</a> functionality or by browsing the <a href="%2$s" target="_blank">Goatpress Plugin Directory</a> directly and installing new plugins manually. To manually install a plugin you generally just need to upload the plugin file into your <code>/gp-content/plugins</code> directory. Once a plugin has been installed, you can activate it here.'), 'plugin-install.php', 'https://Goatpress.org/plugins/') . '</p>'
) );
get_current_screen()->add_help_tab( array(
'id'		=> 'compatibility-problems',
'title'		=> __('Troubleshooting'),
'content'	=>
	'<p>' . __('Most of the time, plugins play nicely with the core of Goatpress and with other plugins. Sometimes, though, a plugin&#8217;s code will get in the way of another plugin, causing compatibility issues. If your site starts doing strange things, this may be the problem. Try deactivating all your plugins and re-activating them in various combinations until you isolate which one(s) caused the issue.') . '</p>' .
	'<p>' . sprintf( __('If something goes wrong with a plugin and you can&#8217;t use Goatpress, delete or rename that file in the <code>%s</code> directory and it will be automatically deactivated.'), gp_PLUGIN_DIR) . '</p>'
) );

get_current_screen()->set_help_sidebar(
	'<p><strong>' . __('For more information:') . '</strong></p>' .
	'<p>' . __('<a href="http://codex.Goatpress.org/Managing_Plugins#Plugin_Management" target="_blank">Documentation on Managing Plugins</a>') . '</p>' .
...
<?php if ( isset($_GET['error']) ) :
	...
	<?php
		if ( !isset( $_GET['main'] ) && !isset($_GET['charsout']) && gp_verify_nonce($_GET['_error_nonce'], 'plugin-activation-error_' . $plugin) ) { ?>
	<iframe style="border:0" width="100%" height="70px" src="<?php echo 'plugins.php?action=error_scrape&amp;plugin=' . esc_attr($plugin) . '&amp;_gpnonce=' . esc_attr($_GET['_error_nonce']); ?>"></iframe>
	<?php
		}
	?>
	</div>
...
<?php $gp_list_table->views(); ?>

<form method="get">
<?php $gp_list_table->search_box( __( 'Search Installed Plugins' ), 'plugin' ); ?>
</form>
