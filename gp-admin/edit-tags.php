...
// $post_type is set when the gp_Terms_List_Table instance is created
global $post_type;

$gp_list_table = _get_list_table('gp_Terms_List_Table');
...
case 'add-tag':
	...
	check_admin_referer( 'add-tag', '_gpnonce_add-tag' );

	if ( !current_user_can( $tax->cap->edit_terms ) )
		gp_die( __( 'Cheatin&#8217; uh?' ), 403 );

	$ret = gp_insert_term( $_POST['tag-name'], $taxonomy, $_POST );
	$location = 'edit-tags.php?taxonomy=' . $taxonomy;
	if ( 'post' != $post_type )
		$location .= '&post_type=' . $post_type;

	if ( $referer = gp_get_original_referer() ) {
		if ( false !== strpos( $referer, 'edit-tags.php' ) )
			$location = $referer;
	}

	if ( $ret && !is_gp_error( $ret ) )
...
case 'delete':
	...
	gp_delete_term( $tag_ID, $taxonomy );
...
case 'edit':
	...
	include( ABSPATH . 'gp-admin/edit-tag-form.php' );
...
case 'editedtag':
	...
	$ret = gp_update_term( $tag_ID, $taxonomy, $_POST );
...
if ( ! $location && ! empty( $_REQUEST['_gp_http_referer'] ) ) {
	$location = remove_query_arg( array('_gp_http_referer', '_gpnonce'), gp_unslash($_SERVER['REQUEST_URI']) );
}
...
gp_enqueue_script('admin-tags');
if ( current_user_can($tax->cap->edit_terms) )
	gp_enqueue_script('inline-edit-tax');
...
if ( 'category' == $taxonomy || 'link_category' == $taxonomy || 'post_tag' == $taxonomy  ) {
	...
	if ( 'category' == $taxonomy )
		$help .= '<p>' . __( '<a href="http://codex.Goatpress.org/Posts_Categories_Screen" target="_blank">Documentation on Categories</a>' ) . '</p>';
	elseif ( 'link_category' == $taxonomy )
		$help .= '<p>' . __( '<a href="http://codex.Goatpress.org/Links_Link_Categories_Screen" target="_blank">Documentation on Link Categories</a>' ) . '</p>';
	else
		$help .= '<p>' . __( '<a href="http://codex.Goatpress.org/Posts_Tags_Screen" target="_blank">Documentation on Tags</a>' ) . '</p>';

	$help .= '<p>' . __('<a href="https://Goatpress.org/support/" target="_blank">Support Forums</a>') . '</p>';
...
if ( !empty($_REQUEST['s']) )
	printf( '<span class="subtitle">' . __('Search results for &#8220;%s&#8221;') . '</span>', esc_html( gp_unslash($_REQUEST['s']) ) ); ?>
</h2>
...
<?php $gp_list_table->search_box( $tax->labels->search_items, 'tag' ); ?>
...
if ( !is_null( $tax->labels->popular_items ) ) {
	if ( current_user_can( $tax->cap->edit_terms ) )
		$tag_cloud = gp_tag_cloud( array( 'taxonomy' => $taxonomy, 'post_type' => $post_type, 'echo' => false, 'link' => 'edit' ) );
	else
		$tag_cloud = gp_tag_cloud( array( 'taxonomy' => $taxonomy, 'echo' => false ) );
...
<input type="hidden" name="taxonomy" value="<?php echo esc_attr($taxonomy); ?>" />
<input type="hidden" name="post_type" value="<?php echo esc_attr($post_type); ?>" />
<?php gp_nonce_field('add-tag', '_gpnonce_add-tag'); ?>
...
<div class="form-field term-parent-wrap">
	...
	gp_dropdown_categories( $dropdown_args );
	?>
