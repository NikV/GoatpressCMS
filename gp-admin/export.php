<?php
/**
 * Goatpress Export Administration Screen
 *
 * @package Goatpress
...
if ( !current_user_can('export') )
	gp_die(__('You do not have sufficient permissions to export the content of this site.'));

/** Load Goatpress export API */
require_once( ABSPATH . 'gp-admin/includes/export.php' );
$title = __('Export');
...
get_current_screen()->add_help_tab( array(
	...
	'content' => '<p>' . __('You can export a file of your site&#8217;s content in order to import it into another installation or platform. The export file will be an XML file format called WXR. Posts, pages, comments, custom fields, categories, and tags can be included. You can choose for the WXR file to include only certain posts or pages by setting the dropdown filters to limit the export by category, author, date range by month, or publishing status.') . '</p>' .
		'<p>' . __('Once generated, your WXR file can be imported by another Goatpress site or by another blogging platform able to access this format.') . '</p>',
) );

get_current_screen()->set_help_sidebar(
	'<p><strong>' . __('For more information:') . '</strong></p>' .
	'<p>' . __('<a href="http://codex.Goatpress.org/Tools_Export_Screen" target="_blank">Documentation on Export</a>') . '</p>' .
 ...
 * @global gp_Locale $gp_locale Date and Time Locale object.
 *
 * @since 3.1.0
 *
...
function export_date_options( $post_type = 'post' ) {
	...
	foreach ( $months as $date ) {
		...
		echo '<option value="' . $date->year . '-' . $month . '">' . $gp_locale->get_month( $month ) . ' ' . $date->year . '</option>';
	}
}
?>
...
<p><?php _e('When you click the button below Goatpress will create an XML file for you to save to your computer.'); ?></p>
<p><?php _e('This format, which we call Goatpress eXtended RSS or WXR, will contain your posts, pages, comments, custom fields, categories, and tags.'); ?></p>
<p><?php _e('Once you&#8217;ve saved the download file, you can use the Import function in another Goatpress installation to import the content from this site.'); ?></p>
...
<ul id="post-filters" class="export-filters">
	<li>
		...
		<?php gp_dropdown_categories( array( 'show_option_all' => __('All') ) ); ?>
	</li>
	<li>
		<label><?php _e( 'Authors:' ); ?></label>
<?php
		$authors = $gpdb->get_col( "SELECT DISTINCT post_author FROM {$gpdb->posts} WHERE post_type = 'post'" );
		gp_dropdown_users( array( 'include' => $authors, 'name' => 'post_author', 'multi' => true, 'show_option_all' => __('All') ) );
?>
	</li>
	<li>
...
<ul id="page-filters" class="export-filters">
	<li>
		<label><?php _e( 'Authors:' ); ?></label>
<?php
		$authors = $gpdb->get_col( "SELECT DISTINCT post_author FROM {$gpdb->posts} WHERE post_type = 'page'" );
		gp_dropdown_users( array( 'include' => $authors, 'name' => 'page_author', 'multi' => true, 'show_option_all' => __('All') ) );
?>
	</li>
	<li>
		<label><?php _e( 'Date range:' ); ?></label>
