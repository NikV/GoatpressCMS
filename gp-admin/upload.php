...
if ( !current_user_can('upload_files') )
	gp_die( __( 'You do not have permission to upload files.' ) );
...
if ( 'grid' === $mode ) {
	gp_enqueue_media();
	gp_enqueue_script( 'media-grid' );
	gp_enqueue_script( 'media' );

	$q = $_GET;
	// let JS handle this
	unset( $q['s'] );
	$vars = gp_edit_attachments_query_vars( $q );
	...
	gp_localize_script( 'media-grid', '_gpMediaGridSettings', array(
	...
	get_current_screen()->set_help_sidebar(
		...
		'<p>' . __( '<a href="http://codex.Goatpress.org/Media_Library_Screen" target="_blank">Documentation on Media Library</a>' ) . '</p>' .
	...
	?>
	<div class="wrap" id="gp-media-grid" data-search="<?php _admin_search_query() ?>">
		<h2>
		<?php
		echo esc_html( $title );
		if ( current_user_can( 'upload_files' ) ) { ?>
...
$gp_list_table = _get_list_table('gp_Media_List_Table');
...
if ( $doaction ) {
	...
	if ( 'delete_all' == $doaction ) {
		$post_ids = $gpdb->get_col( "SELECT ID FROM $gpdb->posts WHERE post_type='attachment' AND post_status = 'trash'" );
		$doaction = 'delete';
	...
	switch ( $doaction ) {
		case 'detach':
			gp_media_attach_action( $_REQUEST['parent_post_id'], 'detach' );
			break;

		case 'attach':
			gp_media_attach_action( $_REQUEST['found_post_id'] );
			break;
		...
		case 'trash':
			...
			foreach ( (array) $post_ids as $post_id ) {
				if ( !current_user_can( 'delete_post', $post_id ) )
					gp_die( __( 'You are not allowed to move this post to the trash.' ) );
		...
		case 'untrash':
			...
			foreach ( (array) $post_ids as $post_id ) {
				if ( !current_user_can( 'delete_post', $post_id ) )
					gp_die( __( 'You are not allowed to move this post out of the trash.' ) );
		...
		case 'delete':
			...
			foreach ( (array) $post_ids as $post_id_del ) {
				if ( !current_user_can( 'delete_post', $post_id_del ) )
					gp_die( __( 'You are not allowed to delete this post.' ) );

				if ( !gp_delete_attachment( $post_id_del ) )
...
if ( ! empty( $_GET['trashed'] ) && $trashed = absint( $_GET['trashed'] ) ) {
	...
	$message .= ' <a href="' . esc_url( gp_nonce_url( 'upload.php?doaction=undo&action=untrash&ids='.(isset($_GET['ids']) ? $_GET['ids'] : ''), "bulk-media" ) ) . '">' . __('Undo') . '</a>';
...
$messages[4] = __('Media moved to the trash.') . ' <a href="' . esc_url( gp_nonce_url( 'upload.php?doaction=undo&action=untrash&ids='.(isset($_GET['ids']) ? $_GET['ids'] : ''), "bulk-media" ) ) . '">' . __('Undo') . '</a>';
