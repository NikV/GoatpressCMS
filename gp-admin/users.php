...
get_current_screen()->set_help_sidebar(
    ...
    '<p>' . __('<a href="http://codex.Goatpress.org/Users_Screen" target="_blank">Documentation on Managing Users</a>') . '</p>' .
    '<p>' . __('<a href="http://codex.Goatpress.org/Roles_and_Capabilities" target="_blank">Descriptions of Roles and Capabilities</a>') . '</p>' .
    '<p>' . __('<a href="https://Goatpress.org/support/" target="_blank">Support Forums</a>') . '</p>'
);

if ( empty($_REQUEST) ) {
	$referer = '<input type="hidden" name="gp_http_referer" value="'. esc_attr( gp_unslash( $_SERVER['REQUEST_URI'] ) ) . '" />';
} elseif ( isset($_REQUEST['gp_http_referer']) ) {
	$redirect = remove_query_arg(array('gp_http_referer', 'updated', 'delete_count'), gp_unslash( $_REQUEST['gp_http_referer'] ) );
	$referer = '<input type="hidden" name="gp_http_referer" value="' . esc_attr($redirect) . '" />';
} else {
	$redirect = 'users.php';
	$referer = '';
}
...
switch ( $gp_list_table->current_action() ) {
...
case 'promote':
	...
	if ( ! current_user_can( 'promote_users' ) )
		gp_die( __( 'You can&#8217;t edit that user.' ) );
	...
	foreach ( $userids as $id ) {
		...
		if ( $id == $current_user->ID && ! $gp_roles->role_objects[ $_REQUEST['new_role'] ]->has_cap('promote_users')
	...
	gp_redirect(add_query_arg('update', $update, $redirect));
	exit();

case 'dodelete':
	if ( is_multisite() )
		gp_die( __('User deletion is not allowed from this screen.') );
	...
	if ( empty( $_REQUEST['delete_option'] ) ) {
		...
		$url = str_replace( '&amp;', '&', gp_nonce_url( $url, 'bulk-users' ) );
		gp_redirect( $url );
		exit;
	}

	if ( ! current_user_can( 'delete_users' ) )
		gp_die(__('You can&#8217;t delete users.'));
	...
	foreach ( $userids as $id ) {
		if ( ! current_user_can( 'delete_user', $id ) )
			gp_die(__( 'You can&#8217;t delete that user.' ) );
		...
		case 'delete':
			...
			break;
		case 'reassign':
			gp_delete_user( $id, $_REQUEST['reassign_user'] );
			break;
		}
		++$delete_count;
	}
...
case 'delete':
	...
	if ( ! current_user_can( 'delete_users' ) )
		$errors = new gp_Error( 'edit_users', __( 'You can&#8217;t delete users.' ) );
...
<?php gp_nonce_field('delete-users') ?>
<?php echo $referer; ?>
...
<?php if ( $go_delete ) : ?>
	...
	<ul style="list-style:none;">
		...
		gp_dropdown_users( array( 'name' => 'reassign_user', 'exclude' => array_diff( $userids, array($current_user->ID) ) ) ); ?></li>
	</ul></fieldset>
	<?php
	/**
	 ...
	 * @since 4.0.0
	 *
	 * @param gp_User $current_user gp_User object for the user being deleted.
	 */
...
case 'doremove':
	...
	if ( ! is_multisite() )
		gp_die( __( 'You can&#8217;t remove users.' ) );
...
case 'remove':
	...
	if ( !current_user_can('remove_users') )
		$error = new gp_Error('edit_users', __('You can&#8217;t remove users.'));
...
?>
<form method="post" name="updateusers" id="updateusers">
<?php gp_nonce_field('remove-users') ?>
<?php echo $referer; ?>
...
default:
	...
	if ( !empty($_GET['_gp_http_referer']) ) {
		...
		case 'add':
			if ( isset( $_GET['id'] ) && ( $user_id = $_GET['id'] ) && current_user_can( 'edit_user', $user_id ) ) {
				$messages[] = '<div id="message" class="updated"><p>' . sprintf( __( 'New user created. <a href="%s">Edit user</a>' ),
					esc_url( add_query_arg( 'gp_http_referer', urlencode( gp_unslash( $_SERVER['REQUEST_URI'] ) ),
...
<?php $gp_list_table->search_box( __( 'Search Users' ), 'user' ); ?>
